...
/**
     * Array length pair
     */
template <typename T, typename L = std::size_t> struct Span {
	...
	template <typename U, typename M,
		  typename = typename std::enable_if<
			  std::is_same<value_type, U>::value &&
	...
	template <typename C, //
		  ...
		  typename = typename std::enable_if<!std::is_same<
			  typename std::remove_cv<C>::type, Span>::value>::type>
	Span(C &container)
		: data{ container.data() }, length{ container.size() }
	{
	}

	template <L Size> Span(T (&array)[Size]) : data{ array }, length{ Size }
	{
	}
};

template <typename T, typename L> T *begin(Span<T, L> span)
{
	return span.data;
}

template <typename T, typename L> T *end(Span<T, L> span)
{
