...
using unique_ptr = std::unique_ptr<
	T,
	std::function<void(typename std::conditional<
...
template <typename T> T *allocate(std::size_t n)
{
	void *const allocation =
		mmap(nullptr, sizeof(T) * n, PROT_READ | PROT_WRITE,
...
unique_ptr<T> make_unique(Args &&... args)
{
	...
	return unique_ptr<T>{ uninitialized_ptr.release(),
			      initialized_deleter };
}
...
unique_ptr<T> make_unique(std::size_t size)
{
	...
	const std::function<void(element_type *)> uninitialized_deleter =
		[size, &number_initialized](element_type *p) {
	...
	const std::function<void(element_type *)> initialized_deleter =
		[size](element_type *p) {
