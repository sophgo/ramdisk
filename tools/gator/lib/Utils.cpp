...
int writeCStringToFile(const char *fullpath, const char *data)
{
	...
	if (fsEntry.canAccess(false, true, false)) {
		if (lib::writeFileContents(fsEntry, data))
			...
			logg.logMessage("Opened but could not write to %s",
					fullpath);
			return -1;
		}
	} else
...
std::set<int> readCpuMaskFromFile(const char *path)
{
	...
	if (fsEntry.canAccess(true, false, false)) {
		...
		logg.logMessage("Reading cpumask from %s",
				fsEntry.path().c_str());
		...
		while (to < length) {
			...
			if (from < to) {
				if (split > from) {
					...
					contents[split] = 0;
					contents[to] = 0;
					int nf = (int)std::strtol(
						contents.c_str() + from,
						nullptr, 10);
					const int nt = (int)std::strtol(
						contents.c_str() + split + 1,
						nullptr, 10);
					while (nf <= nt) {
						logg.logMessage(
							"    Adding cpu %d to mask",
							nf);
						result.insert(nf);
						nf += 1;
					}
					...
					// found single item
					contents[to] = 0;
					const int n = (int)std::strtol(
						contents.c_str() + from,
						nullptr, 10);
					logg.logMessage(
						"    Adding cpu %d to mask", n);
					result.insert(n);
				}
			}
