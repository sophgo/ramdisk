...
Optional<FsEntry> FsEntryDirectoryIterator::next()
{
	if (directory_ != nullptr) {
		...
		if (entry != nullptr) {
			...
			return FsEntry(parent_->path().append("/").append(
				entry->d_name));
		}
	}
...
FsEntry::Stats::Stats(Type t, bool e, bool s)
	: type_(t), exists_(e), symlink_(s)
{
}

FsEntry::FsEntry(const std::string &p)
	: path_(p), name_offset(std::string::npos)
{
	...
	for (auto pos = path_.find("//"); pos != std::string::npos;
	     pos = path_.find("//")) {
		path_.replace(pos, 2, "/");
	}
	...
	runtime_assert((path_.length() > 0) && (path_[0] == '/'),
...
{
	std::unique_ptr<char[], void (*)(void *)> real_path{
...
{
	const int mode = F_OK | (read ? R_OK : 0) | (write ? W_OK : 0) |
			 (exec ? X_OK : 0);
