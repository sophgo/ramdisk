...
/**
     ...
     */
class WaitForProcessPollerPass
	: public lnx::ProcessPollerBase::IProcessPollerReceiver {
    public:
	WaitForProcessPollerPass(const std::string &commandName,
				 const lib::Optional<lib::FsEntry> &realPath)
	...
	bool shouldTrack(const lib::FsEntry &path) const
	{
		if (!mCommandName.empty()) {
			const auto cmdlineFile =
				lib::FsEntry::create(path, "cmdline");
		...
		if (mRealPath) {
			...
			if (realPath && (*mRealPath == *realPath)) {
				logg.logMessage(
					"Wait for Process: Selected as exe matches (%s)",
					mRealPath->path().c_str());
				return true;
			}
		}
