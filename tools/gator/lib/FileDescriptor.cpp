...
int pipe_cloexec(int pipefd[2])
{
	...
	int fdf;
	if (((fdf = fcntl(pipefd[0], F_GETFD)) == -1) ||
	    (fcntl(pipefd[0], F_SETFD, fdf | FD_CLOEXEC) != 0) ||
	    ((fdf = fcntl(pipefd[1], F_GETFD)) == -1) ||
	    (fcntl(pipefd[1], F_SETFD, fdf | FD_CLOEXEC) != 0)) {
		close(pipefd[0]);
		close(pipefd[1]);
		return -1;
	}
...
bool readAll(const int fd, void *const buf, const size_t count)
{
	size_t pos = 0;
	while (pos < count) {
		ssize_t bytes = read(fd, reinterpret_cast<uint8_t *>(buf) + pos,
				     count - pos);
		if (bytes <= 0) {
...
bool skipAll(const int fd, const size_t count)
{
	...
	while (pos < count) {
		const size_t nToRead =
			std::min<size_t>(sizeof(buf), count - pos);
