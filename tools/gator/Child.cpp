...
void handleException()
{
	...
	if (gSessionData.mLocalCapture) {
		...
		int errorCodeForRemovingDir =
			local_capture::removeDirAndAllContents(
				gSessionData.mTargetPath);
		if (errorCodeForRemovingDir != 0)
			logg.logError(
				"Could not remove incomplete APC directory.");
	}
...
void Child::signalHandler(int signum)
{
	...
	if (beenHere || signum == SIGALRM) {
		logg.logError("Gator child is being forced to shut down: %s",
			      strsignal(signum));
		if (!singleton->commandTerminated)
			logg.logError("Failed to terminate command (%d)",
				      singleton->commandPid);
		if (signum == SIGALRM)
			exit(alarmExitCode);
		else
		...
		beenHere = true;
		logg.logMessage("Gator child is shutting down: %s",
				strsignal(signum));
...
Child::Child(Drivers &drivers, OlySocket *sock, const Child::Config &config)
	: haltPipeline(), senderThreadStarted(), senderSem(), primarySource(),
	  externalSource(), userSpaceSource(), maliHwSource(), sender(),
	  drivers(drivers), socket(sock), numExceptions(0), sessionEnded(),
	  commandTerminated(true), commandPid(), config(config),
	  sharedData(shared_memory::make_unique<SharedData>())
{
	// update singleton
	const Child *const prevSingleton =
		gSingleton.exchange(this, std::memory_order_acq_rel);
	runtime_assert(prevSingleton == nullptr,
		       "Two Child instances active concurrently");
...
Child::~Child()
{
	// update singleton
	const Child *const prevSingleton =
		gSingleton.exchange(nullptr, std::memory_order_acq_rel);
	runtime_assert(
		prevSingleton == this,
		"Exchanged Child::gSingleton with something other than this");
}

void Child::run()
{
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-child"), 0,
	      0, 0);
	...
	if ((config.events.empty() && config.spes.empty()) ||
	    gSessionData.mConfigurationXMLPath != nullptr) {
		auto &&result = configuration_xml::getConfigurationXML(
		...
		for (auto &&counter : result.counterConfigurations) {
			if (config.events.count(counter) == 0) {
				checkError(configuration_xml::addCounterToSet(
					counterConfigs, std::move(counter)));
			} else {
				logg.logMessage(
					"Overriding <counter> '%s' from configuration.xml",
					counter.counterName.c_str());
			}
		}
		for (auto &&spe : result.speConfigurations) {
			if (config.spes.count(spe) == 0) {
				checkError(configuration_xml::addSpeToSet(
					speConfigs, std::move(spe)));
			} else {
				logg.logMessage(
					"Overriding <spe> '%s' from configuration.xml",
					spe.id.c_str());
			}
		}
	}

	checkError(configuration_xml::setCounters(
		counterConfigs, !countersAreDefaults, drivers));
	...
	for (const auto &speConfig : speConfigs) {
		...
		for (Driver *driver : drivers.getAll()) {
			...
			if (capturedSpe) {
				capturedSpes.push_back(
					std::move(capturedSpe.get()));
				claimed = true;
				break;
			}
		}
		...
		if (!claimed) {
			logg.logWarning("No driver claimed %s",
					speConfig.id.c_str());
		}
	}
	...
	if (socket) {
		...
		if (gSessionData.mSessionXMLPath) {
			...
			if (xmlString) {
				...
				logg.logWarning(
					"Unable to read session xml(%s) , using default values",
					gSessionData.mSessionXMLPath);
			}
			free(xmlString);
		}
		...
		// Write events XML
		events_xml::write(
			gSessionData.mAPCDir, drivers.getAllConst(),
			primarySourceProvider.getCpuInfo().getClusters());
	}
	...
	if (!gSessionData.mCaptureCommand.empty()) {
		...
		Command commandResult = runCommand(sharedData->startProfile, [this]() {
			...
			if (gSessionData.mStopOnExit) {
				logg.logMessage(
					"Ending session because command exited");
				endSession();
			}
		});
	...
	if (gSessionData.mWaitForProcessCommand != nullptr) {
		logg.logMessage("Waiting for pids for command '%s'",
				gSessionData.mWaitForProcessCommand);

		WaitForProcessPoller poller{
			gSessionData.mWaitForProcessCommand
		};
		...
		logg.logMessage("Got pids for command '%s'",
				gSessionData.mWaitForProcessCommand);
	}
	...
	if (!sessionEnded) {
		...
		// app to run
		std::set<int> watchPids =
			appPids.empty() ? gSessionData.mPids : appPids;

		appPids.insert(gSessionData.mPids.begin(),
			       gSessionData.mPids.end());

		// Set up the driver; must be done after gSessionData.mPerfCounterType[] is populated
		primarySource = primarySourceProvider.createPrimarySource(
			*this, senderSem, sharedData->startProfile, appPids,
		...
		if (primarySourceProvider.supportsMaliCapture() &&
		    primarySourceProvider.isCapturingMaliCounters() &&
		    !primarySourceProvider.supportsMaliCaptureSampleRate(
			    gSessionData.mSampleRate)) {
			logg.logError(
				"Mali counters are not supported with Sample Rate: %i.",
				gSessionData.mSampleRate);
			handleException();
		}
		...
		externalSource.reset(
			new ExternalSource(*this, &senderSem, drivers));
		if (!externalSource->prepare()) {
			logg.logError(
				"Unable to prepare external source for capture");
			handleException();
		}
		...
		if (!primarySource->prepare()) {
			logg.logError("%s", primarySourceProvider
						    .getPrepareFailedMessage());
			handleException();
		}
		auto getMonotonicStarted =
			[&primarySourceProvider]() -> std::int64_t {
		...
		if (drivers.getMaliHwCntrs().countersEnabled()) {
			maliHwSource.reset(new mali_userspace::MaliHwCntrSource(
				*this, &senderSem, getMonotonicStarted,
				drivers.getMaliHwCntrs()));
			if (!maliHwSource->prepare()) {
				logg.logError(
					"Unable to prepare midgard hardware counters source for capture");
				handleException();
			}
			maliHwSource->start();
		}
		...
		if (gSessionData.mDuration > 0) {
			durationThread = std::thread(
				[&]() { durationThreadEntryPoint(waiter); });
		}
		...
		if (gSessionData.mStopOnExit && !watchPids.empty()) {
			watchPidsThread = std::thread([&]() {
				watchPidsThreadEntryPoint(watchPids, waiter);
			});
		}

		if (UserSpaceSource::shouldStart(drivers.getAllPolledConst())) {
			userSpaceSource.reset(new UserSpaceSource(
				*this, &senderSem, getMonotonicStarted,
				drivers.getAllPolled()));
			if (!userSpaceSource->prepare()) {
				logg.logError(
					"Unable to prepare userspace source for capture");
				handleException();
			}
	...
	if (gSessionData.mLocalCapture) {
		...
		captured_xml::write(gSessionData.mAPCDir, capturedSpes,
...
void Child::endSession()
{
	...
	std::thread{ []() {
		// rename thread
		prctl(PR_SET_NAME,
		      reinterpret_cast<unsigned long>("gatord-alarm"), 0, 0, 0);
		// sleep
		::sleep(5);
		// terminate
...
void Child::cleanupException()
{
	if (numExceptions++ > 0) {
		...
		logg.logMessage(
			"Received multiple exceptions, terminating the child");
	...
	if (socket) {
		if (sender) {
			...
			sender->writeData(logg.getLastError(),
					  strlen(logg.getLastError()),
...
{
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-duration"),
	      0, 0, 0);
...
{
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-stopper"),
	      0, 0, 0);
	while (gSessionData.mSessionIsActive) {
		...
		const int result = socket->receiveNBytes(
			reinterpret_cast<char *>(&header), sizeof(header));
		const char type = header[0];
		const int length = (header[1] << 0) | (header[2] << 8) |
				   (header[3] << 16) | (header[4] << 24);
		if (result == -1) {
			...
			if ((type != COMMAND_APC_STOP) &&
			    (type != COMMAND_PING)) {
				logg.logMessage(
					"INVESTIGATE: Received unknown command type %d",
					type);
			} else {
				...
				if (length == 0) {
					...
					kill(getppid(),
					     Child::SIG_LIVE_CAPTURE_STOPPED);
					...
					logg.logMessage(
						"INVESTIGATE: Received APC_STOP or PING command but with length = %d",
						length);
				}
			}
		}
...
void Child::senderThreadEntryPoint()
{
	...
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-sender"), 0,
	      0, 0);
	sem_wait(&haltPipeline);

	while ((!externalSource->isDone()) ||
	       ((maliHwSource != NULL) && (!maliHwSource->isDone())) ||
		...
		if (clock_gettime(CLOCK_REALTIME, &timeout) != 0) {
			logg.logError("clock_gettime failed: %d, (%s)", errno,
				      strerror(errno));
			handleException();
		}
		...
		if (sem_timedwait(&senderSem, &timeout) != 0) {
			if (errno == ETIMEDOUT) {
				logg.logMessage(
					"Timeout waiting for sender thread");
			} else {
				logg.logError("wait failed: %d, (%s)", errno,
					      strerror(errno));
			}
		}
...
void Child::watchPidsThreadEntryPoint(std::set<int> &pids,
				      const lib::Waiter &waiter)
{
	// rename thread
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>("gatord-pidwatcher"),
	      0, 0, 0);
	...
	while (!pids.empty()) {
		...
		const auto &alivePids =
			lib::getNumericalDirectoryEntries<int>("/proc");
		auto it = pids.begin();
		while (it != pids.end()) {
	...
	logg.logMessage(
		"Ending session because all watched processes have exited");
	endSession();
