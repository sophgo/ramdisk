...
namespace cpu_utils
{
void updateCpuIdsFromTopologyInformation(
	lib::Span<int> cpuIds, const std::map<unsigned, unsigned> &cpuToCpuIds,
	...
	const std::map<unsigned, std::set<unsigned> > &clusterToCpuIds)
{
	...
	for (unsigned cpu = 0; cpu < cpuIds.length; ++cpu) {
		...
		if (cpuIdIt != cpuToCpuIds.end()) {
			...
			if (cpuClusterIt != cpuToCluster.end()) {
				...
				const auto cpuIdsIt =
					clusterToCpuIds.find(clusterId);
				if (cpuIdsIt != clusterToCpuIds.end()) {
					const std::set<unsigned> &clusterCpuIds =
						cpuIdsIt->second;
	...
	for (unsigned cpu = 0; cpu < cpuIds.length; ++cpu) {
		...
		if ((currentCpuId == 0) || (currentCpuId == 0xfffff)) {
			...
			if (cpuClusterIt != cpuToCluster.end()) {
				const unsigned clusterId = cpuClusterIt->second;
				const auto clusterCpusIt =
					clusterToCpus.find(clusterId);
				if (clusterCpusIt != clusterToCpus.end()) {
					const std::set<unsigned> &clusterCpus =
						clusterCpusIt->second;
					std::set<unsigned> siblingCpuIds;
					for (unsigned sibling : clusterCpus) {
						const unsigned siblingCpuId =
							cpuIds[sibling] &
							0xfffff;
						if ((cpu != sibling) &&
						    (siblingCpuId != 0) &&
						    (siblingCpuId != 0xfffff)) {
							siblingCpuIds.insert(
								siblingCpuId);
						}
					}
		...
		logg.logMessage("CPU %u is configured to use CPUID 0x%05x", cpu,
				cpuIds[cpu]);
	}
}
} // namespace cpu_utils
