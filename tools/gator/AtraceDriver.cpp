...
AtraceCounter::AtraceCounter(DriverCounter *next, const char *name, int flag)
	: DriverCounter(next, name), mFlag(flag)
{
}
...
AtraceDriver::AtraceDriver(const FtraceDriver &ftraceDriver)
	: SimpleDriver("Atrace"), mSupported(false), mNotifyPath(),
...
void AtraceDriver::readEvents(mxml_node_t *const xml)
{
	...
	strncat(mNotifyPath, "notify.dex",
		sizeof(mNotifyPath) - strlen(mNotifyPath) - 1);
	...
	while (true) {
		...
		if (flagStr == NULL) {
			logg.logError(
				"The atrace counter %s is missing the required flag attribute",
				counter);
			handleException();
		}
		int flag;
		if (!stringToInt(&flag, flagStr, 16)) {
			logg.logError(
				"The flag attribute of the atrace counter %s is not a hex integer",
				counter);
			handleException();
		}
...
void AtraceDriver::start()
{
	...
	int flags = 0;
	for (AtraceCounter *counter =
		     static_cast<AtraceCounter *>(getCounters());
	     counter != NULL;
