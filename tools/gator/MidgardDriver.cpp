...
struct PacketHeader {
	uint32_t mImplSpec : 8, mReserved0 : 8,
...
class MidgardCounter : public DriverCounter {
    public:
	MidgardCounter(DriverCounter *next, const char *name,
...
void MidgardDriver::query() const
{
	...
	if (gSessionData.mSharedData->mMaliMidgardCountersSize > 0) {
		...
		if (uds < 0) {
			...
			while (true) {
				if (!lib::readAll(uds, &header,
						  sizeof(PacketHeader))) {
					logg.logError(
						"Unable to read Midgard header");
					handleException();
				}
				if (first && (reinterpret_cast<const uint8_t *>(
						      &header)[0] != 0)) {
					logg.logMessage(
						"Midgard data is not in encapsulated format");
					break;
				}
				first = false;
				...
				if (header.mSequenceNumbered) {
					logg.logError(
						"sequence_numbered is true and is unsupported");
					handleException();
				}

				logg.logMessage(
					"MIPE Packet: 0x%x 0x%x 0x%x 0x%x 0x%x",
					header.mDataLength, header.mImplSpec,
					header.mPacketIdentifier,
					header.mReserved0, header.mReserved1);

				switch (header.mPacketIdentifier) {
				case PACKET_SHARED_PARAMETER: {
					SharedParameterPacket packet;
					if (header.mDataLength <
					    sizeof(packet)) {
						logg.logError(
							"Unable to read Shared Parameter Packet because it's at least %zu bytes long but only %" PRIu32
							" bytes were given",
							sizeof(packet),
							header.mDataLength);
						handleException();
					}
					if (!lib::readAll(uds, &packet,
							  sizeof(packet))) {
						logg.logError(
							"Unable to read Shared Parameter Packet");
						handleException();
					}
					if (!lib::skipAll(
						    uds,
						    header.mDataLength -
							    sizeof(packet))) {
						logg.logError(
							"Unable to skip Shared Parameter Packet pool");
						handleException();
					}

					if (header.mImplSpec == 0 &&
					    packet.mReserved2 == 0) {
						if (packet.mMaliMagic !=
						    0x6D616C69) {
							logg.logError(
								"mali_magic does not match expected value");
							handleException();
						}
					}
					break;
				...
				case PACKET_HARDWARE_COUNTER_DIRECTORY: {
					if (header.mImplSpec == 0) {
						constexpr size_t buffSize = sizeof(
							gSessionData
								.mSharedData
						...
						if (header.mDataLength >
						    buffSize) {
							logg.logError(
								"Unable to read Hardware Counter Directory Packet because it's %" PRIu32
						...
						if (!lib::readAll(
							    ...
							    header.mDataLength)) {
							logg.logError(
								"Unable to read Hardware Counter Directory Packet");
							handleException();
						}
						gSessionData.mSharedData
							->mMaliMidgardCountersSize =
							header.mDataLength;
						goto allDone;
					}
				}
	...
	for (int i = 0; i + sizeof(HardwareCounter) < size;) {
		const HardwareCounter *counter =
			reinterpret_cast<const HardwareCounter *>(buf + i);
		char *name;
		if (asprintf(&name, "ARM_Mali-%s", counter->mCounterName) <=
		    0) {
		...
		const_cast<MidgardDriver *>(this)->setCounters(
			new MidgardCounter(getCounters(), name, &cd));
		::free(name);
	...
	if (size > 0) {
		...
		const_cast<MidgardDriver *>(this)->setCounters(
			new MidgardCounter(getCounters(),
					   "ARM_Mali-Midgard_Filmstrip2_cnt0",
					   &cd));
		...
		const_cast<MidgardDriver *>(this)->setCounters(
			new MidgardCounter(getCounters(),
					   "ARM_Mali-Midgard_fragment", &cd));
		const_cast<MidgardDriver *>(this)->setCounters(
			new MidgardCounter(getCounters(),
					   "ARM_Mali-Midgard_vertex", &cd));
		const_cast<MidgardDriver *>(this)->setCounters(
			new MidgardCounter(getCounters(),
					   "ARM_Mali-Midgard_opencl", &cd));
	}
}
...
bool MidgardDriver::start(const int uds)
{
	...
	size_t bufPos = 0;
	char buf[ARRAY_LENGTH(enabled) * sizeof(GPUPerfPeriod) +
		 sizeof(GLESWindump)];
	...
	for (MidgardCounter *counter =
		     static_cast<MidgardCounter *>(getCounters());
	     counter != NULL;
	     counter = static_cast<MidgardCounter *>(counter->getNext())) {
		if (!counter->isEnabled() ||
		    counter->getType() != CounterData::PERF) {
			continue;
		}
	...
	for (int i = 0; i < ARRAY_LENGTH(enabled); ++i) {
		...
		// MALI_GPUPERF_PERIOD
		m.mDeclId = 0;
		m.mMicroseconds = gSessionData.mSampleRate > 0 ?
					  1000000 / gSessionData.mSampleRate :
					  100000;
		m.mStartIndex = 64 * i;
	...
	for (MidgardCounter *counter =
	     ...
	     counter = static_cast<MidgardCounter *>(counter->getNext())) {
		if (!counter->isEnabled() ||
		    counter->getType() != CounterData::WINDUMP) {
			continue;
		}

		if (foundWindumpCounter) {
			logg.logError(
				"Only one Mali Midgard filmstrip counter can be enabled at a time");
			handleException();
		}
...
void MidgardDriver::setupCounter(Counter &counter)
{
	MidgardCounter *const midgardCounter =
		static_cast<MidgardCounter *>(findCounter(counter));
	...
	if (midgardCounter->getType() == CounterData::ACTIVITY &&
	    midgardCounter->getCores() > 0) {
