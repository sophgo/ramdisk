...
{
std::unique_ptr<mxml_node_t, void (*)(mxml_node_t *)>
getTree(lib::Span<const GatorCpu> clusters)
{
#include "events_xml.h" // defines and initializes char events_xml[] and int events_xml_len
	...
	if (gSessionData.mEventsXMLPath) {
		std::unique_ptr<FILE, int (*)(FILE *)> fl{
			lib::fopen_cloexec(gSessionData.mEventsXMLPath, "r"),
			fclose
		};
		if (fl != nullptr) {
			mainXml = makeMxmlUniquePtr(mxmlLoadFile(
				nullptr, fl.get(), MXML_NO_CALLBACK));
			if (mainXml == nullptr) {
				logg.logError("Unable to parse %s",
					      gSessionData.mEventsXMLPath);
				handleException();
			}
		}
	}
	if (mainXml == nullptr) {
		...
		mainXml = makeMxmlUniquePtr(mxmlLoadString(
			nullptr, reinterpret_cast<const char *>(events_xml),
			MXML_NO_CALLBACK));
	}
	...
	if (gSessionData.mEventsXMLAppend) {
		std::unique_ptr<FILE, int (*)(FILE *)> fl{
			lib::fopen_cloexec(gSessionData.mEventsXMLAppend, "r"),
			fclose
		};
		if (fl == nullptr) {
			logg.logError("Unable to open additional events XML %s",
				      gSessionData.mEventsXMLAppend);
			handleException();
		}

		mxml_unique_ptr appendXml = makeMxmlUniquePtr(
			mxmlLoadFile(nullptr, fl.get(), MXML_NO_CALLBACK));
...
getXML(lib::Span<const Driver *const> drivers,
