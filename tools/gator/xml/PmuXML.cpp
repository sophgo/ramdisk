...
GatorCpu::GatorCpu(std::string coreName, std::string id, std::string counterSet,
		   const char *dtName, const char *speName,
		   const std::set<int> &cpuIds, int pmncCounters, bool isV8)
	: mCoreName(coreName), mId(id), mCounterSet(counterSet),
	  mDtName(dtName != nullptr ? dtName : ""),
	  mSpeName(speName != nullptr ? speName : ""),
	  mCpuIds(cpuIds.begin(), cpuIds.end()), mPmncCounters(pmncCounters),
	  mIsV8(isV8)
{
...
{
	return (std::find(mCpuIds.begin(), mCpuIds.end(), cpuId) !=
		mCpuIds.end());
}
...
const GatorCpu *PmuXML::findCpuByName(const char *const name) const
{
	for (const GatorCpu &gatorCpu : cpus) {
		...
		if (((strncasecmp(name, OLD_PMU_PREFIX,
				  sizeof(OLD_PMU_PREFIX) - 1) == 0) &&
		     (strncasecmp(gatorCpu.getId(), NEW_PMU_PREFIX,
				  sizeof(NEW_PMU_PREFIX) - 1) == 0) &&
		     (strcasecmp(name + sizeof(OLD_PMU_PREFIX) - 1,
				 gatorCpu.getId() + sizeof(NEW_PMU_PREFIX) -
					 1) == 0))) {
			return &gatorCpu;
		}
	}
...
UncorePmu::UncorePmu(std::string coreName, std::string id,
		     std::string counterSet, int pmncCounters,
		     bool hasCyclesCounter)
	: mCoreName(coreName), mId(id), mCounterSet(counterSet),
	  mPmncCounters(pmncCounters), mHasCyclesCounter(hasCyclesCounter)
{
}
