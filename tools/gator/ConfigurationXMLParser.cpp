...
ConfigurationXMLParser::ConfigurationXMLParser()
	: counterConfigurations(), speConfigurations()
{
}
...
int static configurationsTag(mxml_node_t *node)
{
	...
	if (revision < CONFIGURATION_REVISION) {
		logg.logError(
			"Revision issue, please check configuration XML v%d",
			revision);
		return 1; // revision issue
	}
...
int ConfigurationXMLParser::readCounter(mxml_node_t *node)
{
	...
	if (mxmlElementGetAttr(node, ATTR_COUNT)) {
		if (!stringToInt(&count, mxmlElementGetAttr(node, ATTR_COUNT),
				 10)) {
			logg.logError(
				"Configuration XML count must be an integer");
			return PARSER_ERROR;
		}
		counter.count = count;
	}
	if (mxmlElementGetAttr(node, ATTR_CORES)) {
		if (!stringToInt(&cores, mxmlElementGetAttr(node, ATTR_CORES),
				 10)) {
			logg.logError(
				"Configuration XML cores must be an integer");
			return PARSER_ERROR;
		}
		counter.cores = cores;
	}
	...
	if (eventStr) {
		if (!stringToInt(&event, eventStr, 16)) {
			logg.logError(
				"Configuration XML event must be an integer");
			return PARSER_ERROR;
		} else {
			counter.event = event;
		}
...
int ConfigurationXMLParser::readSpe(mxml_node_t *node)
{
	...
	spe.id = id;
	const char *attrEventFilter =
		mxmlElementGetAttr(node, ATTR_EVENT_FILTER);
	if (attrEventFilter) {
		char *end;
		errno = 0;
		...
		if (event_mask == 0 && end == attrEventFilter) {
			logg.logError(
				"Configuration XML spe event-filter must be an integer");
			return PARSER_ERROR;
		} else if (event_mask == ULLONG_MAX && errno) {
			logg.logError(
				"Configuration XML spe event-filter must be in the range of unsigned long long");
			return PARSER_ERROR;
		} else if (*end) {
			logg.logError(
				"Configuration XML spe event-filter must be an integer");
	...
	if (attrLoadFilter) {
		if (strcmp(attrLoadFilter, "true") == 0) {
			...
			logg.logError(
				"Configuration XML spe load-filter must be either true or false");
			return PARSER_ERROR;
		}
	}
	const char *attrStoreFilter =
		mxmlElementGetAttr(node, ATTR_STORE_FILTER);
	if (attrStoreFilter) {
		if (strcmp(attrStoreFilter, "true") == 0) {
			spe.ops.insert(SpeOps::STORE); //set
		} else if (strcmp(attrStoreFilter, "false") != 0) {
			logg.logError(
				"Configuration XML spe store-filter must be either true or false");
			return PARSER_ERROR;
		}
	}
	const char *attrBranchFilter =
		mxmlElementGetAttr(node, ATTR_BRANCH_FILTER);
	if (attrBranchFilter) {
		if (strcmp(attrBranchFilter, "true") == 0) {
			spe.ops.insert(SpeOps::BRANCH); //set
		} else if (strcmp(attrBranchFilter, "false") != 0) {
			logg.logError(
				"Configuration XML spe branch-filter must be either true or false");
			return PARSER_ERROR;
		}
	}
	...
	if (attrMinLatency) {
		if (!stringToInt(&minLatency, attrMinLatency, 10)) {
			logg.logError(
				"Configuration XML spe min-latency must be an integer");
			return PARSER_ERROR;
		}
 ...
 */
int ConfigurationXMLParser::parseConfigurationContent(
	const char *config_xml_content)
{
	...
	if (node) {
		while (node && mxmlGetType(node) != MXML_ELEMENT)
			node = mxmlFindElement(node, tree.get(),
					       TAG_CONFIGURATIONS,
					       ATTR_REVISION, NULL,
					       MXML_NO_DESCEND);
		...
		while (node) {
			if (mxmlGetType(node) != MXML_ELEMENT) {
				node = mxmlWalkNext(node, tree.get(),
						    MXML_NO_DESCEND);
				continue;
			}
			...
			} else if (strcasecmp(mxmlGetElement0,
					      TAG_CONFIGURATION) == 0) {
				read_ret = readCounter(node);
			} else {
				logg.logMessage(
					"Ignoring unknown element while parsing configuration xml (%s)",
					mxmlGetElement0);
				read_ret = 0;
			}
			...
			if (read_ret == PARSER_ERROR) {
				...
				logg.logError(
					"Error while parsing configuration xml");
				return PARSER_ERROR;
			}
...
const std::vector<CounterConfiguration> &
ConfigurationXMLParser::getCounterConfiguration()
{
	return counterConfigurations;
}
const std::vector<SpeConfiguration> &
ConfigurationXMLParser::getSpeConfiguration()
{
