...
class MemInfoCounter : public DriverCounter {
    public:
	MemInfoCounter(DriverCounter *next, const char *const name,
		       int64_t *const value);
	~MemInfoCounter();
...
MemInfoCounter::MemInfoCounter(DriverCounter *next, const char *const name,
			       int64_t *const value)
...
MemInfoDriver::MemInfoDriver()
	: PolledDriver("MemInfo"), mBuf(), mMemUsed(0), mMemFree(0),
	  mBuffers(0), mCached(0), mSlab(0)
{
}
...
void MemInfoDriver::readEvents(mxml_node_t *const)
{
	if (access("/proc/meminfo", R_OK) == 0) {
		setCounters(new MemInfoCounter(
			getCounters(), "Linux_meminfo_memused2", &mMemUsed));
		setCounters(new MemInfoCounter(
			getCounters(), "Linux_meminfo_memfree", &mMemFree));
		setCounters(new MemInfoCounter(
			getCounters(), "Linux_meminfo_bufferram", &mBuffers));
		setCounters(new MemInfoCounter(
			getCounters(), "Linux_meminfo_cached", &mCached));
		setCounters(new MemInfoCounter(getCounters(),
					       "Linux_meminfo_slab", &mSlab));
	} else {
		logg.logSetup(
