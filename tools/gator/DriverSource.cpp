...
DriverSource::DriverSource(Child &child, sem_t &senderSem, sem_t &startProfile,
	...
	: Source(child), mBuffer(NULL), mFifo(NULL), mSenderSem(senderSem),
	  mStartProfile(startProfile), mBufferSize(0), mBufferFD(0), mLength(1),
	  mFtraceDriver(ftraceDriver)
{
	...
	int enable = -1;
	if (lib::readIntFromFile("/dev/gator/enable", enable) != 0 ||
	    enable != 0) {
		logg.logError(
			"Driver already enabled, possibly a session is already in progress.");
		handleException();
	}

	if (lib::readIntFromFile("/dev/gator/buffer_size", mBufferSize) ||
	    mBufferSize <= 0) {
...
bool DriverSource::prepare()
{
	...
	logg.logMessage(
		"Created %d MB collector buffer with a %d-byte ragged end",
		gSessionData.mTotalBufferSize, mBufferSize);
	mFifo = new Fifo(mBufferSize + 5,
...
void DriverSource::run()
{
	...
	if (lib::writeReadIntInFile("/dev/gator/backtrace_depth",
	...
	if (mBufferFD < 0) {
		logg.logError(
			"The gator driver did not set up properly. Please view the linux console or dmesg log for more information on the failure.");
		handleException();
	}

	// set the tick rate of the profiling timer
	if (lib::writeReadIntInFile("/dev/gator/tick",
	...
	const ResponseType response_type = gSessionData.mLocalCapture ?
						   ResponseType::NONE :
						   ResponseType::APC_DATA;
	if (lib::writeIntToFile("/dev/gator/response_type",
	...
	// Set the live rate
	if (lib::writeReadInt64InFile("/dev/gator/live_rate",
	...
	if (lib::writeCStringToFile("/dev/gator/enable", "1") != 0) {
		logg.logError(
			"The gator driver did not start properly. Please view the linux console or dmesg log for more information on the failure.");
		handleException();
	}
	...
	if (pthread_create(&bootstrapThreadID, NULL, bootstrapThreadStatic,
			   this) != 0) {
	...
	do {
		...
		if (bytesCollected == -1 && errno == EINTR) {
			bytesCollected =
				read(mBufferFD, collectBuffer, mBufferSize);
		}
