...
Buffer::Buffer(const int32_t core, const FrameType frameType, const int size,
	       sem_t *const readerSem)
	: mBuf(new char[size]), mReaderSem(readerSem),
	  mCommitTime(gSessionData.mLiveRate), mWriterSem(), mSize(size),
	  mReadPos(0), mWritePos(0), mCommitPos(0), mAvailable(true),
	  mIsDone(false), mCore(core), mFrameType(frameType),
	  mLastEventTime(INVALID_LAST_EVENT_TIME), mLastEventCore(core),
	  mLastEventTid(0)
{
	if ((mSize & mask) != 0) {
...
void Buffer::write(ISender *const sender)
{
	...
	logg.logMessage("Sending data length1: %i length2: %i", length1,
			length2);
...
bool Buffer::commit(const uint64_t time, const bool force)
{
	...
	for (size_t byte = 0; byte < sizeof(int32_t); byte++) {
		mBuf[(commitPos + typeLength + byte) & mask] =
			(length >> byte * 8) & 0xFF;
	}

	logg.logMessage(
		"Committing data mReadPos: %i mWritePos: %i mCommitPos: %i",
		mReadPos.load(std::memory_order_relaxed), mWritePos, commitPos);
...
bool Buffer::check(const uint64_t time)
{
	...
	if (filled >= ((mSize * 3) / 4) ||
	    (gSessionData.mLiveRate > 0 && time >= mCommitTime)) {
		return commit(time);
	}
	return false;
}
...
void Buffer::frame()
{
	runtime_assert(mFrameType != FrameType::UNKNOWN,
...
{
	runtime_assert((mFrameType == frameType) ||
			       (mFrameType == FrameType::UNKNOWN),
		       "invalid frameType");
	runtime_assert((mCore == core) || (mFrameType == FrameType::UNKNOWN),
		       "invalid core");
...
{
	return ((frameType == FrameType::BLOCK_COUNTER) ||
		(frameType == FrameType::PERF_ATTRS) ||
		(frameType == FrameType::PERF_DATA) ||
		(frameType == FrameType::PERF_SYNC) ||
		(frameType == FrameType::NAME) ||
		(frameType == FrameType::SCHED_TRACE));
}
...
{
	if (checkSpace(buffer_utils::MAXSIZE_PACK32 +
		       buffer_utils::MAXSIZE_PACK64)) {
		...
		mLastEventTid =
			0; // this is also reset in CommonProtocolV22 when timestamp changes
...
{
	if (checkSpace(buffer_utils::MAXSIZE_PACK64 +
		       buffer_utils::MAXSIZE_PACK32)) {
		packInt(key);
		packInt64(value);
...
bool Buffer::counterMessage(uint64_t curr_time, int core, int key,
			    int64_t value)
{
	return threadCounterMessage(curr_time, core, 0, key, value);
}

bool Buffer::threadCounterMessage(uint64_t curr_time, int core, int tid,
				  int key, int64_t value)
{
	if ((mLastEventTime != curr_time) ||
	    (mLastEventTime == INVALID_LAST_EVENT_TIME)) {
