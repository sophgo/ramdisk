...
ExternalSource::ExternalSource(Child &child, sem_t *senderSem,
			       Drivers &mDrivers)
	: Source(child), mBufferSem(),
	  mBuffer(0, FrameType::EXTERNAL, 128 * 1024, senderSem), mMonitor(),
	  mMveStartupUds(MALI_VIDEO_STARTUP, sizeof(MALI_VIDEO_STARTUP)),
	  mMidgardStartupUds(MALI_GRAPHICS_STARTUP,
	  ...
	  mInterruptFd(-1), mMidgardUds(-1), mMveUds(-1), mDrivers(mDrivers)
{
...
void ExternalSource::configureConnection(const int fd,
...
bool ExternalSource::connectMidgard()
{
	...
	configureConnection(mMidgardUds, MALI_GRAPHICS_V1,
			    sizeof(MALI_GRAPHICS_V1));
...
void ExternalSource::connectFtrace()
{
	...
	const std::pair<std::vector<int>, bool> ftraceFds =
		mDrivers.getFtraceDriver().prepare();
...
{
	if (!mMonitor.init() || !lib::setNonblock(mMveStartupUds.getFd()) ||
	    !mMonitor.add(mMveStartupUds.getFd()) ||
	    !lib::setNonblock(mMidgardStartupUds.getFd()) ||
	    !mMonitor.add(mMidgardStartupUds.getFd()) ||
	    !lib::setNonblock(mUtgardStartupUds.getFd()) ||
	    !mMonitor.add(mUtgardStartupUds.getFd())
#ifdef TCP_ANNOTATIONS
	    || !lib::setNonblock(mAnnotate.getFd()) ||
	    !mMonitor.add(mAnnotate.getFd())
#endif
	    || !lib::setNonblock(mAnnotateUds.getFd()) ||
	    !mMonitor.add(mAnnotateUds.getFd())) {
		return false;
	}
...
void ExternalSource::run()
{
	...
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-external"),
	      0, 0, 0);
	...
	uint64_t val = 1;
	if (::write(gSessionData.mAnnotateStart, &val, sizeof(val)) !=
	    sizeof(val)) {
	...
	while (monotonicStarted <= 0 && gSessionData.mSessionIsActive) {
		usleep(1);
		monotonicStarted = mDrivers.getPrimarySourceProvider()
					   .getMonotonicStarted();
	}
	...
	while (gSessionData.mSessionIsActive) {
		...
		for (int i = 0; i < ready; ++i) {
			...
			if (fd == mMveStartupUds.getFd()) {
				...
				if (!lib::setNonblock(client) ||
				    !mMonitor.add(client)) {
					logg.logError(
						"Unable to set socket options on incoming annotation connection");
					handleException();
				}
			}
#endif
