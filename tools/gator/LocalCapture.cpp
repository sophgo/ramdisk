...
static char *createUniqueDirectory(const char *initialPath, const char *ending)
{
	...
	if (initialPath == 0 || strlen(initialPath) == 0) {
		logg.logError(
			"Missing -o command line option required for a local capture.");
		handleException();
...
void createAPCDirectory(const char *target_path)
{
	...
	if ((removeDirAndAllContents(gSessionData.mAPCDir) != 0 ||
	     mkdir(gSessionData.mAPCDir,
		   S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) != 0)) {
		logg.logError("Unable to create directory %s",
			      gSessionData.mAPCDir);
		handleException();
	}
}
				...
				if (strcmp(entry->d_name, ".") != 0 &&
					...
					std::unique_ptr<char[]> newpath(
						new char[strlen(path) +
							 strlen(entry->d_name) +
							 2]);
					sprintf(newpath.get(), "%s/%s", path,
						entry->d_name);
					error = removeDirAndAllContents(
						newpath.get());
					if (error) {
						break;
					}
...
void copyImages(const std::list<std::string> &list)
{
	...
	for (const auto &element : list) {
		...
		dstfilename[PATH_MAX - 1] =
			0; // strncpy does not guarantee a null-terminated string
		if (gSessionData.mAPCDir[strlen(gSessionData.mAPCDir) - 1] !=
		    '/') {
			strncat(dstfilename, "/",
				PATH_MAX - strlen(dstfilename) - 1);
		}
		strncat(dstfilename, getFilePart(element.c_str()),
			PATH_MAX - strlen(dstfilename) - 1);
		if (copyFile(element.c_str(), dstfilename)) {
			logg.logMessage("copied file %s to %s", element.c_str(),
					dstfilename);
		} else {
			logg.logMessage("copy of file %s to %s failed",
					element.c_str(), dstfilename);
		}
	}
}
} // namespace local_capture
