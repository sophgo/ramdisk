...
static int readPackedInt(char *const buf, const size_t bufSize,
			 size_t *const pos, uint64_t *const l)
{
	uint8_t shift = 0;
	uint8_t b = ~0;
...
ExternalDriver::ExternalDriver()
	: SimpleDriver("External"), mUds(-1), mQueried(false), mStarted(false)
{
}
...
{
	if (mUds < 0) {
		mUds = OlySocket::connect(MALI_UTGARD_SETUP,
					  sizeof(MALI_UTGARD_SETUP));
		if (mUds >= 0 && !lib::writeAll(mUds, SETUP_VERSION,
						sizeof(SETUP_VERSION) - 1)) {
...
void ExternalDriver::query() const
{
	...
	const size_t bufSize =
		sizeof(gSessionData.mSharedData->mMaliUtgardCounters);
	size_t size = 0;

	if (!connect()) {
		size = gSessionData.mSharedData->mMaliUtgardCountersSize;
		logg.logMessage(
			"Unable to connect, using cached version; size: %zi",
			size);
	} else {
		...
		if (!lib::writeAll(mUds, buf, pos)) {
			logg.logError(
				"Unable to send request counters message");
			handleException();
		}

		if (!lib::readAll(mUds, buf, HEADER_SIZE) ||
		    (buf[0] != HEADER_COUNTERS)) {
			logg.logError(
				"Unable to read request counters response header");
			handleException();
		}
		size = readLEInt(buf + 1);
		if (size > bufSize ||
		    !lib::readAll(mUds, buf, size - HEADER_SIZE)) {
			logg.logError(
				"Unable to read request counters response");
			handleException();
		}
	...
	while (pos < size) {
		...
		if (name != NULL) {
			if (readPackedInt(buf, bufSize, &pos, &cores) == 0) {
				...
				const_cast<ExternalDriver *>(this)->setCounters(
					new ExternalCounter(getCounters(), name,
							    cores));
			}
			::free(name);
		}
...
void ExternalDriver::start()
{
	...
	pos = HEADER_SIZE;
	for (ExternalCounter *counter =
		     static_cast<ExternalCounter *>(getCounters());
	     counter != NULL;
		...
		if (pos + nameLen + 1 + 2 * buffer_utils::MAXSIZE_PACK32 >
		    sizeof(buf)) {
			logg.logError(
				"Unable to enable counters, message is too large");
			handleException();
		}
	...
	buffer_utils::packInt(buf, pos,
			      NS_PER_S / (gSessionData.mSampleRate == 0 ?
						  10 :
...
void ExternalDriver::setupCounter(Counter &counter)
{
	ExternalCounter *const externalCounter =
		static_cast<ExternalCounter *>(findCounter(counter));
