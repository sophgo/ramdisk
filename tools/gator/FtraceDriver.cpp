...
class FtraceCounter : public DriverCounter {
    public:
	FtraceCounter(DriverCounter *next, const char *name,
		      const char *enable);
	~FtraceCounter();

	bool readTracepointFormat(const uint64_t currTime,
...
FtraceCounter::FtraceCounter(DriverCounter *next, const char *name,
			     const char *enable)
	: DriverCounter(next, name),
	  mEnable(enable == NULL ? NULL : strdup(enable)), mWasEnabled(false)
{
}
...
void FtraceCounter::prepare()
{
	if (mEnable == NULL) {
		if (gSessionData.mFtraceRaw) {
			logg.logError(
				"The ftrace counter %s is not compatible with the more efficient ftrace collection as it is missing the enable attribute. Please either add the enable attribute to the counter in events XML or disable the counter in counter configuration.",
				getName());
			handleException();
		}
		return;
	...
	if ((lib::readIntFromFile(buf, mWasEnabled) != 0) ||
	    (lib::writeIntToFile(buf, 1) != 0)) {
...
bool FtraceCounter::readTracepointFormat(const uint64_t currTime,
					 IPerfAttrsConsumer &attrsConsumer)
{
...
class FtraceReader {
    public:
	FtraceReader(Barrier *const barrier, int cpu, int tfd, int pfd0,
		     int pfd1)
		: mNext(mHead), mBarrier(barrier), mThread(), mCpu(cpu),
		  mTfd(tfd), mPfd0(pfd0), mPfd1(pfd1)
	{
		mHead = this;
	}
...
static ssize_t sys_splice(int fd_in, loff_t *off_in, int fd_out,
			  loff_t *off_out, size_t len, unsigned int flags)
{
	return syscall(__NR_splice, fd_in, off_in, fd_out, off_out, len, flags);
}

#define splice(fd_in, off_in, fd_out, off_out, len, flags)                     \
...
{
	{
		...
		prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&buf), 0, 0,
		      0);
	}
	...
	while (gSessionData.mSessionIsActive) {
		const ssize_t bytes = splice(mTfd, NULL, mPfd1, NULL, pageSize,
					     SPLICE_F_MOVE);
		if (bytes == 0) {
	...
	for (;;) {
		...
		bytes = splice(mTfd, NULL, mPfd1, NULL, pageSize,
			       SPLICE_F_MOVE);
		if (bytes <= 0) {
			break;
		} else {
...

	{
		...
		for (;;) {
			...
			if (bytes == 0) {
				logg.logError(
					"ftrace read unexpectedly returned 0");
				handleException();
			} else if (bytes < 0) {
				if (errno != EAGAIN) {
					logg.logError(
						"reading slop from ftrace failed");
					handleException();
				}
				break;
			} else {
				...
				if (bytes != static_cast<ssize_t>(size)) {
					logg.logError(
						"writing slop to ftrace pipe failed");
					handleException();
				}
			}
		}
...
FtraceDriver::FtraceDriver(bool useForTracepoints, size_t numberOfCores)
	: SimpleDriver("Ftrace"), mValues(NULL), mBarrier(), mTracingOn(0),
	  mSupported(false), mMonotonicRawSupport(false),
	  mUseForTracepoints(useForTracepoints), mNumberOfCores(numberOfCores)
{
}
...
void FtraceDriver::readEvents(mxml_node_t *const xml)
{
	...
	while (true) {
		...
		if (regex == NULL) {
			logg.logError(
				"The regex counter %s is missing the required regex attribute",
				counter);
			handleException();
		}
		...
		if (!mUseForTracepoints && tracepoint != NULL) {
			logg.logMessage("Not using ftrace for counter %s",
					counter);
			continue;
		}
		if (enable != NULL) {
			char buf[1 << 10];
			snprintf(buf, sizeof(buf), EVENTS_PATH "/%s/enable",
				 enable);
...
std::pair<std::vector<int>, bool> FtraceDriver::prepare()
{
	if (gSessionData.mFtraceRaw) {
		...
		if (lib::writeCStringToFile(TRACING_PATH "/events/enable",
					    "0") != 0) {
	...
	for (FtraceCounter *counter =
		     static_cast<FtraceCounter *>(getCounters());
	     counter != NULL;
	...
	if (lib::writeCStringToFile(TRACING_PATH "/tracing_on", "0") != 0) {
		logg.logError(
			"Unable to turn ftrace off before truncating the buffer");
		handleException();
	}

	{
		...
		fd = open(TRACING_PATH "/trace", O_WRONLY | O_TRUNC | O_CLOEXEC,
			  0666);
		if (fd < 0) {
			logg.logError("Unable truncate ftrace buffer: %s",
				      strerror(errno));
			handleException();
		}
		close(fd);
	...
	if ((trace_clock_file_length =
		     ::read(fd, trace_clock_file_content,
			    max_trace_clock_file_length - 1)) < 0) {
	...
	if (must_switch_clock &&
	    lib::writeCStringToFile(TRACING_PATH "/trace_clock", clock) != 0) {
		logg.logError(
			"Unable to switch ftrace to the %s clock, please ensure you are running Linux %s or later",
			clock, mMonotonicRawSupport ? "4.2" : "3.10");
		handleException();
	}

	if (!gSessionData.mFtraceRaw) {
		const int fd =
			open(TRACING_PATH "/trace_pipe", O_RDONLY | O_CLOEXEC);
		if (fd < 0) {
	...
	for (size_t cpu = 0; cpu < mNumberOfCores; ++cpu) {
		...
		if (pipe2(pfd, O_CLOEXEC) != 0) {
			logg.logError("pipe2 failed, %s (%i)", strerror(errno),
				      errno);
			handleException();
		}
		...
		snprintf(buf, sizeof(buf),
			 TRACING_PATH "/per_cpu/cpu%zu/trace_pipe_raw", cpu);
...
std::vector<int> FtraceDriver::stop()
{
	...
	std::vector<int> fds;
	if (gSessionData.mFtraceRaw) {
		for (FtraceReader *reader = FtraceReader::getHead();
		     reader != NULL; reader = reader->getNext()) {
			reader->interrupt();
					 ...
					 DynBuf *const printb, DynBuf *const b)
{
	...
	while ((dirent = readdir(dir)) != NULL) {
		...
		if (!printb->printf(EVENTS_PATH "/ftrace/%s/format",
				    dirent->d_name)) {
			logg.logMessage("DynBuf::printf failed");
			return false;
		}
		...
		attrsConsumer.marshalFormat(currTime, b->getLength(),
					    b->getBuf());
	}
	closedir(dir);
