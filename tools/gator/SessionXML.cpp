...
void SessionXML::parse()
{
	...
	node = mxmlFindElement(tree, tree, TAG_SESSION, NULL, NULL,
			       MXML_DESCEND);
...
{
	int version = 0;
	if (mxmlElementGetAttr(node, ATTR_VERSION) &&
	    !stringToInt(&version, mxmlElementGetAttr(node, ATTR_VERSION),
			 10)) {
	...
	if (mxmlElementGetAttr(node, ATTR_BUFFER_MODE)) {
		strncpy(parameters.buffer_mode,
			mxmlElementGetAttr(node, ATTR_BUFFER_MODE),
			sizeof(parameters.buffer_mode));
		parameters.buffer_mode[sizeof(parameters.buffer_mode) - 1] =
			0; // strncpy does not guarantee a null-terminated string
	}
	if (((gSessionData.parameterSetFlag & USE_CMDLINE_ARG_SAMPLE_RATE) ==
	     0)) {
		if (mxmlElementGetAttr(node, ATTR_SAMPLE_RATE)) {
			strncpy(parameters.sample_rate,
				mxmlElementGetAttr(node, ATTR_SAMPLE_RATE),
				sizeof(parameters.sample_rate));
			parameters.sample_rate[sizeof(parameters.sample_rate) -
					       1] =
				0; // strncpy does not guarantee a null-terminated string
		}
	}
	if (((gSessionData.parameterSetFlag &
	      USE_CMDLINE_ARG_CAPTURE_WORKING_DIR) == 0)) {
		if (mxmlElementGetAttr(node, ATTR_CAPTURE_WORKING_DIR)) {
			...
			gSessionData.mCaptureWorkingDir =
				strdup(mxmlElementGetAttr(
					node, ATTR_CAPTURE_WORKING_DIR));
		}
	}

	if (((gSessionData.parameterSetFlag &
	      USE_CMDLINE_ARG_CAPTURE_COMMAND) == 0) &&
		...
		gSessionData.mCaptureCommand.push_back(
			mxmlElementGetAttr(node, ATTR_CAPTURE_COMMAND));
	}
	if (((gSessionData.parameterSetFlag & USE_CMDLINE_ARG_STOP_GATOR) ==
	     0)) {
		if (mxmlElementGetAttr(node, ATTR_STOP_GATOR)) {
			gSessionData.mStopOnExit = stringToBool(
				mxmlElementGetAttr(node, ATTR_STOP_GATOR),
				false);
		}
	}
	if (mxmlElementGetAttr(node, ATTR_CAPTURE_USER)) {
		...
		gSessionData.mCaptureUser =
			strdup(mxmlElementGetAttr(node, ATTR_CAPTURE_USER));
	}

	// integers/bools
	if (((gSessionData.parameterSetFlag &
	      USE_CMDLINE_ARG_CALL_STACK_UNWINDING) == 0)) {
		parameters.call_stack_unwinding = stringToBool(
			mxmlElementGetAttr(node, ATTR_CALL_STACK_UNWINDING),
			false);
	}
	if (((gSessionData.parameterSetFlag & USE_CMDLINE_ARG_DURATION) == 0)) {
		if (mxmlElementGetAttr(node, ATTR_DURATION)) {
			if (!stringToInt(&gSessionData.mDuration,
					 mxmlElementGetAttr(node,
							    ATTR_DURATION),
					 10)) {
				logg.logError(
					"Invalid session.xml duration must be an integer");
				handleException();
			}
		}
	}
	if ((gSessionData.parameterSetFlag & USE_CMDLINE_ARG_FTRACE_RAW) == 0) {
		gSessionData.mFtraceRaw = stringToBool(
			mxmlElementGetAttr(node, USE_EFFICIENT_FTRACE),
			false); // default to false
	}
	if (mxmlElementGetAttr(node, ATTR_LIVE_RATE)) {
		if (!stringToInt(&parameters.live_rate,
				 mxmlElementGetAttr(node, ATTR_LIVE_RATE),
				 10)) {
			logg.logError(
				"Invalid session.xml live_rate must be an integer");
			handleException();
		}
	}
	// parse subtags
