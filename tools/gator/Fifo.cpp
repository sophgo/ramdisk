...
Fifo::Fifo(int singleBufferSize, int bufferSize, sem_t *readerSem)
	: mSingleBufferSize(singleBufferSize), mWrite(0), mRead(0),
	  mReadCommit(0), mRaggedEnd(0), mWrapThreshold(bufferSize),
	  mWaitForSpaceSem(), mReaderSem(readerSem),
	  mBuffer(new char[bufferSize + singleBufferSize]), mEnd(false)
{
	if (mBuffer == nullptr) {
		logg.logError("failed to allocate %d bytes",
			      bufferSize + singleBufferSize);
		handleException();
	}
...
bool Fifo::willFill(int additional) const
{
	if (mWrite > mRead) {
		...
		if (numBytesFilled() + additional <
		    mWrapThreshold - mSingleBufferSize) {
			return false;
		}
	}
	return true;
