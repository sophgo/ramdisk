...
Sender::Sender(OlySocket *socket)
	: mDataSocket(socket), mDataFile(nullptr, fclose),
	  mDataFileName(nullptr), mSendMutex()
{
	...
	if (socket) {
		...
		while (strcmp("STREAMLINE", streamline) != 0) {
			if (mDataSocket->receiveString(
				    streamline, sizeof(streamline)) == -1) {
	...
	if (pthread_mutexattr_init(&attr) != 0 ||
	    ...
	    pthread_mutexattr_destroy(&attr) != 0 || false) {
...
void Sender::createDataFile(const char *apcDir)
{
	...
	if (!mDataFile) {
		logg.logError("Failed to open binary file: %s",
			      mDataFileName.get());
		handleException();
	}
}

void Sender::writeDataParts(
	lib::Span<const lib::Span<const char, int> > dataParts,
	ResponseType type, bool ignoreLockErrors)
{
	int length = 0;
	for (const auto &data : dataParts) {
		length += data.length;
		if (data.length < 0) {
			logg.logError("Negative length message part (%d)",
				      data.length);
			handleException();
	...
	if (mDataSocket) {
		...
		for (const auto &data : dataParts) {
			int pos = 0;
			while (true) {
				mDataSocket->send(data.data + pos,
						  std::min(data.length - pos,
							   chunkSize));
				pos += chunkSize;
				if (pos >= data.length) {
					break;
	...
	if (mDataFile &&
	    (type == ResponseType::APC_DATA || type == ResponseType::RAW)) {
		...
		auto writeData = [this](lib::Span<const char, int> data) {
			if (fwrite(data.data, 1, data.length,
				   mDataFile.get()) !=
			    static_cast<size_t>(data.length)) {
				logg.logError("Failed writing binary file %s",
					      mDataFileName.get());
				handleException();
			}
		};
