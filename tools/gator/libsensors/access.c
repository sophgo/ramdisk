...
static int sensors_eval_expr(const sensors_chip_features *chip_features,
			     const sensors_expr *expr, double val, int depth,
			     double *result);
...
static sensors_chip *sensors_for_all_config_chips(const sensors_chip_name *name,
...
sensors_lookup_subfeature_nr(const sensors_chip_features *chip, int subfeat_nr)
{
	if (subfeat_nr < 0 || subfeat_nr >= chip->subfeature_count)
		return NULL;
...
{
	if (feat_nr < 0 || feat_nr >= chip->feature_count)
		return NULL;
			       ...
			       int depth, double *result)
{
	...
	if (!(subfeature =
		      sensors_lookup_subfeature_nr(chip_features, subfeat_nr)))
		return -SENSORS_ERR_NO_ENTRY;
		      ...
		      double value)
{
	...
	to_write = value;
	if (expr)
		if ((res = sensors_eval_expr(chip_features, expr, value, 0,
					     &to_write)))
			return res;
...
const sensors_feature *sensors_get_features(const sensors_chip_name *name,
					    int *nr)
{
...
const sensors_subfeature *sensors_get_subfeature(const sensors_chip_name *name,
						 const sensors_feature *feature,
						 sensors_subfeature_type type)
{
	...
	for (i = feature->first_subfeature;
	     i < chip->subfeature_count &&
	     chip->subfeature[i].mapping == feature->number;
	     i++) {
			     ...
			     double *result)
{
	...
	if (expr->kind == sensors_kind_var) {
		if (!(subfeature = sensors_lookup_subfeature_name(
			      chip_features, expr->data.var)))
			return -SENSORS_ERR_NO_ENTRY;
...
static int sensors_do_this_chip_sets(const sensors_chip_name *name)
{
	...
	for (chip = NULL; (chip = sensors_for_all_config_chips(name, chip));)
		for (i = 0; i < chip->sets_count; i++) {
			subfeature = sensors_lookup_subfeature_name(
				chip_features, chip->sets[i].name);
			if (!subfeature) {
