				  ...
				  int (*func)(const char *, const char *))
{
	...
	path_off = snprintf(path, NAME_MAX, "%s/class/%s", sensors_sysfs_mount,
			    class_name);
	if (!(dir = opendir(path)))
		return errno;
...
static char *get_feature_name(sensors_feature_type ftype, char *sfname)
{
...
/* Return the subfeature type and channel number based on the subfeature
   name */
static sensors_subfeature_type sensors_subfeature_get_type(const char *name,
							   int *nr)
{
	char c;
	int i, count;
				     ...
				     const char *dev_path)
{
	...
	while ((ent = readdir(dir))) {
		...
		if (all_types[ftype].count < nr + 1) {
			...
			int step = ftype < SENSORS_FEATURE_VID ?
					   8 :
	...
	for (ftype = 0; ftype < SENSORS_FEATURE_MAX; ftype++) {
		...
		for (i = 0; i < all_types[ftype].count * feature_size; i++) {
			...
			if (i / feature_size != prev_slot) {
				...
				dyn_features[fnum].name = get_feature_name(
					ftype, all_types[ftype].sf[i].name);
...
static int classify_device(const char *dev_name, const char *subsys,
			   sensors_chip_features *entry)
{
	...
	if ((!subsys || !strcmp(subsys, "i2c")) &&
		...
		entry->chip.bus.nr = 0;
	} else if ((!subsys || !strcmp(subsys, "platform") ||
...
static int find_bus_type(const char *dev_path, const char *dev_name,
			 sensors_chip_features *entry)
{
...
int sensors_read_sysfs_attr(const sensors_chip_name *name,
			    const sensors_subfeature *subfeature, double *value)
{
	char n[NAME_MAX];
	int f;
