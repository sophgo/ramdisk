...
{
static bool addCounter(const char *counterName, int event, int count, int cores,
		       int mIndex, bool printWarningIfUnclaimed,
...
Contents getConfigurationXML(lib::Span<const GatorCpu> clusters)
{
	// try the configuration.xml file first
	{
		...
		std::unique_ptr<char, void (*)(void *)> configurationXML{
			readFromDisk(path), &free
		};

		if (configurationXML) {
			ConfigurationXMLParser parser;
			if (parser.parseConfigurationContent(
				    configurationXML.get()) == 0) {
				return { std::move(configurationXML), false,
	...
	logg.logMessage(
		"Unable to locate configuration.xml, using default in binary");

	{
		auto &&configurationXML = getDefaultConfigurationXml(clusters);
		ConfigurationXMLParser parser;
		if (parser.parseConfigurationContent(configurationXML.get()) ==
		    0) {
			return { std::move(configurationXML), true,
				 parser.getCounterConfiguration(),
				 parser.getSpeConfiguration() };
		}
	}
...
std::string addCounterToSet(std::set<CounterConfiguration> &configs,
			    CounterConfiguration &&config)
{
	...
	if (!insertion.second) {
		return lib::Format() << "Duplicate <counter> found '"
				     << insertion.first->counterName << "'";
	}

	return {};
}

std::string addSpeToSet(std::set<SpeConfiguration> &configs,
			SpeConfiguration &&config)
{
	if (config.id.empty()) {
	...
	const auto &insertion = configs.insert(std::move(config));
	if (!insertion.second) {
		return lib::Format() << "Duplicate <spe> found \""
				     << insertion.first->id << "\"";
	}
...
std::string
setCounters(const std::set<CounterConfiguration> &counterConfigurations,
	    bool printWarningIfUnclaimed, Drivers &drivers)
{
	...
	for (CounterConfiguration cc : counterConfigurations) {
		...
		const bool added =
			addCounter(cc.counterName.c_str(), cc.event, cc.count,
				   cc.cores, index, printWarningIfUnclaimed,
				   drivers.getAll());
		if (added) {
...
getDefaultConfigurationXml(lib::Span<const GatorCpu> clusters)
{
#include "defaults_xml.h" // defines and initializes char defaults_xml[] and int defaults_xml_len
	...
	mxml_node_t *xml =
		mxmlLoadString(NULL,
			       reinterpret_cast<const char *>(defaults_xml),
			       MXML_NO_CALLBACK);
	for (mxml_node_t *node = mxmlFindElement(xml, xml, TAG_CONFIGURATION,
						 NULL, NULL, MXML_DESCEND),
			 *next = mxmlFindElement(node, xml, TAG_CONFIGURATION,
						 NULL, NULL, MXML_DESCEND);
	     node != NULL;
	     node = next, next = mxmlFindElement(node, xml, TAG_CONFIGURATION,
						 NULL, NULL, MXML_DESCEND)) {
		const char *counter = mxmlElementGetAttr(node, ATTR_COUNTER);
		if (counter != NULL && strncmp(counter, CLUSTER_VAR,
					       sizeof(CLUSTER_VAR) - 1) == 0) {
			for (const GatorCpu &cluster : clusters) {
				mxml_node_t *n = mxmlNewElement(
					mxmlGetParent(node), TAG_CONFIGURATION);
...
void remove()
{
	...
	if (::remove(path) != 0) {
		logg.logError(
			"Invalid configuration.xml file detected and unable to delete it. To resolve, delete configuration.xml on disk");
		handleException();
	}
		       ...
		       lib::Span<Driver *const> drivers)
{
	...
	const auto it = std::find_if(
		gSessionData.globalCounterToEventMap.begin(), end,
	...
	for (Driver *driver : drivers) {
		if (driver->claimCounter(counter)) {
			if ((counter.getDriver() != nullptr) &&
			    (counter.getDriver() != driver)) {
				logg.logError(
					"More than one driver has claimed %s:%i (%s vs %s)",
	...
	if (counter.getDriver() == nullptr) {
		if (printWarningIfUnclaimed)
			logg.logWarning("No driver has claimed %s:%i",
					counter.getType(), counter.getEvent());
