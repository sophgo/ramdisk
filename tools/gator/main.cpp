...
__attribute__((noreturn)) static void handler(int signum)
{
	...
	if (!poll_state()) {
		...
		logg.logMessage(
			"Killing process group as more than one gator-child was running when signal was received");
		kill(0, SIGINT);
		...
		if (!poll_state()) {
			...
			if (!poll_state()) {
				...
				logg.logError(
					"Failed to shut down child. Possible deadlock. Sending SIGKILL to kill everything");
				kill(0, SIGKILL);
			}
		}
	}
...
static void child_exit(int)
{
	...
	int pid = wait(&status);
	if (pid != -1) {
		logg.logMessage("Child process %d exited with status %d", pid,
				status);
...
static const char DST_REQ[] = { 'D', 'S', 'T', '_', 'R', 'E',
				'Q', ' ', 0,   0,   0,   0x64 };
...
class UdpListener {
	...
	void setup(int port)
	{
		...
		memcpy(mDstAns.rviHeader, "STR_ANS ",
		       sizeof(mDstAns.rviHeader));
		if (gethostname(mDstAns.dhcpName,
				sizeof(mDstAns.dhcpName) - 1) != 0) {
	...
	void handle()
	{
		...
		read = recvfrom(mReq, &buf, sizeof(buf), 0,
	...
	int udpPort(int port)
	{
		...
		if (bind(s, reinterpret_cast<struct sockaddr *>(&sockaddr),
			 sizeof(sockaddr)) < 0) {
			logg.logError("socket failed");
			handleException();
		}
...
static bool init_module(const char *const location)
{
	...
	if (fd >= 0) {
		struct stat st;
		if (fstat(fd, &st) == 0) {
			void *const p = mmap(NULL, st.st_size, PROT_READ,
					     MAP_PRIVATE, fd, 0);
			if (p != MAP_FAILED) {
				if (syscall(__NR_init_module, p,
...
bool setupFilesystem(const char *module)
{
	if (module) {
		...
		if (access("/dev/gator/buffer", F_OK) == 0) {
			logg.logError(
				"Unable to remove the running gator.ko. Manually remove the module or use the running module by not specifying one on the commandline");
			handleException();
		}
	}
	...
	if (retval == 1) {
		...
		if (module) {
			...
			if (getApplicationFullPath(location,
						   sizeof(location)) !=
			    0) { // allow some buffer space
				logg.logMessage(
					"Unable to determine the full path of gatord, the cwd will be used");
			}
			strncat(location, "gator.ko",
				sizeof(location) - strlen(location) - 1);
		}
		...
		if (access(location, F_OK) == -1) {
			if (module == NULL) {
				...
				logg.logError("gator module not found at %s",
					      location);
				handleException();
			}
		}
		...
		if (!init_module(location)) {
			logg.logMessage(
				"Unable to load gator.ko driver from location %s",
				location);
			logg.logError(
...
static void handleClient(Drivers &drivers)
{
	...
	if (state.load() == State::CAPTURING) {
		...
		if (pid < 0) {
			// Error
			logg.logError("Fork process failed with errno: %d.",
				      errno);
			handleException();
		} else if (pid == 0) {
			// Child
...
{
	gSessionData.mLocalCapture =
		result.mode == ParserResult::ExecutionMode::LOCAL_CAPTURE;
	...
	if ((gSessionData.mPerfMmapSizeInPages <= 0) && (geteuid() != 0) &&
	    (gSessionData.mPageSize >= 1024)) {
		std::int64_t perfEventMlockKb = 0;
		if (lib::readInt64FromFile(
			    "/proc/sys/kernel/perf_event_mlock_kb",
			    perfEventMlockKb) == 0) {
			if (perfEventMlockKb > 0) {
				const std::uint64_t perfEventMlockPages =
					(perfEventMlockKb /
					 (gSessionData.mPageSize / 1024));
				gSessionData.mPerfMmapSizeInPages =
					int(std::min<std::uint64_t>(
						perfEventMlockPages - 1,
						INT_MAX));
				logg.logMessage(
					"Default perf mmap size set to %d pages (%llukb)",
					gSessionData.mPerfMmapSizeInPages,
					gSessionData.mPerfMmapSizeInPages *
						gSessionData.mPageSize /
						1024ull);
			}
		} else {
			...
			logg.logMessage(
				...
				gSessionData.mPerfMmapSizeInPages,
				gSessionData.mPerfMmapSizeInPages *
					gSessionData.mPageSize / 1024ull);
		}
	}
	...
	if (result.parameterSetFlag & USE_CMDLINE_ARG_CAPTURE_WORKING_DIR) {
		gSessionData.mCaptureWorkingDir =
			strdup(result.mCaptureWorkingDir);
	}
...
int main(int argc, char **argv)
{
	...
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-main"), 0,
	      0, 0);
	...
	// Try to increase the maximum number of file descriptors
	{
		...
		if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
			logg.logMessage(
				"Unable to get the maximum number of files");
			// Not good, but not a fatal error either
		} else {
			rlim.rlim_max = std::max(rlim.rlim_cur, rlim.rlim_max);
			rlim.rlim_cur = std::min(std::max(rlim_t(1) << 15,
							  rlim.rlim_cur),
						 rlim.rlim_max);
			if (setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
				logg.logMessage(
					"Unable to increase the maximum number of files (%" PRIuMAX
					", %" PRIuMAX ")",
	...
	if (PROTOCOL_VERSION < PROTOCOL_DEV) {
		...
		snprintf(versionString, sizeof(versionString),
			 "Streamline gatord development version %d",
			 PROTOCOL_VERSION);
	}
	// Parse the command line parameters
	parser.parseCLIArguments(argc, argv, versionString,
				 MAX_PERFORMANCE_COUNTERS, gSrcMd5);
	...
	{
		auto xml =
			events_xml::getTree(drivers.getPrimarySourceProvider()
						    .getCpuInfo()
						    .getClusters());
		...
		// build map of counter->event
		{
			...
			while (true) {
				node = mxmlFindElement(node, xml.get(), "event",
						       NULL, NULL,
						       MXML_DESCEND);
				if (node == nullptr) {
					break;
				...
				const char *counter =
					mxmlElementGetAttr(node, "counter");
				const char *event =
					mxmlElementGetAttr(node, "event");
				if (counter == nullptr) {
					continue;
				}
				...
				if (event != nullptr) {
					const int eventNo =
						(int)strtol(event, nullptr, 0);
					gSessionData.globalCounterToEventMap
						[counter] = eventNo;
				} else {
					gSessionData.globalCounterToEventMap
						[counter] = -1;
	...
	if (result.mode == ParserResult::ExecutionMode::PRINT) {
		if (result.printables.count(
			    ParserResult::Printable::EVENTS_XML) == 1) {
			std::cout << events_xml::getXML(
					     drivers.getAllConst(),
					     drivers.getPrimarySourceProvider()
						     .getCpuInfo()
						     .getClusters())
					     .get();
		}
		if (result.printables.count(
			    ParserResult::Printable::COUNTERS_XML) == 1) {
			std::cout << counters_xml::getXML(
					     drivers.getPrimarySourceProvider()
						     .supportsMultiEbs(),
					     drivers.getAllConst(),
					     drivers.getPrimarySourceProvider()
		...
		if (result.printables.count(
			    ParserResult::Printable::DEFAULT_CONFIGURATION_XML) ==
		    1) {
			std::cout
				<< configuration_xml::getDefaultConfigurationXml(
					   drivers.getPrimarySourceProvider()
						   .getCpuInfo()
						   .getClusters())
					   .get();
	...
	if (gSessionData.mLocalCapture) {
		...
		if (result.port == DISABLE_TCP_USE_UDS_PORT) {
			sock = new OlyServerSocket(NO_TCP_PIPE,
						   sizeof(NO_TCP_PIPE), true);
			if (!monitor.add(sock->getFd())) {
				logg.logError(
					"Monitor setup failed: couldn't add host listeners");
				handleException();
			}
		} else {
	...
	while (1) {
		...
		for (int i = 0; i < ready; ++i) {
			if (!gSessionData.mLocalCapture &&
			    events[i].data.fd == sock->getFd()) {
				handleClient(drivers);
			} else if (!gSessionData.mLocalCapture &&
				   events[i].data.fd == udpListener.getReq()) {
				udpListener.handle();
			}
#ifdef TCP_ANNOTATIONS
			else if (events[i].data.fd ==
				 annotateListener.getSockFd()) {
				annotateListener.handleSock();
			}
#endif
			else if (events[i].data.fd ==
				 annotateListener.getUdsFd()) {
				annotateListener.handleUds();
			} else if (events[i].data.fd == pipefd[0]) {
				uint64_t val;
				if (read(pipefd[0], &val, sizeof(val)) !=
				    sizeof(val)) {
					logg.logMessage(
						"Reading annotate pipe failed");
				}
