...
void TtraceDriver::readEvents(mxml_node_t *const xml)
{
	...
	while (true) {
		...
		if (flagStr == NULL) {
			logg.logError(
				"The ttrace counter %s is missing the required flag attribute",
				counter);
			handleException();
		}
		int flag;
		if (!stringToInt(&flag, flagStr, 16)) {
			logg.logError(
				"The flag attribute of the ttrace counter %s is not a hex integer",
				counter);
			handleException();
		}
...
void TtraceDriver::setTtrace(const int flags)
{
	...
	const int fd =
		open("/tmp/ttrace_tag", O_CREAT | O_RDWR | O_CLOEXEC, 0666);
	if (fd < 0) {
	...
	uint64_t *const buf = static_cast<uint64_t *>(
		mmap(NULL, sizeof(uint64_t), PROT_READ | PROT_WRITE, MAP_SHARED,
		     fd, 0));
	if (buf == MAP_FAILED) {
...
void TtraceDriver::start()
{
	...
	int flags = 0;
	for (TtraceCounter *counter =
		     static_cast<TtraceCounter *>(getCounters());
	     counter != NULL;
