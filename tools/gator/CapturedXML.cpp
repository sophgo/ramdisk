...
static const char *detectOs()
{
	...
	lib::FsEntry app_process =
		lib::FsEntry::create("/system/bin/app_process");
...
static mxml_node_t *getTree(bool includeTime, lib::Span<const CapturedSpe> spes,
			    ...
			    const std::map<unsigned, unsigned> &maliGpuIds)
{
	...
	mxmlElementSetAttr(
		captured, "backtrace_processing",
		(gSessionData.mBacktraceDepth > 0) ?
			primarySourceProvider.getBacktraceProcessingMode() :
			"none");
	mxmlElementSetAttr(captured, "type",
			   primarySourceProvider.getCaptureXmlTypeValue());
	mxmlElementSetAttrf(captured, "protocol", "%d", PROTOCOL_VERSION);
	if (includeTime) { // Send the following only after the capture is complete
		if (time(NULL) >
		    1267000000) { // If the time is reasonable (after Feb 23, 2010)
			mxmlElementSetAttrf(
				captured, "created", "%lu",
				time(NULL)); // Valid until the year 2038
		}
	}

	target = mxmlNewElement(captured, "target");
	mxmlElementSetAttrf(target, "sample_rate", "%d",
	...
	mxmlElementSetAttrf(target, "gatord_build_id", "%s",
			    STRIFY(GATORD_BUILD_ID));

	assert(cpuIds.size() >
	       0); // gatord should've died earlier if there were no cpus
	mxmlElementSetAttrf(target, "cpuid", "0x%x",
			    *std::max_element(begin(cpuIds), end(cpuIds)));
	...
	mxmlElementSetAttrf(target, "os_version", GATOR_TARGET_OS_VERSION_FMT,
			    GATOR_TARGET_OS_VERSION);
#endif
#endif
	...
	for (x = 0; x < MAX_PERFORMANCE_COUNTERS; x++) {
		...
		if (counter.isEnabled()) {
			...
			mxml_node_t *const node =
				mxmlNewElement(counters, "counter");
			mxmlElementSetAttrf(node, "key", "0x%x",
					    counter.getKey());
			mxmlElementSetAttr(node, "type", counter.getType());
			if (counter.getEvent() != -1) {
				mxmlElementSetAttrf(node, "event", "0x%x",
						    counter.getEvent());
			}
			if (counter.getCount() > 0) {
				mxmlElementSetAttrf(node, "count", "%d",
						    counter.getCount());
			}
			if (counter.getCores() > 0) {
				mxmlElementSetAttrf(node, "cores", "%d",
						    counter.getCores());
			}
		}
	}
...
getXML(bool includeTime, lib::Span<const CapturedSpe> spes,
       const PrimarySourceProvider &primarySourceProvider,
       const std::map<unsigned, unsigned> &maliGpuIds)
{
	mxml_node_t *xml =
		getTree(includeTime, spes, primarySourceProvider, maliGpuIds);
	char *xml_string = mxmlSaveAllocString(xml, mxmlWhitespaceCB);
	mxmlDelete(xml);
	return { xml_string, &free };
}

void write(const char *path, lib::Span<const CapturedSpe> spes,
	   const PrimarySourceProvider &primarySourceProvider,
	   const std::map<unsigned, unsigned> &maliGpuIds)
{
	char file[PATH_MAX];
	...
	if (writeToDisk(file,
			getXML(true, spes, primarySourceProvider, maliGpuIds)
				.get()) < 0) {
