...
unsigned int getMaxCoreNum()
{
	...
	if (dir == NULL) {
		logg.logError(
			"Unable to determine the number of cores on the target, opendir failed");
		handleException();
	}
	...
	while ((dirent = readdir(dir)) != NULL) {
		if (strncmp(dirent->d_name, "cpu", 3) == 0) {
			long coreNum;
			if (stringToLong(&coreNum, dirent->d_name + 3, 10) &&
			    (coreNum >= maxCoreNum)) {
	...
	if (maxCoreNum < 1) {
		logg.logError(
			"Unable to determine the number of cores on the target, no cpu# directories found");
		handleException();
	}
...
static std::string parseProcCpuInfo(bool justGetHardwareName,
				    lib::Span<int> cpuIds)
{
	...
	while (fgets(temp, sizeof(temp), f)) {
		...
		const bool foundHardware =
			!foundCoreName &&
			strncmp(temp, HARDWARE, sizeof(HARDWARE) - 1) == 0;
		const bool foundCPUImplementer =
			strncmp(temp, CPU_IMPLEMENTER,
				sizeof(CPU_IMPLEMENTER) - 1) == 0;
		const bool foundCPUPart =
			strncmp(temp, CPU_PART, sizeof(CPU_PART) - 1) == 0;
		const bool foundProcessor =
			strncmp(temp, PROCESSOR, sizeof(PROCESSOR) - 1) == 0;
		if (foundHardware || foundCPUImplementer || foundCPUPart ||
		    foundProcessor) {
			char *position = strchr(temp, ':');
			if (position == NULL ||
			    static_cast<unsigned int>(position - temp) + 2 >=
				    strlen(temp)) {
				logg.logMessage(
					"Unknown format of /proc/cpuinfo\n"
					"The core name in the captured xml file will be 'unknown'.");
				return hardwareName;
			}
			position += 2;
			...
			if (foundCPUImplementer) {
				...
				if (!stringToInt(&implementer, position, 0)) {
					...
					setImplementer(cpuIds[processor],
						       implementer);
				} else {
					setImplementer(outOfPlaceCpuId,
						       implementer);
					invalidFormat = true;
				}
			}
			...
			if (foundProcessor) {
				...
				const bool converted =
					stringToInt(&processorId, position, 0);
				...
				if (foundProcessorInSection) {
					...
					if (processor >= cpuIds.length) {
						logg.logError(
							"Found processor %zu but max is %zu",
							processor,
							cpuIds.length);
						handleException();
					}
	...
	if (invalidFormat && (outOfPlaceCpuId != -1) &&
	    (minProcessor <= maxProcessor)) {
		minProcessor = (minProcessor > 0 ? minProcessor : 0);
		maxProcessor =
			(maxProcessor < cpuIds.length ? maxProcessor + 1 :
							cpuIds.length);

		for (size_t processor = minProcessor; processor < maxProcessor;
		     ++processor) {
			if (cpuIds[processor] == -1) {
				logg.logMessage(
					"Setting global CPUID 0x%x for processors %zu ",
					outOfPlaceCpuId, processor);
	...
	if (!foundCoreName) {
		logg.logMessage(
			"Could not determine core name from /proc/cpuinfo\n"
			"The core name in the captured xml file will be 'unknown'.");
	}
...
std::string readCpuInfo(bool ignoreOffline, lib::Span<int> cpuIds)
{
	...
	std::vector<std::unique_ptr<PerCoreIdentificationThread> >
		perCoreThreads;
	...
	//   to ensure that the cores remain online until cpuinfo is read
	{
				...
				[&](unsigned c, unsigned /*core_id*/,
					...
					if (physical_package_id !=
						...
						if (midr_el1 !=
						    PerCoreIdentificationThread::
							    INVALID_MIDR_EL1) {
							clusterToCpuIds[physical_package_id]
								.insert(cpuId);
						}
						...
						for (int sibling :
							...
							if (cpuToCluster.count(
								    sibling_cpu) ==
							    0) {
								cpuToCluster[sibling_cpu] =
									physical_package_id;
							}
						}
					}
		...
		cv.wait_for(lock, std::chrono::seconds(10), [&] {
			return identificationThreadCallbackCounter >=
			       cpuIds.length;
		});
	}
	...
	for (const auto &pair : cpuToCpuIds) {
		logg.logMessage("Read CPU %u CPUID from MIDR_EL1 -> 0x%05x",
				pair.first, pair.second);
	}
	for (const auto &pair : cpuToCluster) {
		logg.logMessage("Read CPU %u CLUSTER %u", pair.first,
				pair.second);
	}
	...
	std::string hardwareName = parseProcCpuInfo(
		/* justGetHardwareName = */ knowAllMidrValues, cpuIds);

	// update/set known items from MIDR map and topology information. This will override anything read from /proc/cpuinfo
	updateCpuIdsFromTopologyInformation(cpuIds, cpuToCpuIds, cpuToCluster,
					    clusterToCpuIds);
