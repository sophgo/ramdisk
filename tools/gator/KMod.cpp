...
{
	return ((strncmp(counter.getType(), ARM_MALI_MIDGARD,
			 sizeof(ARM_MALI_MIDGARD) - 1) == 0) ||
		(strncmp(counter.getType(), ARM_MALI_BIFROST,
			 sizeof(ARM_MALI_BIFROST) - 1) == 0) ||
		(strncmp(counter.getType(), ARM_MALI_T,
			 sizeof(ARM_MALI_T) - 1) == 0));
}
...
void KMod::resetCounters()
{
	...
	if (dir) {
		while ((ent = readdir(dir)) != NULL) {
			...
			if (ent->d_name[0] == '.')
				continue;
			snprintf(base, sizeof(base), "/dev/gator/events/%s",
				 ent->d_name);
...
void KMod::setupCounter(Counter &counter)
{
	...
	if (access(text, F_OK) == 0) {
		...
		if (lib::writeReadIntInFile(text, count) &&
		    counter.getCount() > 0) {
			logg.logError(
				"Cannot enable EBS for %s:%i with a count of %d",
		...
		logg.logError(
			"Event Based Sampling is only supported with kernel versions 3.0.0 and higher with CONFIG_PERF_EVENTS=y, and CONFIG_HW_PERF_EVENTS=y. The invalid configuration.xml has been removed.");
		handleException();
	}
}
...
void KMod::checkVersion()
{
	...
	if (driverVersion != PROTOCOL_VERSION) {
		if ((driverVersion > PROTOCOL_DEV) ||
		    (PROTOCOL_VERSION > PROTOCOL_DEV)) {
			...
			logg.logError(
				...
				">> * gator daemon\n"
				">> * Streamline",
				driverVersion, PROTOCOL_VERSION);
			handleException();
		} else {
			// Release version mismatch
			logg.logError(
				"gator driver version \"%d\" is different than gator daemon version \"%d\".\n"
				">> Please upgrade the driver and daemon to the latest versions.",
				driverVersion, PROTOCOL_VERSION);
			handleException();
		}
	}
...
std::vector<GatorCpu> KMod::writePmuXml(const PmuXML &pmuXml)
{
	...
	for (const GatorCpu &gatorCpu : pmuXml.cpus) {
		snprintf(buf, sizeof(buf), "/dev/gator/pmu/%s",
			 gatorCpu.getId());
		if (access(buf, X_OK) == 0) {
			continue;
		}
		if (gatorCpu.getCpuIds().size() != 1) {
			logg.logMessage("CPU %s is not supported by gator.ko",
					gatorCpu.getCoreName());
			continue;
		}
		const int cpuId = *gatorCpu.getCpuIds().begin();
		lib::writeCStringToFile("/dev/gator/pmu/export",
					gatorCpu.getId());
		snprintf(buf, sizeof(buf), "/dev/gator/pmu/%s/cpuid",
			 gatorCpu.getId());
		lib::writeIntToFile(
			buf,
			cpuId); // gator.ko does not support multiple cpuids
		snprintf(buf, sizeof(buf), "/dev/gator/pmu/%s/core_name",
			 gatorCpu.getId());
		lib::writeCStringToFile(buf, gatorCpu.getCoreName());
		if (gatorCpu.getDtName() != NULL) {
			snprintf(buf, sizeof(buf), "/dev/gator/pmu/%s/dt_name",
				 gatorCpu.getId());
			lib::writeCStringToFile(buf, gatorCpu.getDtName());
		}
		snprintf(buf, sizeof(buf), "/dev/gator/pmu/%s/pmnc_counters",
			 gatorCpu.getId());
		lib::writeIntToFile(buf, gatorCpu.getPmncCounters());
	}

	for (const UncorePmu &uncorePmu : pmuXml.uncores) {
		snprintf(buf, sizeof(buf), "/dev/gator/uncore_pmu/%s",
			 uncorePmu.getId());
		if (access(buf, X_OK) == 0) {
			continue;
		}
		lib::writeCStringToFile("/dev/gator/uncore_pmu/export",
					uncorePmu.getId());
		snprintf(buf, sizeof(buf), "/dev/gator/uncore_pmu/%s/core_name",
			 uncorePmu.getId());
		lib::writeCStringToFile(buf, uncorePmu.getCoreName());
		snprintf(buf, sizeof(buf),
			 "/dev/gator/uncore_pmu/%s/pmnc_counters",
			 uncorePmu.getId());
		lib::writeIntToFile(buf, uncorePmu.getPmncCounters());
		snprintf(buf, sizeof(buf),
			 "/dev/gator/uncore_pmu/%s/has_cycles_counter",
			 uncorePmu.getId());
		lib::writeIntToFile(buf, uncorePmu.getHasCyclesCounter());
		snprintf(buf, sizeof(buf), "/dev/gator/uncore_pmu/%s/cpumask",
			 uncorePmu.getId());
	...
	bool foundCpu = false;
	for (const GatorCpu &gatorCpu : pmuXml.cpus) {
		snprintf(buf, sizeof(buf), "/dev/gator/events/%s_cnt0",
			 gatorCpu.getId());
		if (access(buf, X_OK) == 0) {
			foundCpu = true;
			break;
