...
static int getUid(const char *const name, const char *const tmpDir,
		  uid_t *const uid)
{
	...
	char cmd[128];
	snprintf(cmd, sizeof(cmd), "chown %s %s || rm -f %s", name, gatorTemp,
		 gatorTemp);
...
static void checkCommandStatus(int status)
{
	if (WIFEXITED(status)) {
		...
		if ((gSessionData.mCaptureCommand.size() == 3) &&
		    (gSessionData.mCaptureCommand[0] == "sh") &&
			...
			if (exitCode == 127) {
				logg.logError(
					"Failed to run command %s: Command not found",
					gSessionData.mCaptureCommand[2].c_str());
				handleException();
			}
		}
		...
		if (signal != SIGTERM &&
		    signal != SIGINT) // should we consider any others normal?
			logg.logError("command terminated abnormally: %s",
				      strsignal(signal));
	}
}

Command runCommand(sem_t &waitToStart,
		   std::function<void()> terminationCallback)
{
	...
	if (name != NULL) {
		...
		if (!isRoot) {
			logg.logError(
				"Unable to set user to %s for command because gatord is not running as root",
				name);
			handleException();
		}
		...
		if (!getUid(name, &uid, &gid)) {
			logg.logError(
				"Unable to look up the user %s, please double check that the user exists",
				name);
			handleException();
		}
	}
	...
	if (pid == 0) {
		...
		prctl(PR_SET_NAME,
		      reinterpret_cast<unsigned long>(&"gatord-command"), 0, 0,
		      0);
...

		{
			const char *const path =
				gSessionData.mCaptureWorkingDir == NULL ?
					"/" :
		...
		sem_post(
			&waitToStart); // some other thread might be waiting on this too.

		prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(commands[0]),
		      0, 0, 0);
	...
	fail_exit:
		...
		return {
			pid, std::thread{ [pipefd, pid, terminationCallback]() {
				prctl(PR_SET_NAME,
				      reinterpret_cast<unsigned long>(
					      &"gatord-command-reader"),
				      0, 0, 0);
				char buf[bufSize];
				ssize_t bytesRead = 0;
				while (true) {
					const ssize_t bytes =
						read(pipefd[0], buf + bytesRead,
					...
					if (bytes > 0) {
						...
						buf[bytesRead] = '\0';
						logg.logError(
							"Failed to read pipe from child: %s",
							strerror(errno));
						break;
					}
				}
				...
				if (bytesRead > 0) {
					...
					while (true) {
						...
						if (waitpid(pid, &status, 0) !=
							...
							logg.logMessage(
								"Could not waitpid on child command. (%s)",
								strerror(
									errno));
						}
						break;
