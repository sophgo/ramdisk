...
class NetCounter : public DriverCounter {
    public:
	NetCounter(DriverCounter *next, const char *const name,
...
NetCounter::NetCounter(DriverCounter *next, const char *const name,
		       uint64_t *const value)
...
NetDriver::NetDriver()
	: PolledDriver("Net"), mBuf(), mReceiveBytes(0), mTransmitBytes(0)
{
}
...
void NetDriver::readEvents(mxml_node_t *const)
{
	if (access("/proc/net/dev", R_OK) == 0) {
		setCounters(new NetCounter(getCounters(), "Linux_net_rx",
					   &mReceiveBytes));
		setCounters(new NetCounter(getCounters(), "Linux_net_tx",
					   &mTransmitBytes));
	} else {
		logg.logSetup(
