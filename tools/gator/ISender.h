...
class ISender {
     ...
     */
	virtual void
	writeDataParts(lib::Span<const lib::Span<const char, int> > dataParts,
		       ResponseType type, bool ignoreLockErrors = false) = 0;

	void writeData(const char *data, int length, ResponseType type,
		       bool ignoreLockErrors = false)
	{
		lib::Span<const char, int> dataSpan = { data, length };
		writeDataParts(
			lib::Span<const lib::Span<const char, int> >{ &dataSpan,
								      1 },
			type, ignoreLockErrors);
	}
