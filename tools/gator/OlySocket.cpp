...
int socket_cloexec(int domain, int type, int protocol)
{
	...
	if (sock >= 0) {
		...
		logg.logMessage("Failed socket %i/%i/%i CLOEXEC due to %i %s",
	...
	if (sock < 0) {
		logg.logMessage(
			"Failed socket {domain = %i, type = %i, protocol = %i} due to %i (%s)",
			domain, type, protocol, errno, strerror(errno));
		return -1;
	}
	...
	if ((fdf == -1) || (fcntl(sock, F_SETFD, fdf | FD_CLOEXEC) != 0)) {
		logg.logMessage(
			...
			domain, type, protocol, sock, fdf, errno,
			strerror(errno));
		close(sock);
		return -1;
	}
	...
	: mFDServer(0)
{
	...
	memcpy(sockaddr.sun_path, path,
	       MIN(pathSize, sizeof(sockaddr.sun_path)));
	...
	if (bind(mFDServer,
		 ...
		 calculateAddrlen ?
			 offsetof(struct sockaddr_un, sun_path) + pathSize - 1 :
			 sizeof(sockaddr)) < 0) {
		//                                                                    use sun_path because it is null terminated
		//                                                                    if path was actually empty string
		//                                                                    vv
		const char *const printablePath =
			path[0] != '\0' ? path : &sockaddr.sun_path[1];
		logg.logError("Binding of server socket to '%s' failed: %s",
			      printablePath, strerror(errno));
		handleException();
	}
...
void OlyServerSocket::createServerSocket(int port)
{
	...
	if (mFDServer < 0) {
		...
		if (mFDServer < 0) {
			logg.logError(
				"Error creating server TCP socket (%d : %s)",
	...
	int on = 1;
	if (setsockopt(mFDServer, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) !=
	    0) {
	...
	on = 0;
	if (setsockopt(mFDServer, IPPROTO_IPV6, IPV6_V6ONLY, &on, sizeof(on)) !=
	    0) {
	...
	if (bind(mFDServer,
		 ...
		 sizeof(sockaddr)) < 0) {
		logg.logError(
			"Binding of server socket on port %i failed.\nIs an instance already running or is another application using that port?",
			port);
		handleException();
	}
...
int OlyServerSocket::acceptConnection()
{
	...
	if (mFDServer <= 0) {
		logg.logError(
			"Attempting multiple connections on a single connection server socket or attempting to accept on a client socket");
		handleException();
	}
...
void OlySocket::send(const char *buffer, int size)
{
	...
	while (size > 0) {
		...
		if (n < 0) {
			logg.logError("Socket send error (%d): %s", errno,
				      strerror(errno));
			handleException();
		}
		size -= n;
...
int OlySocket::receive(char *buffer, int size)
{
	...
	if (bytes < 0) {
		logg.logError("Socket receive error (%d): %s", errno,
			      strerror(errno));
		handleException();
	} else if (bytes == 0) {
