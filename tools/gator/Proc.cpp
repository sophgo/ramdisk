...
namespace
{
class ReadProcSysDependenciesPollerVisiter
	: private lnx::ProcessPollerBase::IProcessPollerReceiver,
	  private lnx::ProcessPollerBase {
    public:
	ReadProcSysDependenciesPollerVisiter(uint64_t currTime_,
					     IPerfAttrsConsumer &buffer_)
	...
	virtual void
	onThreadDetails(int pid, int tid,
	...
	{
		buffer.marshalComm(currTime, pid, tid,
				   (exe ? exe->path().c_str() : ""),
				   statRecord.getComm().c_str());
	}
};

class ReadProcMapsPollerVisiter
	: private lnx::ProcessPollerBase::IProcessPollerReceiver,
	  private lnx::ProcessPollerBase {
    public:
	ReadProcMapsPollerVisiter(uint64_t currTime_,
				  IPerfAttrsConsumer &buffer_)
		: currTime(currTime_), buffer(buffer_)
	{
	}
	...
	virtual void onProcessDirectory(int pid,
					const lib::FsEntry &path) override
	{
		const lib::FsEntry mapsFile =
			lib::FsEntry::create(path, "maps");
		const std::string mapsContents =
...
bool readProcSysDependencies(const uint64_t currTime,
			     IPerfAttrsConsumer &buffer, DynBuf *const printb,
			     DynBuf *const b1, FtraceDriver &ftraceDriver)
{
...
bool readKallsyms(const uint64_t currTime, IPerfAttrsConsumer &attrsConsumer,
		  const std::atomic_bool &isDone)
{
	...
	while (!isDone) {
...

		{
			...
			const ssize_t bytes =
				::read(fd, buf + pos, sizeof(buf) - pos - 1);
			if (bytes < 0) {
		...
		for (newline = pos - 1; newline >= 0; --newline) {
			if (buf[newline] == '\n') {
				...
				if (pos - newline >= newline + 1) {
					logg.logError(
						"memcpy src and dst overlap");
					handleException();
				}
				if (pos - newline - 2 > 0) {
					memcpy(buf + 1, buf + newline + 2,
					       pos - newline - 2);
				}
				pos -= newline + 1;
				break;
