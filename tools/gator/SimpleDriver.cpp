...
{
	for (DriverCounter *counter = mCounters; counter != NULL;
...
DriverCounter *SimpleDriver::findCounter(Counter &counter) const
{
	...
	for (DriverCounter *driverCounter = mCounters; driverCounter != NULL;
	     driverCounter = driverCounter->getNext()) {
		if (strcasecmp(driverCounter->getName(), counter.getType()) ==
		    0) {
			...
			std::string driverCounterName =
				driverCounter->getName();
			std::string counterType = counter.getType();
			counterType = counterType + "_cnt";
			driverCounterName = driverCounterName.substr(
				0, counterType.length());
			if (strcasecmp(driverCounterName.c_str(),
				       counterType.c_str()) == 0) {
				dcounter = driverCounter;
				counter.setType(driverCounter->getName());
				break;
