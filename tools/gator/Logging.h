...
#define logError(...) _logError(__func__, __FILE__, __LINE__, __VA_ARGS__)
	...
	_logError(const char *function, const char *file, int line,
		  const char *fmt, ...);
...
#define logSetup(...) _logSetup(__func__, __FILE__, __LINE__, __VA_ARGS__)
	__attribute__((format(printf, 5, 6))) void
	_logSetup(const char *function, const char *file, int line,
		  const char *fmt, ...);
	const char *getSetup()
	{
...
#define logMessage(...) _logMessage(__func__, __FILE__, __LINE__, __VA_ARGS__)
	__attribute__((format(printf, 5, 6))) void
	_logMessage(const char *function, const char *file, int line,
		    const char *fmt, ...);

#define logWarning(...) _logWarning(__func__, __FILE__, __LINE__, __VA_ARGS__)
	__attribute__((format(printf, 5, 6))) void
	_logWarning(const char *function, const char *file, int line,
		    const char *fmt, ...);
