...
static void _MXML_FINI(void)
{
	...
	if (_mxml_key != -1) {
		if ((global = (_mxml_global_t *)pthread_getspecific(
			     _mxml_key)) != NULL)
...
_mxml_global(void)
{
	...
	if ((global = (_mxml_global_t *)pthread_getspecific(_mxml_key)) ==
	    NULL) {
	...
	LPVOID reserved) /* I - Unused */
{
	...
	case DLL_THREAD_DETACH: /* Called when a thread terminates */
		if ((global = (_mxml_global_t *)TlsGetValue(_mxml_tls_index)) !=
		    NULL)
			free(global);
		break;
