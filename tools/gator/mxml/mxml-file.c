...
static int mxml_get_entity(mxml_node_t *parent, void *p, int *encoding,
			   _mxml_getc_cb_t getc_cb, int *line);
	   ...
	   mxml_load_cb_t cb) /* I - Callback function or constant */
{
  ...
  */
	...
	return (mxml_load_data(top, &buf, cb, mxml_fd_getc, MXML_NO_CALLBACK,
			       NULL));
}
	     ...
	     mxml_load_cb_t cb) /* I - Callback function or constant */
{
  ...
  */
	...
	return (mxml_load_data(top, fp, cb, mxml_file_getc, MXML_NO_CALLBACK,
			       NULL));
}
...
mxmlSaveAllocString(
	...
	mxml_save_cb_t
		cb) /* I - Whitespace callback or @code MXML_NO_CALLBACK@ */
{
		   ...
		   cb) /* I - Whitespace callback or @code MXML_NO_CALLBACK@ */
{
  ...
  */
	...
	if ((col = mxml_write_node(node, &buf, cb, 0, mxml_fd_putc, global)) <
	    0)
		return (-1);
		     ...
		     cb) /* I - Whitespace callback or @code MXML_NO_CALLBACK@ */
{
  ...
  */
	...
	if ((col = mxml_write_node(node, fp, cb, 0, mxml_file_putc, global)) <
	    0)
		return (-1);
		...
		cb) /* I - Whitespace callback or @code MXML_NO_CALLBACK@ */
{
  ...
  */
	...
	if ((col = mxml_write_node(node, ptr, cb, 0, mxml_string_putc,
				   global)) < 0)
		return (-1);
...
mxml_fd_read(_mxml_fdbuf_t *buf) /* I - File descriptor buffer */
{
  ...
  */
	...
	while ((bytes = (int)read(buf->fd, buf->buffer, sizeof(buf->buffer))) <
	       0)
#ifdef EINTR
		...
		int *line) /* IO - Current line number */
{
	...
	*entptr = '\0';
	...
	if (entity[0] == '#') {
		...
		mxml_error(
			"Entity name '%s;' not supported under parent <%s> on line %d.",
			entity, parent ? parent->value.element.name : "null",
			*line);

	if (mxml_bad_char(ch)) {
		mxml_error(
			"Bad control character 0x%02x under parent <%s> on line %d not allowed by XML standard.",
  ...
  */
	...
	while ((ch = (*getc_cb)(p, &encoding)) != EOF) {
      ...
      */
			...
			*bufptr = '\0';
			...
			case MXML_REAL:
				node = mxmlNewReal(parent,
						   strtod(buffer, &bufptr));
				break;
          ...
          */
					...
					if ((*global->custom_load_cb)(node,
								      buffer)) {
						mxml_error(
							"Bad custom value '%s' in parent <%s> on line %d.",
							buffer,
							parent ?
								parent->value
									.element
    ...
    */
      ...
      */
				...
				if ((node = mxmlNewElement(parent, buffer)) ==
				    NULL) {
					/*
      ...
      */
					...
					mxml_error(
						"Unable to add comment node to parent <%s> on line %d.",
    ...
    */
      ...
      */
					...
					mxml_error(
						"Unable to add CDATA node to parent <%s> on line %d.",
						parent ? parent->value.element
								 .name :
							 "null",
						line);
    ...
    */
      ...
      */
					...
					mxml_error(
						"Unable to add declaration node to parent <%s> on line %d.",
						parent ? parent->value.element
								 .name :
							 "null",
						line);
    ...
    */
      ...
      */
					...
					mxml_error(
						"Unable to add element node to parent <%s> on line %d.",
						parent ? parent->value.element
								 .name :
							 "null",
						line);
				...
				if (mxml_isspace(ch)) {
					...
					if ((ch = (*getc_cb)(p, &encoding)) !=
					    '>') {
						mxml_error(
							"Expected > but got '%c' instead for element <%s/> on line %d.",
							ch, buffer, line);
						mxmlDelete(node);
						goto error;
					}
      ...
      */
			...
			if ((ch = mxml_get_entity(parent, p, &encoding, getc_cb,
						  &line)) == EOF)
				goto error;
  ...
  */
    ...
    */
      ...
      */
			...
			mxml_error("Bare < in element %s on line %d.",
		...
		*ptr = '\0';
		...
		if (mxmlElementGetAttr(node, name)) {
			mxml_error(
				...
				name, node->value.element.name, name, *line);
			goto error;
		}
      ...
      */
			...
			if (ch == EOF) {
				mxml_error(
					...
					name, node->value.element.name, *line);
				goto error;
			}
		...
		_mxml_global_t *global) /* I - Global data */
{
  ...
  */
    ...
    */
		...
		switch (current->type) {
		case MXML_ELEMENT:
			col = mxml_write_ws(current, p, cb, MXML_WS_BEFORE_OPEN,
					    col, putc_cb);
        ...
        */
				...
				for (ptr = current->value.element.name; *ptr;
				     ptr++)
					if ((*putc_cb)(*ptr, p) < 0)
						return (-1);
			} else if (!strncmp(current->value.element.name,
					    "![CDATA[", 8)) {
				/*
        ...
        */
				...
				if ((*putc_cb)(']', p) < 0)
					return (-1);
			} else if (mxml_write_name(current->value.element.name,
						   p, putc_cb) < 0)
				return (-1);

			col += strlen(current->value.element.name) + 1;

			for (i = current->value.element.num_attrs,
        ...
        */
				...
				col = mxml_write_ws(current, p, cb,
        ...
        */
		...
		case MXML_OPAQUE:
			if (mxml_write_string(current->value.opaque, p,
					      putc_cb) < 0)
				return (-1);
		...
		case MXML_TEXT:
			...
			if (mxml_write_string(current->value.text.string, p,
					      putc_cb) < 0)
				return (-1);
