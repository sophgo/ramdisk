     ...
     char *argv[]) /* I - Command-line args */
{
  ...
  */
	...
	if (tree->type != MXML_ELEMENT) {
		fprintf(stderr,
			"ERROR: Parent has type %s (%d), expected MXML_ELEMENT.\n",
	...
	if (strcmp(tree->value.element.name, "element")) {
		fprintf(stderr,
			"ERROR: Parent value is \"%s\", expected \"element\".\n",
	...
	mxmlLoadString(tree,
		       "<group type='string'>string string string</group>",
		       MXML_NO_CALLBACK);
	...
	if (node->type != MXML_INTEGER) {
		fprintf(stderr,
			"ERROR: First child has type %s (%d), expected MXML_INTEGER.\n",
	...
	if (node->value.integer != 123) {
		fprintf(stderr,
			"ERROR: First child value is %d, expected 123.\n",
			node->value.integer);
		mxmlDelete(tree);
		return (1);
	}
	...
	if (node->type != MXML_OPAQUE) {
		fprintf(stderr,
			"ERROR: Second child has type %s (%d), expected MXML_OPAQUE.\n",
			node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
				"UNKNOWN" :
				types[node->type],
			node->type);
	...
	if (node->type != MXML_REAL) {
		fprintf(stderr,
			"ERROR: Third child has type %s (%d), expected MXML_REAL.\n",
			node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
				"UNKNOWN" :
				types[node->type],
			node->type);
	...
	if (node->value.real != 123.4f) {
		fprintf(stderr,
			"ERROR: Third child value is %f, expected 123.4.\n",
			node->value.real);
		mxmlDelete(tree);
		return (1);
	}
	...
	if (node->type != MXML_TEXT) {
		fprintf(stderr,
			"ERROR: Fourth child has type %s (%d), expected MXML_TEXT.\n",
			node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
				"UNKNOWN" :
				types[node->type],
			node->type);
	...
	if (!node->value.text.whitespace || !node->value.text.string ||
		...
		fprintf(stderr,
			...
			node->value.text.string ? node->value.text.string :
						  "(null)");
		mxmlDelete(tree);
		return (1);
	}
	...
	for (i = 0; i < 4; i++) {
		...
		if (!node) {
			fprintf(stderr,
				"ERROR: No group #%d child node in basic test.\n",
				i + 1);
			mxmlDelete(tree);
			return (1);
		}
  ...
  */
	...
	if (!node) {
		...
		return (1);
		   ...
		   strcmp(node->value.opaque, "value")) {
  ...
  */
	...
	for (i = 0; i < 12; i++) {
		if (tree->child)
			...
			fprintf(stderr,
				"ERROR: Child pointer prematurely NULL on child #%d\n",
				i + 1);
			mxmlDelete(tree);
			return (1);
		}
  ...
  */
    ...
    */
		...
		if ((node = mxmlFindPath(tree, "group/option/keyword")) ==
		    NULL) {
		...
		if ((text = mxmlGetText(node, NULL)) == NULL ||
		    strcmp(text, "InputSlot")) {
			fprintf(stderr,
				"Child node of group/option/value has value \"%s\" instead of \"InputSlot\".\n",
				text ? text : "(null)");
			mxmlDelete(tree);
			return (1);
		}
  ...
  */
    ...
    */
		...
		if (!mxmlFindElement(node, tree, "choice", NULL, NULL,
				     MXML_NO_DESCEND)) {
  ...
  */
    ...
    */
		...
		if ((fd = open(buffer, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY,
			       0666)) < 0) {
			perror(buffer);
			mxmlDelete(tree);
			return (1);
    ...
    */
	...
	if (!strcmp(argv[1], "test.xml")) {
		if (event_counts[MXML_SAX_CDATA] != 1) {
			fprintf(stderr,
				"MXML_SAX_CDATA seen %d times, expected 1 times.\n",
		...
		if (event_counts[MXML_SAX_COMMENT] != 1) {
			fprintf(stderr,
				"MXML_SAX_COMMENT seen %d times, expected 1 times.\n",
		...
		if (event_counts[MXML_SAX_DATA] != 60) {
			fprintf(stderr,
				"MXML_SAX_DATA seen %d times, expected 60 times.\n",
		...
		if (event_counts[MXML_SAX_DIRECTIVE] != 1) {
			fprintf(stderr,
				"MXML_SAX_DIRECTIVE seen %d times, expected 1 times.\n",
	    ...
	    void *data) /* I - SAX user data */
{
  ...
  */
	...
	if (!node)
		fprintf(stderr,
			"ERROR: SAX callback for event %s has NULL node.\n",
			events[event]);
	      ...
	      int where) /* I - Open or close tag? */
{
  ...
  */
	...
	if (!strcmp(name, "html") || !strcmp(name, "head") ||
	    !strcmp(name, "body") || !strcmp(name, "pre") ||
	    !strcmp(name, "p") || !strcmp(name, "h1") || !strcmp(name, "h2") ||
	    !strcmp(name, "h3") || !strcmp(name, "h4") || !strcmp(name, "h5") ||
	    !strcmp(name, "h6")) {
		/*
