  ...
  */
    ...
    */
		...
		while ((last - first) > 1) {
			...
			if ((diff = index_find(ind, element, value,
					       ind->nodes[current])) == 0) {
				/*
    ...
    */
		...
		for (current = first; current <= last; current++)
			if (!index_find(ind, element, value,
					ind->nodes[current])) {
				/*
...
mxmlIndexNew(
	mxml_node_t *node, /* I - XML node tree */
	...
	const char *attr) /* I - Attribute to index or @code NULL@ for none */
{
  ...
  */
	...
	if (!element && !attr)
		...
		current = mxmlFindElement(node, node, element, attr, NULL,
					  MXML_DESCEND);
	   ...
	   mxml_node_t *node) /* I - Node */
{
  ...
  */
	...
	if (value) {
		if ((diff = strcmp(value,
				   mxmlElementGetAttr(node, ind->attr))) != 0)
			return (diff);
	}
  ...
  */
    ...
    */
      ...
      */
			...
			while ((templ < right) &&
			       index_compare(ind, ind->nodes[templ], pivot) <=
				       0)
				templ++;
