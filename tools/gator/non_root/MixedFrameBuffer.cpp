...
MixedFrameBuffer::Frame::Frame(MixedFrameBuffer &parent_,
			       std::uint64_t currentTime_, FrameType frameType,
			       std::int32_t core)
	: parent(parent_), currentTime(currentTime_),
	  bytesAvailable(parent.buffer.bytesAvailable() -
			 buffer_utils::MAX_FRAME_HEADER_SIZE),
	  frameStart(-1), valid(false)
{
	if (bytesAvailable >= 0) {
...
bool MixedFrameBuffer::counterFrameMessage(std::uint64_t currentTime,
					   std::int32_t core, std::int32_t key,
					   std::uint64_t value)
{
...
bool MixedFrameBuffer::schedFrameThreadExitMessage(std::uint64_t currentTime,
						   std::int32_t core,
						   std::int32_t tid)
{
	Frame frame(*this, currentTime, FrameType::SCHED_TRACE, core);
...
bool MixedFrameBuffer::summaryFrameSummaryMessage(
	std::uint64_t currentTime, std::uint64_t timestamp,
	std::uint64_t uptime, std::uint64_t monotonicDelta, const char *uname,
	unsigned long pageSize, bool nosync)
{
	MixedFrameBuffer::Frame frame(*this, currentTime, FrameType::SUMMARY,
				      0);
