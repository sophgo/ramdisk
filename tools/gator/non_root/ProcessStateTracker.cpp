...
namespace
{
	...
	convertClkTicksToNS(unsigned long long ticks,
			    unsigned long long bootTimeBaseNS,
			    unsigned long divider)
	{
		return std::max<long long>(
			(ticks * (1e9 / divider)) - bootTimeBaseNS, 0);
	}
} // namespace

ProcessStateTracker::ActiveScan::ActiveScan(ProcessStateTracker &parent_,
...
void ProcessStateTracker::ActiveScan::addProcess(
	int pid, int tid, const lnx::ProcPidStatFileRecord &statRecord,
	const lib::Optional<lnx::ProcPidStatmFileRecord> &statmRecord,
	const lib::Optional<lib::FsEntry> &exe)
{
	// forward to parent
	const unsigned long long processTimeDelta =
		parent.add(timestampNS, pid, tid, statRecord, statmRecord, exe);
...
ProcessStateTracker::ProcessInfo::ProcessInfo()
	: statsTracker(0, 0, 0), startTimeNS(0), parentPid(PARENT_PID_UNKNOWN),
	  state(State::EMPTY)
{
}

ProcessStateTracker::ProcessInfo::ProcessInfo(int pid_, int tid_,
					      unsigned long pageSize_,
					      unsigned long long timestampNS_)
	: statsTracker(pid_, tid_, pageSize_), startTimeNS(timestampNS_),
	  parentPid(PARENT_PID_UNKNOWN), state(State::NEW)
{
}

ProcessStateTracker::ProcessInfo::ProcessInfo(ProcessInfo &&that)
	: statsTracker(std::move(that.statsTracker)),
	  startTimeNS(that.startTimeNS), parentPid(that.parentPid),
	  state(that.state)
{
...
ProcessStateTracker::ProcessInfo &ProcessStateTracker::ProcessInfo::
operator=(ProcessInfo &&that)
{
...
unsigned long long ProcessStateTracker::ProcessInfo::update(
	unsigned long long bootTimeBaseNS, unsigned long clktck,
	...
	const lib::Optional<lib::FsEntry> &exe)
{
	if (isNew()) {
		...
		// update start time
		startTimeNS = convertClkTicksToNS(statRecord.getStarttime(),
						  bootTimeBaseNS, clktck);
	}
...
bool ProcessStateTracker::ProcessInfo::hasExitedAndRestartedSince(
	unsigned long long bootTimeBaseNS, unsigned long clktck, int pid,
	int tid, const lnx::ProcPidStatFileRecord &record)
{
	if (!isNew()) {
		...
		const unsigned long long recordStartTimeNS =
			convertClkTicksToNS(record.getStarttime(),
					    bootTimeBaseNS, clktck);
		...
		if ((parentPid != PARENT_PID_UNKNOWN) &&
		    (parentPid != record.getPpid())) {
			return true;
		}
	}
...
void ProcessStateTracker::ProcessInfo::sendStats(
	unsigned long long timestampNS, ProcessStateChangeHandler &handler,
...
ProcessStateTracker::ProcessStateTracker(ProcessStateChangeHandler &handler_,
					 ...
					 unsigned long pageSize_)
	: handler(handler_), lastTimestampNS(0),
	  bootTimeBaseNS(bootTimeBaseNS_), clktck(clktck_), pageSize(pageSize_),
	  trackedProcesses(), firstIteration(true)
{
}

std::unique_ptr<ProcessStateTracker::ActiveScan>
ProcessStateTracker::beginScan(unsigned long long timestampNS)
{
	return std::unique_ptr<ProcessStateTracker::ActiveScan>(
		new ActiveScan(*this, timestampNS));
}

void ProcessStateTracker::replaceProcessInfo(
	unsigned long long timestampNS,
	const lnx::ProcPidStatFileRecord &statRecord, ProcessInfo &processInfo)
{
	const unsigned long long newStartTimestampNS = convertClkTicksToNS(
		statRecord.getStarttime(), bootTimeBaseNS, clktck);
	const unsigned long long timestampToUse =
		std::min(timestampNS, newStartTimestampNS - 1);

	// send exit event
	sendProcessExit(timestampToUse, processInfo);

	// create new
	processInfo = ProcessInfo(statRecord.getPgid(), statRecord.getPid(),
				  pageSize, newStartTimestampNS);
}

unsigned long long ProcessStateTracker::add(
	unsigned long long timestampNS, int pid, int tid,
	const lnx::ProcPidStatFileRecord &statRecord,
	const lib::Optional<lnx::ProcPidStatmFileRecord> &statmRecord,
	const lib::Optional<lib::FsEntry> &exe)
{
	...
	auto &processInfo =
		getProcessInfoFor(timestampNS, statRecord.getPgid(), pid, tid);

	if (processInfo.hasExitedAndRestartedSince(bootTimeBaseNS, clktck, pid,
						   tid, statRecord)) {
	...
	return processInfo.update(bootTimeBaseNS, clktck, statRecord,
				  statmRecord, exe);
}
...
void ProcessStateTracker::endScan(
	...
	const std::map<unsigned long, unsigned long long>
		&accumulatedTimePerCore)
{
	runtime_assert(firstIteration ||
			       (activeScan.timestampNS > lastTimestampNS),
		       "timestampNS <= lastTimestampNS");

	const unsigned long long scanDurationNS =
		(!firstIteration ? activeScan.timestampNS - lastTimestampNS :
				   0);
	...
	for (const auto &entry : accumulatedTimePerCore) {
		if (entry.second > 0) {
			const unsigned long long coreDurationTicks =
				entry.second;
			const unsigned long long coreDurationNs =
				coreDurationTicks * hzToNs;
			const double multiplier =
				scanDurationNS / double(entry.second);
			...
			if (coreDurationNs < scanDurationNS) {
				...
				coreRunningTimeMultiplier[entry.first] =
					multiplier;
			}
		} else {
	...
	while (iterator != end) {
		...
		if (processInfo.isSeenSinceLastScan()) {
			...
			if (processInfo.isNew()) {
				handler.onNewProcess(
					processInfo.getStartTimeNS(),
			...
			const unsigned long long processRunningTime =
				processInfo.getTimeRunningDelta();

			// whether or not to fake scheduling events for process
			bool shouldSendSchedEvent = (!firstIteration) &&
						    (!processInfo.isNew()) &&
						    (processRunningTime > 0);

			// calculate fake timestamp for process
			unsigned long long &relativeTimestampEntryRef =
				relativeTimestampMap[processInfo.getProcessor()];
			const unsigned long long fakeTimestampNS =
				relativeTimestampEntryRef + lastTimestampNS;
			const unsigned long long totalGapTimeNs =
				coreTotalTimeMultiplier
					[processInfo.getProcessor()] *
				processRunningTime;
			const unsigned long long fakeRunningTimeNS =
				coreRunningTimeMultiplier
					[processInfo.getProcessor()] *
				processRunningTime;
			// update fake timestamp tracker for core by some relative fraction of overall ticks
			if (shouldSendSchedEvent) {
				relativeTimestampEntryRef += std::max(
					fakeRunningTimeNS, totalGapTimeNs);
			...
			processInfo.sendStats((shouldSendSchedEvent ?
						       fakeTimestampNS :
						       activeScan.timestampNS),
					      handler, shouldSendSchedEvent);
			...
			if (!processInfo.isEmpty()) {
				...
				sendProcessExit(activeScan.timestampNS,
						processInfo);
			}
...
ProcessStateTracker::ProcessInfo &
ProcessStateTracker::getProcessInfoFor(unsigned long long timestampNS, int pgid,
				       int pid, int tid)
{
	const int pidToUse =
		(pgid != 0 ?
			 pid :
			 0); // kernel threads have pgid 0, and pid == tid, change their pid to 0
...
void ProcessStateTracker::sendProcessExit(unsigned long long timestampNS,
					  ProcessInfo &processInfo)
{
	handler.onExitProcess(timestampNS, processInfo.getProcessor(),
			      processInfo.getTid());
}
} // namespace non_root
