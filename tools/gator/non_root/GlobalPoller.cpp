...
GlobalPoller::GlobalPoller(GlobalStatsTracker &globalStateTracker_,
...
void GlobalPoller::poll()
{
	// do /proc/loadavg
	{
		...
		if (lnx::ProcLoadAvgFileRecord::parseLoadAvgFile(
			    loadAvgRecord,
			    lib::readFileContents(PROC_LOADAVG).c_str())) {
			globalStateTracker.updateFromProcLoadAvgFileRecord(
				loadAvgRecord);
		}
	}
	...
	// do /proc/stat
	{
		lnx::ProcStatFileRecord statRecord(
			lib::readFileContents(PROC_STAT).c_str());
