...
namespace non_root
{
NonRootSource::NonRootSource(NonRootDriver &driver_, Child &child_,
			     sem_t &senderSem_, sem_t &startProfile_,
			     const ICpuInfo &cpuInfo)
	: Source(child_),
	  mSwitchBuffers(FrameType::SCHED_TRACE, 1 * 1024 * 1024, senderSem_),
	  mGlobalCounterBuffer(0, FrameType::BLOCK_COUNTER, 1 * 1024 * 1024,
			       &senderSem_),
	  mProcessCounterBuffer(0, FrameType::BLOCK_COUNTER, 1 * 1024 * 1024,
				&senderSem_),
	  mMiscBuffer(0, FrameType::UNKNOWN, 1 * 1024 * 1024, &senderSem_),
	  interrupted(false), timestampSource(CLOCK_MONOTONIC_RAW),
	  driver(driver_), startProfile(startProfile_), done(false),
	  cpuInfo(cpuInfo)
...
{
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-nrsrc"), 0,
	      0, 0);

	std::map<NonRootCounter, int> enabledCounters =
		driver.getEnabledCounters();
	...
	// global stuff
	GlobalStateChangeHandler globalChangeHandler(mGlobalCounterBuffer,
						     enabledCounters);
	...
	ProcessStateTracker processStateTracker(
		processChangeHandler, getBootTimeTicksBase(), clktck, pageSize);
	...
	const useconds_t sleepIntervalUs =
		(gSessionData.mSampleRate < 1000 ?
			 10000 :
			 1000); // select 1ms or 10ms depending on normal or low rate

	while (gSessionData.mSessionIsActive) {
		// check buffer not full
		if (gSessionData.mOneShot && gSessionData.mSessionIsActive &&
		    ((mGlobalCounterBuffer.bytesAvailable() <= 0) ||
		...
		const unsigned long long timestampNowUs =
			(timestampSource.getTimestampNS() + 500) /
			1000; // round to nearest uS
		const useconds_t sleepUs =
			sleepIntervalUs - (timestampNowUs % sleepIntervalUs);
...
{
	return done && mGlobalCounterBuffer.isDone() &&
	       mProcessCounterBuffer.isDone() && mMiscBuffer.isDone() &&
...
bool NonRootSource::summary()
{
	...
	miscBuffer.summaryFrameSummaryMessage(currTime, timestamp,
					      monotonicStarted,
	...
	for (size_t cpu = 0; cpu < cpuInfo.getNumberOfCores(); ++cpu) {
		...
		const GatorCpu *const gatorCpu =
			driver.getPmuXml().findCpuById(cpuId);
		if (gatorCpu != nullptr) {
			miscBuffer.summaryFrameCoreNameMessage(
				currTime, cpu, cpuId, gatorCpu->getCoreName());
		} else {
			snprintf(buf, sizeof(buf), "Unknown (0x%.3x)", cpuId);
			miscBuffer.summaryFrameCoreNameMessage(currTime, cpu,
							       cpuId, buf);
		}
	}
