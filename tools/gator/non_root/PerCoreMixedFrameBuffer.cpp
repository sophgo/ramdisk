...
namespace non_root
{
PerCoreMixedFrameBuffer::PerCoreMixedFrameBuffer(FrameType frameType_,
						 int bufferSize_,
						 sem_t &readerSem_)
	: buffers(), wrappers(), readerSem(readerSem_), frameType(frameType_),
...
MixedFrameBuffer &PerCoreMixedFrameBuffer::operator[](core_type core)
{
	...
	if (wrapperPtrRef == nullptr) {
		...
		if (bufferPtrRef == nullptr) {
			bufferPtrRef.reset(new Buffer(core, frameType,
						      bufferSize, &readerSem));
		}
