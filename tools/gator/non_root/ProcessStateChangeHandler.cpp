...
namespace non_root
{
ProcessStateChangeHandler::ProcessStateChangeHandler(
	Buffer &counterBuffer_, Buffer &miscBuffer_,
	PerCoreMixedFrameBuffer &switchBuffers_,
	const std::map<NonRootCounter, int> &enabledCounters_)
	: miscBuffer(miscBuffer_), cookies(), counterBuffer(counterBuffer_),
	  switchBuffers(switchBuffers_), enabledCounters(enabledCounters_),
	  cookieCounter(3)
{
}

void ProcessStateChangeHandler::onNewProcess(unsigned long long timestampNS,
					     unsigned long core, int ppid,
					     int pid, int tid,
					     ...
					     const std::string &exe)
{
	...
	const cookie_type cookie =
		getCookie(timestampNS, core, pid, tid, exe, comm);
...
void ProcessStateChangeHandler::onExeChange(unsigned long long timestampNS,
					    unsigned long core, int pid,
					    int tid, const std::string &exe)
{
	const cookie_type cookie =
		getCookie(timestampNS, core, pid, tid, exe, exe);

	miscBuffer.activityFrameLinkMessage(timestampNS, cookie, pid, tid);
}

void ProcessStateChangeHandler::onExitProcess(unsigned long long timestampNS,
					      unsigned long core, int tid)
{
						...
						unsigned long long value)
{
	...
	if (it != enabledCounters.end()) {
		counterBuffer.threadCounterMessage(timestampNS, core, tid,
						   it->second, value);
	}
}
...
ProcessStateChangeHandler::cookie_type ProcessStateChangeHandler::getCookie(
	unsigned long long timestampNS, unsigned long core, int pid, int tid,
	const std::string &exe, const std::string &comm)
{
	if (pid == 0) {
		...
		const std::string &nameToUse =
			(!exe.empty() ?
				 exe :
				 comm); // assumes comm is the name of the exe, but the exe was deleted
		...
		if (it != cookies.end()) {
			...
			miscBuffer.nameFrameCookieNameMessage(
				timestampNS, core, newCookie, nameToUse);
			return newCookie;
		}
	}
}

void ProcessStateChangeHandler::idle(unsigned long long timestampNS,
				     unsigned long core)
{
