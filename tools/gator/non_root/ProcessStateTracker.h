...
class ProcessStateTracker {
	...
	class ActiveScan {
		...
		/**
             ...
             */
		void addProcess(int pid, int tid,
		...
		ActiveScan(ProcessStateTracker &parent,
			   unsigned long long timestampNS);
	...
	beginScan(unsigned long long timestampNS);
	...
	class ProcessInfo {
		...
		ProcessInfo();
		ProcessInfo(int pid, int tid, unsigned long pageSize,
			    unsigned long long timestampNS);
		...
		bool hasExitedAndRestartedSince(
		...
		unsigned long long
		update(unsigned long long bootTimeBaseNS, unsigned long clktck,
		       const lnx::ProcPidStatFileRecord &statRecord,
		       const lib::Optional<lnx::ProcPidStatmFileRecord>
			       &statmRecord,
		       const lib::Optional<lib::FsEntry> &exe);
	...
	/**
         ...
         */
	unsigned long long
	add(unsigned long long timestampNS, int pid, int tid,
	    const lnx::ProcPidStatFileRecord &statRecord,
	...
	ProcessInfo &getProcessInfoFor(unsigned long long timestampNS, int pgid,
				       int pid, int tid);

	/** Send process exited event for object */
	void sendProcessExit(unsigned long long timestampNS,
			     ProcessInfo &processInfo);
