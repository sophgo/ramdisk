...
namespace
{
	class NonRootDriverCounter : public DriverCounter {
	    public:
		NonRootDriverCounter(DriverCounter *next, NonRootCounter type,
				     const char *name, const char *label,
				     const char *title, const char *description,
				     const char *display,
				     const char *counterClass, const char *unit,
				     const char *seriesComposition,
				     double multiplier, bool percpu, bool proc)
			: DriverCounter(next, name), mType(type), mLabel(label),
			  mTitle(title), mDescription(description),
			  mDisplay(display), mCounterClass(counterClass),
			  mUnit(unit), mSeriesComposition(seriesComposition),
			  mMultiplier(multiplier), mPerCPU(percpu), mProc(proc)
		{
		}

		NonRootDriverCounter(DriverCounter *next, bool system,
				     const std::string &name)
			: DriverCounter(next, name.c_str()),
			  mType(system ? NonRootCounter::ACTIVITY_SYSTEM :
					 NonRootCounter::ACTIVITY_USER),
			  mLabel(nullptr), mTitle(nullptr),
			  mDescription(nullptr), mDisplay(nullptr),
			  mCounterClass(nullptr), mUnit(nullptr),
			  mSeriesComposition(nullptr), mMultiplier(0),
			  mPerCPU(false), mProc(false)
		{
		}
...
NonRootDriver::NonRootDriver(PmuXML &&pmuXml,
			     lib::Span<const GatorCpu> clusters)
	: SimpleDriver("NonRootDriver"), pmuXml(std::move(pmuXml)),
	  clusters(clusters)
{
}
...
void NonRootDriver::readEvents(mxml_node_t *)
{
	...
	const bool canAccessProcSelfStat =
		(access("/proc/self/stat", R_OK) == 0);
	const bool canAccessProcSelfStatm =
		(access("/proc/self/statm", R_OK) == 0);
	...
	if (canAccessProcLoadAvg) {
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_ABS_LOADAVG_1_MINUTE,
			"nonroot_global_abs_loadavg_1_minute", //
			...
			"average", "absolute", nullptr, "overlay",
			1.0 / GlobalStatsTracker::LOADAVG_MULTIPLIER, false,
			false));
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_ABS_LOADAVG_5_MINUTES,
			"nonroot_global_abs_loadavg_5_minutes", //
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_ABS_LOADAVG_15_MINUTES,
			"nonroot_global_abs_loadavg_15_minutes", //
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_ABS_NUM_PROCESSES_EXISTING,
			"nonroot_global_abs_num_processes_existing", //
			"Total Processes", //
			"System", //
			...
			"average", "absolute", nullptr, "overlay", 1.0, false,
			false));
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_ABS_NUM_PROCESSES_RUNNING,
			"nonroot_global_abs_num_processes_running", //
			"Running Processes", //
			"System", //
	...
	if (canAccessProcStat) {
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_NUM_CONTEXT_SWITCHES,
			"nonroot_global_delta_num_context_switches", //
			"Context Switches", //
			"Scheduler", //
			"The number of context switches that the system underwent. "
			"See the description of /proc/stat [ctxt] in 'man proc.5' for more details.", //
			"accumulate", "delta", nullptr, "overlay", 1.0, false,
			false));
		setCounters(new NonRootDriverCounter(
			getCounters(), NonRootCounter::GLOBAL_DELTA_NUM_FORKS,
			"nonroot_global_delta_num_forks", //
			"Forks", //
			"System", //
			...
			false));
		setCounters(new NonRootDriverCounter(
			getCounters(), NonRootCounter::GLOBAL_DELTA_NUM_IRQ,
			"nonroot_global_delta_num_irq", //
			"IRQ", //
			"Interrupts", //
			...
			false));
		setCounters(new NonRootDriverCounter(
			getCounters(), NonRootCounter::GLOBAL_DELTA_NUM_SOFTIRQ,
			"nonroot_global_delta_num_softirq", //
			"Soft IRQ", //
			"Interrupts", //
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_GUEST_NICE,
			"nonroot_global_delta_time_cpu_guest_nice", //
			"Guest Nice", //
			"CPU Times", //
			...
			"accumulate", "delta", "s", "stacked", ticks_mult, true,
			false));
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_GUEST,
			"nonroot_global_delta_time_cpu_guest", //
			"Guest", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_IDLE,
			"nonroot_global_delta_time_cpu_idle", //
			"Idle", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_IOWAIT,
			"nonroot_global_delta_time_cpu_iowait", //
			"IO wait", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_IRQ,
			"nonroot_global_delta_time_cpu_irq", //
			"IRQ", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_NICE,
			"nonroot_global_delta_time_cpu_nice", //
			"Nice", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_SOFTIRQ,
			"nonroot_global_delta_time_cpu_softirq", //
			"Soft IRQ", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_STEAL,
			"nonroot_global_delta_time_cpu_steal", //
			"Steal", //
			"CPU Times", //
			"The amount of time, measured in units of USER_HZ, "
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_SYSTEM,
			"nonroot_global_delta_time_cpu_system", //
			"System", //
			"CPU Times", //
		...
		setCounters(new NonRootDriverCounter(
			getCounters(),
			NonRootCounter::GLOBAL_DELTA_TIME_CPU_USER,
			"nonroot_global_delta_time_cpu_user", //
			"User", //
			"CPU Times", //
	...
	if (gSessionData.mAndroidApiLevel < ANDROID_N_API_LEVEL) {
		...
		if (canAccessProcSelfStatm) {
			setCounters(new NonRootDriverCounter(
				...
				"maximum", "absolute", "B", "overlay", 1.0,
				false, true));
			setCounters(new NonRootDriverCounter(
				getCounters(),
		...
		if (canAccessProcSelfStat) {
			setCounters(new NonRootDriverCounter(
				...
				"maximum", "absolute", nullptr, "overlay", 1.0,
				false, true));
			setCounters(new NonRootDriverCounter(
				getCounters(),
			...
			setCounters(new NonRootDriverCounter(
				...
				"accumulate", "delta", nullptr, "overlay", 1.0,
				false, true));
			setCounters(new NonRootDriverCounter(
				getCounters(),
			...
			setCounters(new NonRootDriverCounter(
				...
				"accumulate", "delta", "s", "stacked",
				ticks_mult, false, true));
			setCounters(new NonRootDriverCounter(
				getCounters(),
			...
			for (const GatorCpu &cluster : clusters) {
				const std::string sysName =
					(lib::Format()
					 << cluster.getId() << "_system");
				const std::string userName =
					(lib::Format()
					 << cluster.getId() << "_user");

				setCounters(new NonRootDriverCounter(
					getCounters(), true, sysName));
				setCounters(new NonRootDriverCounter(
					getCounters(), false, userName));
			}
		} else {
			logg.logSetup(
		...
		logg.logSetup(
			"/proc limited on Android 7+\nDisabled per-process proc counters on Android 7+ due to access restrictions on /proc (Android API level detected as %d)",
...
void NonRootDriver::writeEvents(mxml_node_t *root) const
{
	...
	for (NonRootDriverCounter *counter =
		     static_cast<NonRootDriverCounter *>(getCounters());
	     counter != nullptr; counter = static_cast<NonRootDriverCounter *>(
					 counter->getNext())) {
		if ((counter->getType() != NonRootCounter::ACTIVITY_SYSTEM) &&
			...
			mxmlElementSetAttr(node, "display",
					   counter->getDisplay());
			mxmlElementSetAttr(node, "class",
					   counter->getCounterClass());
			if (counter->getUnit() != nullptr) {
				mxmlElementSetAttr(node, "units",
						   counter->getUnit());
			}
			if (counter->getMultiplier() != 1.0) {
				mxmlElementSetAttrf(node, "multiplier", "%f",
						    counter->getMultiplier());
			}
			if (strcmp(counter->getDisplay(), "average") == 0 ||
			    strcmp(counter->getDisplay(), "maximum") == 0) {
				mxmlElementSetAttr(node, "average_selection",
						   "yes");
			}
			if (counter->isPerCPU()) {
				mxmlElementSetAttr(node, "per_cpu", "yes");
			}
			mxmlElementSetAttr(node, "proc",
					   counter->isProc() ? "yes" : "no");
			mxmlElementSetAttr(node, "series_composition",
					   counter->getSeriesComposition());
			mxmlElementSetAttr(node, "rendering_type", "line");
			mxmlElementSetAttr(node, "description",
