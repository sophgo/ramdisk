...
class ProcessStateChangeHandler {
	...
	ProcessStateChangeHandler(
		Buffer &counterBuffer, Buffer &miscBuffer,
		PerCoreMixedFrameBuffer &switchBuffers,
		const std::map<NonRootCounter, int> &enabledCounters);

	void onNewProcess(unsigned long long timestampNS, unsigned long core,
			  int ppid, int pid, int tid, const std::string &comm,
			  const std::string &exe);
	void onCommChange(unsigned long long timestampNS, unsigned long core,
			  int tid, const std::string &comm);
	void onExeChange(unsigned long long timestampNS, unsigned long core,
			 int pid, int tid, const std::string &exe);
	void onExitProcess(unsigned long long timestampNS, unsigned long core,
			   int tid);
	void absoluteCounter(unsigned long long timestampNS, unsigned long core,
			     int tid, AbsoluteProcessCounter id,
			     unsigned long long value);
	void deltaCounter(unsigned long long timestampNS, unsigned long core,
			  int tid, DeltaProcessCounter id,
			  unsigned long long delta);
	void threadActivity(unsigned long long timestampNS, int tid,
