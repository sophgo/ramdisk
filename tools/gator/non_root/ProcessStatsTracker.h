...
class ProcessStatsTracker {
	...
	{
		return std::max<long long>(stat_stime.delta(), 0) +
		       std::max<long long>(stat_utime.delta(), 0);
	}

	void sendStats(unsigned long long timestampNS,
		       ProcessStateChangeHandler &handler,
		       bool sendFakeSchedulingEvents);
	void updateFromProcPidStatFileRecord(
		const lnx::ProcPidStatFileRecord &record);
	void updateFromProcPidStatmFileRecord(
