...
class GlobalStatsTracker {
	...
	class PerCoreStatsTracker {
		...
		void sendStats(unsigned long long timestampNS,
			       ...
			       unsigned long cpuID);
		void updateFromProcStatFileRecordCpuTime(
		...
		void writeCounter(unsigned long long timestampNS,
				  GlobalStateChangeHandler &handler,
				  unsigned long cpuID, DeltaGlobalCounter id,
	...
	void sendStats(unsigned long long timestampNS);
	void updateFromProcLoadAvgFileRecord(
		const lnx::ProcLoadAvgFileRecord &record);
	void
	updateFromProcStatFileRecord(const lnx::ProcStatFileRecord &record);
	...
	template <typename T>
	void writeCounter(unsigned long long timestampNS, DeltaGlobalCounter id,
			  DeltaCounter<T> &counter);
};
} // namespace non_root
