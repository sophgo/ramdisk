...
class Ids {
	...
	shared_memory::unique_ptr<int[]> ids =
		shared_memory::make_unique<int[]>(maxCoreNumber * 2);
};

class CpuInfo : public ICpuInfo {
    public:
	CpuInfo(Ids &&ids, std::vector<GatorCpu> &&clusters,
		const char *modelName)
		: ids(std::move(ids)), clusters(std::move(clusters)),
		  modelName(modelName)
	{
...
class GatorKoPrimarySource : public PrimarySourceProvider {
	...
	/**
         ...
         * @return
         */
	static std::unique_ptr<PrimarySourceProvider>
	tryCreate(const char *module, const PmuXML &pmuXml, Ids &ids,
		  const char *modelName)
	{
		...
		if (setupFilesystem(module)) {
			...
			CpuInfo cpuInfo{ std::move(ids), std::move(clusters),
					 modelName };

			result.reset(
				new GatorKoPrimarySource(std::move(cpuInfo)));
		}
	...
	virtual std::int64_t getMonotonicStarted() const override
	{
		...
		if (lib::readInt64FromFile("/dev/gator/started",
					   monotonicStarted) == -1) {
	...
	virtual std::unique_ptr<Source>
	createPrimarySource(Child &child, sem_t &senderSem, sem_t &startProfile,
			    const std::set<int> &, FtraceDriver &ftraceDriver,
			    bool) override
	{
		return std::unique_ptr<Source>(new DriverSource(
			child, senderSem, startProfile, ftraceDriver));
	}
	...
	GatorKoPrimarySource(CpuInfo &&cpuInfo)
		: PrimarySourceProvider(createPolledDrivers()), driver(),
...
class PerfPrimarySource : public PrimarySourceProvider {
	...
	/**
         ...
         */
	static std::unique_ptr<PrimarySourceProvider>
	tryCreate(bool systemWide, PmuXML &pmuXml, const char *maliFamilyName,
		  Ids &ids, const char *modelName)
	{
		std::unique_ptr<PerfDriverConfiguration> configuration =
			PerfDriverConfiguration::detect(
				systemWide, ids.getCpuIds(), pmuXml);
		if (configuration != nullptr) {
			...
			return std::unique_ptr<PrimarySourceProvider>{
				new PerfPrimarySource(std::move(*configuration),
						      std::move(pmuXml),
						      maliFamilyName,
						      std::move(cpuInfo))
			};
	...
	{
		return std::unique_ptr<Source>(new PerfSource(
			driver, child, senderSem, startProfile, appTids,
			ftraceDriver, enableOnCommandExec, cpuInfo));
	}
	...
	static std::vector<PolledDriver *> createPolledDrivers()
	{
		return std::vector<PolledDriver *>{
			{ new HwmonDriver(), new FSDriver(), new DiskIODriver(),
	...
	PerfPrimarySource(PerfDriverConfiguration &&configuration,
		...
		: PrimarySourceProvider(createPolledDrivers()),
		  cpuInfo(std::move(cpuInfo)),
		  driver(std::move(configuration), std::move(pmuXml),
			 maliFamilyName, this->cpuInfo)
	{
	}
...
class NonRootPrimarySource : public PrimarySourceProvider {
    public:
	static std::unique_ptr<PrimarySourceProvider>
	tryCreate(PmuXML &&pmuXml, Ids &ids, const char *modelName)
	{
		...
		for (int cpuId : ids.getClusterIds()) {
			if (cpuId >= 0 && added.count(cpuId) != 0) {
				const GatorCpu *gatorCpu =
					pmuXml.findCpuById(cpuId);
				if (gatorCpu != nullptr) {
					// create the cluster
					added.insert(cpuId);
		...
		if (clusters.empty()) {
			...
			clusters.emplace_back("Other", "Other", "Other",
					      nullptr, nullptr,
					      std::set<int>{ 0xfffff }, 6,
					      true);
			...
			clusters.emplace_back("Other", "Other", "Other",
					      ...
					      false);
#else
			clusters.emplace_back("Other", "Perf_Hardware",
					      "Perf_Hardware", nullptr, nullptr,
					      std::set<int>{ 0xfffff }, 6,
					      false);
#endif
		}
	...
	virtual std::unique_ptr<Source>
	createPrimarySource(Child &child, sem_t &senderSem, sem_t &startProfile,
			    const std::set<int> &, FtraceDriver &,
			    bool) override
	{
		return std::unique_ptr<Source>(new non_root::NonRootSource(
			driver, child, senderSem, startProfile, cpuInfo));
	}
...
PrimarySourceProvider::PrimarySourceProvider(
	const std::vector<PolledDriver *> &polledDrivers_)
...
const std::vector<PolledDriver *> &
PrimarySourceProvider::getAdditionalPolledDrivers() const
{
	return polledDrivers;
}

std::unique_ptr<PrimarySourceProvider>
PrimarySourceProvider::detect(const char *module, bool systemWide,
			      PmuXML &&pmuXml, const char *maliFamilyName)
{
	Ids ids{ cpu_utils::getMaxCoreNum() };
	const std::string modelName =
		lib::FsEntry::create("/proc/device-tree/model")
			.readFileContents();
	const std::string hardwareName =
		cpu_utils::readCpuInfo(false, ids.getCpuIds());
	const char *modelNameToUse = !modelName.empty() ?
					     modelName.c_str() :
					     !hardwareName.empty() ?
					     hardwareName.c_str() :
					     CORE_NAME_UNKNOWN;
	...
	if (isRoot && systemWide) {
		...
		result = GatorKoPrimarySource::tryCreate(module, pmuXml, ids,
							 modelNameToUse);
		if (result != nullptr) {
			...
			logg.logError(
				"Using deprecated gator.ko for primary data source");
			return result;
		} else
	...
	result = PerfPrimarySource::tryCreate(
		systemWide, pmuXml, maliFamilyName, ids, modelNameToUse);
	if (result != nullptr) {
		logg.logMessage("...Success");
		logg.logSetup(
		...
		logg.logError(
			"...Perf API is not available to non-root users in system-wide mode.\n"
			"To use it\n"
			" * try --system-wide=no,\n"
			" * run as root,\n"
	...
	if (isRoot)
		...
		logg.logMessage(
			"Trying /proc counters as non-root; limited system profiling information available...");

	result = NonRootPrimarySource::tryCreate(std::move(pmuXml), ids,
						 modelNameToUse);
	if (result != nullptr) {
		logg.logMessage("...Success");
		logg.logSetup(
