			    ...
			    const ICpuInfo &cpuInfo)
{
	...
	if (count == 0) {
		logg.logError(
			"No counters found, this could be because /dev/gator/events can not be read or because perf is not working correctly");
		handleException();
	}
	...
	for (size_t cluster = 0; cluster < cpuInfo.getClusters().size();
	     ++cluster) {
		mxml_node_t *node = mxmlNewElement(counters, "cluster");
		mxmlElementSetAttrf(node, "id", "%zi", cluster);
		mxmlElementSetAttr(node, "name",
				   cpuInfo.getClusters()[cluster].getId());
	}
	for (size_t cpu = 0; cpu < cpuInfo.getClusterIds().size(); ++cpu) {
		if (cpuInfo.getClusterIds()[cpu] >= 0) {
			...
			mxmlElementSetAttrf(node, "cluster", "%i",
					    cpuInfo.getClusterIds()[cpu]);
		}
	}
	return xml;
}
...
getXML(bool supportsMultiEbs, lib::Span<const Driver *const> drivers,
       const ICpuInfo &cpuInfo)
{
	char *xml_string;
...
void write(const char *path, bool supportsMultiEbs,
	   lib::Span<const Driver *const> drivers, const ICpuInfo &cpuInfo)
{
	char file[PATH_MAX];
	...
	if (writeToDisk(file,
			getXML(supportsMultiEbs, drivers, cpuInfo).get()) < 0) {
