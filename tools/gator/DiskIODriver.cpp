...
class DiskIOCounter : public DriverCounter {
    public:
	DiskIOCounter(DriverCounter *next, const char *const name,
...
DiskIOCounter::DiskIOCounter(DriverCounter *next, const char *const name,
			     uint64_t *const value)
...
DiskIODriver::DiskIODriver()
	: PolledDriver("DiskIO"), mBuf(), mReadBytes(0), mWriteBytes(0)
{
}
...
void DiskIODriver::readEvents(mxml_node_t *const)
{
	if (access("/proc/diskstats", R_OK) == 0) {
		setCounters(new DiskIOCounter(
			getCounters(), "Linux_block_rq_rd", &mReadBytes));
		setCounters(new DiskIOCounter(
			getCounters(), "Linux_block_rq_wr", &mWriteBytes));
	} else {
		logg.logSetup(
...
void DiskIODriver::doRead()
{
	...
	while (*line != '\0') {
		...
		const int count =
			sscanf(line,
			       "%*d %*d %n%*s%n %*u %*u %" SCNu64
			       " %*u %*u %*u %" SCNu64,
			       &nameStart, &nameEnd, &readBytes, &writeBytes);
		if (count != 2) {
		...
		if ((lastName == NULL) ||
		    (strncmp(lastName, line + nameStart, lastNameLen) != 0)) {
