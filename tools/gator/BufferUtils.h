...
static constexpr const size_t MAX_FRAME_HEADER_SIZE =
	(2 * MAXSIZE_PACK32) + sizeof(int32_t);

int packInt(char *const buf, int &writePos, int32_t x,
	    int writePosWrapMask = -1);
int packInt64(char *const buf, int &writePos, int64_t x,
	      int writePosWrapMask = -1);
