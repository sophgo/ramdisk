...
PerCoreIdentificationThread::PerCoreIdentificationThread(
	bool ignoreOffline, unsigned cpu, ConsumerFunction consumerFunction)
	: thread(), consumerFunction(consumerFunction), terminatedFlag(false),
	  cpu(cpu), ignoreOffline(ignoreOffline)
{
...
void PerCoreIdentificationThread::launch(
	PerCoreIdentificationThread *_this) noexcept
{
	_this->run();
}
...
bool PerCoreIdentificationThread::configureAffinity()
{
	...
	for (unsigned count = 0;
	     count < AFFINE_LOOP_COUNT && !affinitySucceeded; ++count) {
	...
	if (!affinitySucceeded) {
		logg.logMessage(
			"Error calling sched_setaffinity on %u: %d (%s)", cpu,
	...
	if (sched_setscheduler(tid, SCHED_FIFO | SCHED_RESET_ON_FORK, &param) !=
	    0) {
		logg.logMessage(
			"Unable to schedule sync thread as FIFO, trying OTHER: %d (%s)",
			errno, strerror(errno));
		param.sched_priority = sched_get_priority_max(SCHED_OTHER);
		if (sched_setscheduler(tid, SCHED_OTHER | SCHED_RESET_ON_FORK,
				       &param) != 0) {
			logg.logMessage(
...
void PerCoreIdentificationThread::run() noexcept
{
	...
	// attempt to read topology and identification information
	{
		...
		snprintf(
			...
			cpu);
		physical_package_id_valid =
			(lib::readIntFromFile(buffer, physical_package_id) ==
			 0);
		...
		snprintf(
			...
			cpu);
		midr_el1_valid =
			(lib::readInt64FromFile(buffer, midr_el1) == 0);
	}
	...
	consumerFunction(cpu, (core_id_valid ? core_id : INVALID_CORE_ID),
			 (physical_package_id_valid ? physical_package_id :
						      INVALID_PACKAGE_ID),
			 core_siblings,
