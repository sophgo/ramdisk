...
CoreOnliner::CoreOnliner(unsigned core)
	: core(core), known(false), changed(false), online(false)
{
	char buffer[128];
	snprintf(buffer, sizeof(buffer), "/sys/devices/system/cpu/cpu%u/online",
		 core);
	...
	changed = (known && !online ?
			   (lib::writeCStringToFile(buffer, "1") == 0) //
			   :
			   false);

	logg.logMessage("CoreOnliner(core=%u, known=%u, online=%u, changed=%u)",
			core, known, online, changed);
}
...
CoreOnliner::~CoreOnliner()
{
	if (changed) {
		...
		snprintf(buffer, sizeof(buffer),
			 "/sys/devices/system/cpu/cpu%u/online", core);
		if (lib::writeCStringToFile(buffer, "0") != 0) {
			logg.logError("Failed to restore online state for %u",
				      core);
		}
	}
}
...
CoreOnliner::CoreOnliner(CoreOnliner &&that)
	: core(that.core), known(false), changed(false), online(false)
{
