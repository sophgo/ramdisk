...
PerfSource::PerfSource(PerfDriver &driver, Child &child, sem_t &senderSem,
		       sem_t &startProfile, const std::set<int> &appTids,
		       FtraceDriver &ftraceDriver, bool enableOnCommandExec,
		       ICpuInfo &cpuInfo)
	: Source(child), mSummary(1024 * 1024, &senderSem),
	  mCountersBuf(createPerfBufferConfig()),
	  mCountersGroup(
		  driver.getConfig(), mCountersBuf.getDataBufferLength(),
		  ...
		  gSessionData.mBacktraceDepth, gSessionData.mSampleRate,
		  !gSessionData.mIsEBS, cpuInfo.getClusters(),
		  cpuInfo.getClusterIds(), getTracepointId(SCHED_SWITCH)),
	  mMonitor(), mUEvent(), mAppTids(appTids), mDriver(driver),
	  mAttrsBuffer(NULL), mSenderSem(senderSem),
	  mStartProfile(startProfile), mInterruptFd(-1), mIsDone(false),
	  mFtraceDriver(ftraceDriver), mCpuInfo(cpuInfo), mSyncThreads(),
	  enableOnCommandExec(false)
{
	...
	if ((!mConfig.is_system_wide) && (!mConfig.has_attr_clockid_support)) {
		logg.logMessage(
			"Tracing gatord as well as target application as no clock_id support");
	...
	this->enableOnCommandExec =
		(enableOnCommandExec && !mConfig.is_system_wide &&
		 mConfig.has_attr_clockid_support &&
...
bool PerfSource::prepare()
{
	...
	mAttrsBuffer = new PerfAttrsBuffer(
		gSessionData.mTotalBufferSize * 1024 * 1024, &mSenderSem);
	...
	if (mConfig.is_system_wide &&
	    (!mUEvent.init() || !mMonitor.add(mUEvent.getFd()))) {
		logg.logMessage("uevent setup failed");
		return false;
	}

	if (mConfig.can_access_tracepoints &&
	    !mDriver.sendTracepointFormats(currTime, *mAttrsBuffer)) {
	...
	const OnlineEnabledState onlineEnabledState =
		(enableOnCommandExec ? OnlineEnabledState::ENABLE_ON_EXEC :
				       OnlineEnabledState::NOT_ENABLED);
	int numOnlined = 0;

	for (size_t cpu = 0; cpu < mCpuInfo.getNumberOfCores(); ++cpu) {
		using Result = OnlineResult;
		const std::pair<OnlineResult, std::string> result =
			mCountersGroup.onlineCPU(
				currTime, cpu, mAppTids, onlineEnabledState,
				*mAttrsBuffer, //
				[this](int fd) -> bool {
					return mMonitor.add(fd);
				},
				[this](int fd, int cpu, bool hasAux) -> bool {
					return mCountersBuf.useFd(fd, cpu,
								  hasAux);
				},
				&lnx::getChildTids);
		switch (result.first) {
	...
	if (!mDriver.summary(mSummary, []() -> uint64_t {
		    return (gSessionData.mMonotonicStarted = getTime());
	    })) {
...
{
	const ProcThreadArgs *const args =
		reinterpret_cast<const ProcThreadArgs *>(arg);

	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-proc"), 0,
	      0, 0);
	...
	if (!readKallsyms(args->mCurrTime, *args->mAttrsBuffer,
			  args->mIsDone)) {
...
void PerfSource::run()
{
...

	{
		...
		const uint64_t currTime =
			getTime() - gSessionData.mMonotonicStarted;
		...
		if (!readProcSysDependencies(currTime, *mAttrsBuffer, &printb,
					     &b1, mFtraceDriver)) {
			if (mDriver.getConfig().is_system_wide) {
				...
				logg.logMessage(
					"readProcSysDependencies failed");
			}
		}
		mAttrsBuffer->commit(currTime);
		...
		if (pthread_create(&procThread, NULL, procFunc,
				   &procThreadArgs)) {
	...
	if (!mUEvent.enabled()) {
		onlineMonitorThread.reset(new PerfCpuOnlineMonitor(
			[&](unsigned cpu, bool online) -> void {
				logg.logMessage(
					"CPU online state changed: %u -> %s",
					cpu, (online ? "online" : "offline"));
				const uint64_t currTime =
					getTime() -
					gSessionData.mMonotonicStarted;
				if (online) {
	...
	// start sync threads
	mSyncThreads = PerfSyncThreadBuffer::create(
		gSessionData.mMonotonicStarted,
		this->mDriver.getConfig().has_attr_clockid_support,
		this->mCountersGroup.hasSPE(), mSenderSem);
	...
	const uint64_t rate =
		gSessionData.mLiveRate > 0 && gSessionData.mSampleRate > 0 ?
			gSessionData.mLiveRate :
			NO_RATE;
	uint64_t nextTime = 0;
	...
	while (gSessionData.mSessionIsActive) {
		...
		std::vector<struct epoll_event> events{
			mCpuInfo.getNumberOfCores() + 2
		};
		int ready =
			mMonitor.wait(events.data(), events.size(), timeout);
		if (ready < 0) {
		...
		for (int i = 0; i < ready; ++i) {
			if (events[i].data.fd == mUEvent.getFd()) {
				if (!handleUEvent(currTime)) {
					logg.logError(
						"PerfSource::handleUEvent failed");
					handleException();
				}
				break;
			}
		...
		if (gSessionData.mOneShot && gSessionData.mSessionIsActive &&
		    ((mSummary.bytesAvailable() <= 0) ||
		     (mAttrsBuffer->bytesAvailable() <= 0) ||
		     mCountersBuf.isFull())) {
		...
		if (rate != NO_RATE) {
			...
			timeout = std::max<int>(
				0, (nextTime + NS_PER_MS - 1 - getTime() +
				    gSessionData.mMonotonicStarted) /
					   NS_PER_MS);
		}
	}
...
bool PerfSource::handleUEvent(const uint64_t currTime)
{
	...
	if (strcmp(result.mSubsystem, "cpu") == 0) {
		if (strncmp(result.mDevPath, CPU_DEVPATH,
			    sizeof(CPU_DEVPATH) - 1) != 0) {
			logg.logMessage("Unexpected cpu DEVPATH format");
			return false;
		}
		int cpu;
		if (!stringToInt(&cpu,
				 result.mDevPath + sizeof(CPU_DEVPATH) - 1,
				 10)) {
		...
		if (static_cast<size_t>(cpu) >= mCpuInfo.getNumberOfCores()) {
			logg.logError(
				"Only %zu cores are expected but core %i reports %s",
				mCpuInfo.getNumberOfCores(), cpu,
				result.mAction);
			handleException();
		}
	...
	const std::pair<OnlineResult, std::string> result =
		mCountersGroup.onlineCPU(
			...
			*mAttrsBuffer, //
			...
			[this](int fd, int cpu, bool hasAux) -> bool {
				return mCountersBuf.useFd(fd, cpu, hasAux);
			},
			&lnx::getChildTids);
...
{
	const bool ret = mCountersGroup.offlineCPU(cpu, [this](int cpu) {
		mCountersBuf.discard(cpu);
	});
