...
{
	const char *const command[] = { "getprop", "security.perf_harden",
					nullptr };
	const lib::PopenResult getprop = lib::popen(command);
	if (getprop.pid < 0) {
		logg.logMessage(
			"lib::popen(%s %s) failed: %s. Probably not android",
			command[0], command[1], strerror(-getprop.pid));
		return false;
	}
...
{
	const char *const command[] = { "setprop", "security.perf_harden",
					on ? "1" : "0", nullptr };

	const lib::PopenResult setprop = lib::popen(command);
	if (setprop.pid < 0) {
		logg.logError("lib::popen(%s %s %s) failed: %s", command[0],
			      command[1], command[2], strerror(-setprop.pid));
		return;
	}

	const int status = lib::pclose(setprop);
	if (!WIFEXITED(status)) {
		logg.logError("'%s %s %s' exited abnormally", command[0],
			      command[1], command[2]);
		return;
	}
	...
	if (exitCode != 0) {
		logg.logError("'%s %s %s' failed: %d", command[0], command[1],
			      command[2], exitCode);
	}
}
...
std::unique_ptr<PerfDriverConfiguration>
PerfDriverConfiguration::detect(bool systemWide, lib::Span<const int> cpuIds,
				const PmuXML &pmuXml)
{
	...
	const bool hasArmv7PmuDriver = beginsWith(utsname.machine, "arm") &&
				       !beginsWith(utsname.machine, "arm64") &&
	...
	if (!isRoot && !disablePerfHarden()) {
		logg.logSetup(
			"Failed to disable property security.perf_harden\n" //
			"Try 'adb shell setprop security.perf_harden 0'");
		return nullptr;
	}

	int perf_event_paranoid;
	if (lib::readIntFromFile("/proc/sys/kernel/perf_event_paranoid",
				 perf_event_paranoid) != 0) {
		if (isRoot) {
			logg.logSetup(
				"perf_event_paranoid not accessible\nIs CONFIG_PERF_EVENTS enabled?");
			return nullptr;
		} else {
			logg.logSetup(
				"perf_event_paranoid not accessible\nAssuming high paranoia.");
#if defined(CONFIG_ASSUME_PERF_HIGH_PARANOIA) &&                               \
	CONFIG_ASSUME_PERF_HIGH_PARANOIA
	...
	const bool can_access_tracepoints =
		(lib::access(EVENTS_PATH, R_OK) == 0) &&
		(isRoot || perf_event_paranoid == -1);
	...
	if (systemWide && (!can_access_tracepoints) &&
	    (kernelVersion < KERNEL_VERSION(4, 3, 0))) {
		logg.logSetup(
			EVENTS_PATH
	...
	std::unique_ptr<PerfDriverConfiguration> configuration{
		new PerfDriverConfiguration()
	};

	configuration->config.has_fd_cloexec =
		(kernelVersion >= KERNEL_VERSION(3, 14, 0));
	configuration->config.has_count_sw_dummy =
		(kernelVersion >= KERNEL_VERSION(3, 12, 0));
	configuration->config.has_sample_identifier =
		(kernelVersion >= KERNEL_VERSION(3, 12, 0));
	configuration->config.has_attr_comm_exec =
		(kernelVersion >= KERNEL_VERSION(3, 16, 0));
	configuration->config.has_attr_mmap2 =
		(kernelVersion >= KERNEL_VERSION(3, 16, 0));
	configuration->config.has_attr_clockid_support =
		(kernelVersion >= KERNEL_VERSION(4, 1, 0));
	configuration->config.has_attr_context_switch =
		(kernelVersion >= KERNEL_VERSION(4, 3, 0));
	configuration->config.has_ioctl_read_id =
		(kernelVersion >= KERNEL_VERSION(3, 12, 0));
	configuration->config.has_aux_support =
		(kernelVersion >= KERNEL_VERSION(4, 1, 0));
	...
	if (dir.exists()) {
		...
		while ((dirent = children.next())) {
			...
			if (gatorCpu != NULL) {
				int type;
				const std::string path(lib::Format()
						       << PERF_DEVICES << "/"
						       << name << "/type");
				if (lib::readIntFromFile(path.c_str(), type) ==
				    0) {
					configuration->cpus.push_back(
						PerfCpu{ *gatorCpu, type });
			...
			const UncorePmu *uncorePmu =
				pmuXml.findUncoreByName(name);
			if (uncorePmu != NULL) {
				int type;
				const std::string path(lib::Format()
						       << PERF_DEVICES << "/"
				...
				if (lib::readIntFromFile(path.c_str(), type) ==
				    0) {
					configuration->uncores.push_back(
						PerfUncore{ *uncorePmu, type });
					continue;
				}
			}
			...
			if (beginsWith(name, "arm_spe_")) {
				...
				const std::string typePath(
					lib::Format() << PERF_DEVICES << "/"
						      << name << "/type");
				const std::string maskPath(
					lib::Format() << PERF_DEVICES << "/"
						      << name << "/cpumask");
				if (lib::readIntFromFile(typePath.c_str(),
							 type) == 0) {
					const std::set<int> cpuNumbers =
						lib::readCpuMaskFromFile(
							maskPath.c_str());
					for (int cpuNumber : cpuNumbers) {
						configuration->cpuNumberToSpeType
							[cpuNumber] = type;
					}
					continue;
				}
	...
	for (int cpuId : cpuIds) {
		...
		if (gatorCpu == nullptr) {
			...
			if ((cpuId != UNKNOWN_CPUID) && (cpuId != -1) &&
			    (cpuId != 0)) {
				unrecognisedCpuIds.insert(cpuId);
			}
		} else if ((cpusDetectedViaSysFs.count(gatorCpu) == 0) &&
			   (cpusDetectedViaCpuid.count(gatorCpu) == 0)) {
			logg.logMessage("generic pmu: %s",
					gatorCpu->getCoreName());
			configuration->cpus.push_back(
				PerfCpu{ *gatorCpu, PERF_TYPE_RAW });
	...
	for (unsigned int i = 0; i < configuration->cpus.size(); ++i) {
		...
		if (anyV7 && anyV8) {
			...
			configuration->cpus.erase(
				std::remove_if(
					configuration->cpus.begin(),
					configuration->cpus.end(),
					[](const PerfCpu &cpu) {
						return !(
							cpu.gator_cpu.getIsV8());
					}),
				configuration->cpus.end());
			break;
		}
	...
	const bool hasNoCpus =
		cpusDetectedViaSysFs.empty() && cpusDetectedViaCpuid.empty();
	const bool haveUnknownSpe =
		!configuration->cpuNumberToSpeType.empty() &&
		!haveFoundKnownCpuWithSpe;
	const bool addOtherForUnknownSpe =
		(haveUnknownSpe && configuration->cpus.empty());
	...
	if (haveUnknownSpe && !configuration->cpus.empty()) {
		for (unsigned int i = 0; i < configuration->cpus.size(); ++i) {
			...
			configuration->cpus[i] =
				PerfCpu{ GatorCpu(currentValue.gator_cpu,
						  ARMV82_SPE),
	...
	if ((hasNoCpus || addOtherForUnknownSpe) &&
	    unrecognisedCpuIds.empty()) {
	...
	if (!unrecognisedCpuIds.empty()) {
		...
		const char *const speName =
			(addOtherForUnknownSpe ? ARMV82_SPE : nullptr);

#if defined(__aarch64__)
		configuration->cpus.push_back(
			PerfCpu{ { "Other", "Other", "Other", nullptr, speName,
				   unrecognisedCpuIds, 6, true },
				 PERF_TYPE_RAW });
#elif defined(__arm__)
		configuration->cpus.push_back(
			PerfCpu{ { "Other", "Other", "Other", nullptr, speName,
				   unrecognisedCpuIds, 6, anyV8 },
				 PERF_TYPE_RAW });
#else
		configuration->cpus.push_back(PerfCpu{
			{ "Other", "Perf_Hardware", "Perf_Hardware", nullptr,
			  speName, unrecognisedCpuIds, 6, false },
			PERF_TYPE_HARDWARE });
#endif
	}

	if (cpusDetectedViaSysFs.empty() && !cpusDetectedViaCpuid.empty() &&
	    (dir.exists())) {
		logg.logSetup(
