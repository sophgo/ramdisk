...
static bool readAndSend(const uint64_t currTime,
			...
			const struct perf_event_attr &attr, const int fd,
			const int keyCount, const int *const keys)
{
	for (int retry = 0; retry < 10; ++retry) {
		...
		attrsConsumer.marshalKeysOld(currTime, keyCount, keys, bytes,
					     buf);
		return true;
	}

	/* not able to pin even, log and return true, data is skipped */
	logg.logError("Could not pin event %u:0x%llx, skipping", attr.type,
		      attr.config);
	return true;
}
} // namespace
...
PerfEventGroup::PerfEventGroup(const PerfEventGroupIdentifier &groupIdentifier,
	...
	: groupIdentifier(groupIdentifier), sharedConfig(sharedConfig),
	  events(), cpuToEventIndexToTidToFdMap()
{
}
...
bool PerfEventGroup::addEvent(const bool leader, const uint64_t timestamp,
			      IPerfAttrsConsumer &attrsConsumer, const int key,
			      const IPerfGroups::Attr &attr, bool hasAuxData)
{
	...
	event.attr.sample_type =
		...
		| (sharedConfig.perfConfig.has_sample_identifier ?
		...
		| (sharedConfig.perfConfig.can_access_tracepoints ||
	...
	event.attr.inherit =
		(sharedConfig.perfConfig.is_system_wide ?
			 0 :
			 1); // make sure all new children are counted too
	...
	event.attr.read_format =
		PERF_FORMAT_ID | (event.attr.inherit ? 0 : PERF_FORMAT_GROUP);
	...
	const bool isNotInAReadFormatGroup =
		((event.attr.read_format & PERF_FORMAT_GROUP) == 0);
	const bool everyAttributeInGroupIsPinned = (!requiresLeader());
	event.attr.pinned = ((leader || isNotInAReadFormatGroup ||
			      everyAttributeInGroupIsPinned) ?
				     1 :
				     0);
	...
	event.attr.use_clockid =
		sharedConfig.perfConfig.has_attr_clockid_support ? 1 : 0;
	event.attr.clockid = sharedConfig.perfConfig.has_attr_clockid_support ?
				     CLOCK_MONOTONIC_RAW :
				     0;
	...
	event.attr.exclude_kernel =
		(sharedConfig.perfConfig.exclude_kernel ? 1 : 0);
	event.attr.exclude_hv =
		(sharedConfig.perfConfig.exclude_kernel ? 1 : 0);
	event.attr.exclude_idle =
		(sharedConfig.perfConfig.exclude_kernel ? 1 : 0);
	event.attr.aux_watermark =
		hasAuxData ? sharedConfig.auxBufferLength / 2 : 0;
	event.key = key;
...
bool PerfEventGroup::createGroupLeader(uint64_t timestamp,
				       IPerfAttrsConsumer &attrsConsumer)
{
	switch (groupIdentifier.getType()) {
	case PerfEventGroupIdentifier::Type::PER_CLUSTER_CPU:
...
bool PerfEventGroup::createCpuGroupLeader(const uint64_t timestamp,
					  IPerfAttrsConsumer &attrsConsumer)
{
	...
	if (sharedConfig.perfConfig.can_access_tracepoints) {
		...
		if (sharedConfig.perfConfig.has_attr_context_switch) {
			...
			// otherwise use sampling as leader
				...
				attr.periodOrFreq =
					(sharedConfig.sampleRate > 0 &&
							 sharedConfig
								 .enablePeriodicSampling ?
						 NANO_SECONDS_IN_ONE_SECOND /
							 sharedConfig.sampleRate :
						 0);
				attr.sampleType |=
					PERF_SAMPLE_TID | PERF_SAMPLE_IP |
					PERF_SAMPLE_READ |
					(enableCallChain ?
	...
	// Group leader
	if (!addEvent(true, timestamp, attrsConsumer,
		      sharedConfig.schedSwitchKey, attr, false)) {
		return false;
	}
	...
	if ((attr.config != PERF_COUNT_SW_CPU_CLOCK) &&
		...
		pcAttr.sampleType =
			PERF_SAMPLE_TID | PERF_SAMPLE_IP | PERF_SAMPLE_READ |
			(enableCallChain ? PERF_SAMPLE_CALLCHAIN : 0);
		pcAttr.periodOrFreq =
			NANO_SECONDS_IN_ONE_SECOND / sharedConfig.sampleRate;
		if (!addEvent(false, timestamp, attrsConsumer, nextDummyKey(),
			      pcAttr, false)) {
			return false;
		}
	}
...
bool PerfEventGroup::createUncoreGroupLeader(const uint64_t timestamp,
					     IPerfAttrsConsumer &attrsConsumer)
{
	IPerfGroups::Attr attr{};
	attr.type = PERF_TYPE_SOFTWARE;
	...
	attr.periodOrFreq =
		(sharedConfig.sampleRate > 0 ?
			 NANO_SECONDS_IN_ONE_SECOND / sharedConfig.sampleRate :
			 NANO_SECONDS_IN_100_MS);

	return addEvent(true, timestamp, attrsConsumer, nextDummyKey(), attr,
			false);
}
...
std::pair<OnlineResult, std::string>
PerfEventGroup::onlineCPU(uint64_t timestamp, int cpu, std::set<int> &tids,
			  ...
			  std::function<bool(int, int, bool)> addToBuffer)
{
	...
	const GatorCpu &cpuCluster =
		sharedConfig.clusters[sharedConfig.clusterIds[cpu]];
	const GatorCpu *cluster = groupIdentifier.getCluster();
	const UncorePmu *uncorePmu = groupIdentifier.getUncorePmu();
	const std::map<int, int> *cpuNumberToType =
		groupIdentifier.getSpeTypeMap();
	...
	case PerfEventGroupIdentifier::Type::UNCORE_PMU: {
		...
		const std::set<int> cpuMask =
			perf_utils::readCpuMask(uncorePmu->getId());
	...
	for (std::size_t eventIndex = 0; eventIndex < numberOfEvents;
		...
		event.attr.enable_on_exec =
			(event.attr.pinned && enableOnExec) ? 1 : 0;
		if (replaceType) {
		...
		logg.logMessage(
			...
			(cluster != nullptr ?
				 ...
				 (uncorePmu != nullptr ? uncorePmu->getId() :
							 "<nullptr>")),
			eventIndex, event.attr.type, typeLabel,
			event.attr.config, event.attr.config1,
			event.attr.config2, event.attr.sample_period,
			event.attr.sample_type, event.attr.read_format,
			// need to cast to keep g++ happy otherwise it expects %llu unlike clang
			static_cast<unsigned>(event.attr.pinned),
			static_cast<unsigned>(event.attr.mmap),
			static_cast<unsigned>(event.attr.comm),
			static_cast<unsigned>(event.attr.freq),
			static_cast<unsigned>(event.attr.task),
			static_cast<unsigned>(event.attr.exclude_kernel),
			static_cast<unsigned>(event.attr.enable_on_exec),
			static_cast<unsigned>(event.attr.inherit),
			static_cast<unsigned>(event.attr.sample_id_all),
			event.attr.aux_watermark);

		for (auto tidsIterator = tids.begin();
		     tidsIterator != tids.end();) {
			const int tid = *tidsIterator;

			// This assumes that group leader is added first
			const int groupLeaderFd =
				event.attr.pinned ?
					-1 :
					*(eventIndexToTidToFdMap.at(0).at(tid));
			...
			// try with exclude_kernel clear
			{
				...
				fd = sys_perf_event_open(
					&event.attr, tid, cpu, groupLeaderFd,
			...
			if ((!fd) && (errno == EACCES)) {
				logg.logMessage(
					"Failed when exclude_kernel == 0, retrying with exclude_kernel = 1");
				...
				if ((!fd) && (errno == EACCES)) {
					logg.logMessage(
						"Failed when exclude_kernel == 1, exclude_hv == 0, exclude_idle == 0, retrying with all exclusions enabled");
			...
			logg.logMessage(
				"perf_event_open: tid: %i, leader = %i -> fd = %i",
				tid, groupLeaderFd, *fd);

			if (!fd) {
				logg.logMessage("failed (%d) %s", errno,
						strerror(errno));

				if (errno == ENODEV) {
					// The core is offline
					return std::make_pair(
						OnlineResult::CPU_OFFLINE,
				...
				stringStream
					<< "perf_event_open failed to online counter for "
					<< typeLabel << ":" << event.attr.config
					<< " on CPU " << cpu
					<< " due to errno = " << errno << "("
					<< strerror(errno) << ").";
				...
				if (sharedConfig.perfConfig.is_system_wide) {
					...
					logg.logMessage(
						"PerfBuffer::useFd failed");
				}
			} else if (!addToMonitor(*fd)) {
				std::string message("Monitor::add failed");
				return std::make_pair(OnlineResult::FAILURE,
						      message.c_str());
			} else {
				eventIndexToTidToFdMap[eventIndex][tid] =
					std::move(fd);
			}
	...
	if (sharedConfig.perfConfig.has_ioctl_read_id) {
		...
		attrsConsumer.marshalKeys(timestamp, ids.size(), ids.data(),
					  coreKeys.data());
	} else {
		...
		for (const auto &eventIndexToTidToFdPair :
			...
			const PerfEvent &event = events.at(eventIndex);
			const bool isLeader =
				requiresLeader() && (eventIndex == 0);

			if (event.attr.pinned && !isLeader) {
				for (const auto &tidToFdPair :
				     eventIndexToTidToFdPair.second) {
					const auto &fd = tidToFdPair.second;
					if (!readAndSend(timestamp,
							 attrsConsumer,
							 event.attr, *fd, 1,
							 &event.key)) {
						return std::make_pair(
		...
		assert((requiresLeader() || keysInGroup.empty()) &&
		       "Cannot read group items without leader");
		...
		if (!keysInGroup.empty()) {
			...
			for (const auto &tidToFdPair : tidToFdMap) {
				const auto &fd = tidToFdPair.second;
				if (!readAndSend(timestamp, attrsConsumer,
						 event.attr, *fd,
						 keysInGroup.size(),
						 keysInGroup.data())) {
					return std::make_pair(
	...
	if (enableNow) {
		if (!enable(eventIndexToTidToFdMap) ||
		    !checkEnabled(eventIndexToTidToFdMap)) {
			return std::make_pair(OnlineResult::OTHER_FAILURE,
					      "Unable to enable a perf event");
		}
	}
...
bool PerfEventGroup::offlineCPU(int cpu)
{
	...
	for (auto eventIndexToTidToFdIt = eventIndexToTidToFdMap.rbegin();
		...
		for (auto tidToFdIt = tidToFdMap.rbegin();
		     tidToFdIt != tidToFdRend; ++tidToFdIt) {
...
bool PerfEventGroup::enable(
	const std::map<int, std::map<int, lib::AutoClosingFd> >
		&eventIndexToTidToFdMap)
{
	...
	for (const auto &eventIndexToTidToFdPair : eventIndexToTidToFdMap) {
		...
		for (const auto &tidToFdPair : eventIndexToTidToFdPair.second) {
			...
			if (event.attr.pinned &&
			    (lib::ioctl(*fd, PERF_EVENT_IOC_ENABLE, 0) != 0)) {
...
bool PerfEventGroup::checkEnabled(
	const std::map<int, std::map<int, lib::AutoClosingFd> >
		&eventIndexToTidToFdMap)
{
	...
	for (const auto &eventIndexToTidToFdPair : eventIndexToTidToFdMap) {
		...
		for (const auto &tidToFdPair : eventIndexToTidToFdPair.second) {
			...
			if (event.attr.pinned &&
			    (lib::read(*fd, buf, sizeof(buf)) <= 0)) {
				logg.logError(
					"Unable to read all perf groups, perhaps too many events were enabled");
				return false;
			}
		}
	}
...
void PerfEventGroup::start()
{
	...
	for (const auto &cpuToEventIndexToTidToFdPair :
...
void PerfEventGroup::stop()
{
	for (const auto &cpuToEventIndexToTidToFdPair :
	     cpuToEventIndexToTidToFdMap) {
		const auto &eventIndexToTidToFdMap =
			cpuToEventIndexToTidToFdPair.second;
		const auto eventIndexToTidToFdRend =
			eventIndexToTidToFdMap.rend();
		for (auto eventIndexToTidToFdIt =
			     eventIndexToTidToFdMap.rbegin();
		     eventIndexToTidToFdIt != eventIndexToTidToFdRend;
		     ++eventIndexToTidToFdIt) {
			const auto &tidToFdMap = eventIndexToTidToFdIt->second;
			const auto tidToFdRend = tidToFdMap.rend();
