...
std::vector<std::unique_ptr<PerfSyncThreadBuffer> >
PerfSyncThreadBuffer::create(std::uint64_t monotonicRawBase,
			     bool supportsClockId, bool hasSPEConfiguration,
			     sem_t &senderSem)
{
	...
	const unsigned count =
		(hasSPEConfiguration ? std::thread::hardware_concurrency() :
				       (supportsClockId ? 0 : 1));
	...
	for (unsigned cpu = 0; cpu < count; ++cpu) {
		const bool enableSyncThreadMode =
			(!supportsClockId) && (cpu == 0);
		const bool readTimer = hasSPEConfiguration;
		result.emplace_back(new PerfSyncThreadBuffer(
			monotonicRawBase, cpu, enableSyncThreadMode, readTimer,
			&senderSem));
	}
...
PerfSyncThreadBuffer::PerfSyncThreadBuffer(std::uint64_t monotonicRawBase,
					   unsigned cpu,
					   bool enableSyncThreadMode,
					   bool readTimer, sem_t *readerSem)
	: monotonicRawBase(monotonicRawBase),
	  buffer(cpu, FrameType::PERF_SYNC, 1024 * 1024, readerSem),
	  thread(cpu, enableSyncThreadMode, readTimer, monotonicRawBase,
		 [this](unsigned c, pid_t p, pid_t t, std::uint64_t f,
			std::uint64_t cmr,
			std::uint64_t vcnt) { write(c, p, t, cmr, vcnt, f); })
{
}
...
void PerfSyncThreadBuffer::write(unsigned, pid_t pid, pid_t tid,
				 std::uint64_t monotonicRaw, std::uint64_t vcnt,
				 std::uint64_t freq)
{
	if (buffer.isDone()) {
		return;
	...
	const int minBytesRequired = ((1 * buffer_utils::MAXSIZE_PACK64) +
				      (3 * buffer_utils::MAXSIZE_PACK32)) +
