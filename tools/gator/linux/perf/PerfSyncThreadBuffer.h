...
class PerfSyncThreadBuffer {
     ...
     */
	static std::vector<std::unique_ptr<PerfSyncThreadBuffer> >
	create(std::uint64_t monotonicRawBase, bool supportsClockId,
	       bool hasSPEConfiguration, sem_t &senderSem);
     ...
     */
	PerfSyncThreadBuffer(std::uint64_t monotonicRawBase, unsigned cpu,
			     bool enableSyncThreadMode, bool readTimer,
			     sem_t *readerSem);
	...
	void write(unsigned cpu, pid_t pid, pid_t tid,
