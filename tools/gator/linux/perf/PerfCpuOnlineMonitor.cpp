...
{
	// rename thread
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>("gatord-cpumon"), 0,
	      0, 0);

	// monitor filesystem
	bool firstPass = true;
	const lib::FsEntry sysFsCpuRootPath =
		lib::FsEntry::create("/sys/devices/system/cpu");
	while (!terminated.load(std::memory_order_acquire)) {
		// loop through files
		lib::Optional<lib::FsEntry> child;
		lib::FsEntryDirectoryIterator iterator =
			sysFsCpuRootPath.children();
		...
		while ((child = iterator.next()).valid()) {
			...
			if ((name.length() > 3) && (name.find("cpu") == 0)) {
				// find a CPU node
				const unsigned cpu =
					strtoul(name.c_str() + 3, nullptr, 10);
				// read its online state
				const lib::FsEntry onlineFsEntry =
					lib::FsEntry::create(*child, "online");
				const std::string contents =
					onlineFsEntry
						.readFileContentsSingleLine();
				if (!contents.empty()) {
					const unsigned online = strtoul(
						contents.c_str(), nullptr, 0);
