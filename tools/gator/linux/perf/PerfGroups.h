...
class PerfGroups : public IPerfGroups {
    public:
	PerfGroups(const PerfConfig &perfConfig, size_t dataBufferLength,
	...
	virtual bool add(uint64_t timestamp, IPerfAttrsConsumer &attrsConsumer,
			 ...
			 int key, const Attr &attr,
			 bool hasAuxData = false) override;

	virtual void
	addGroupLeader(const uint64_t timestamp,
	...
	onlineCPU(uint64_t timestamp, int cpu, const std::set<int> &appPids,
	...
	PerfEventGroup &
	getGroup(const uint64_t timestamp, IPerfAttrsConsumer &attrsConsumer,
		 const PerfEventGroupIdentifier &groupIdentifier);

	PerfEventGroupSharedConfig sharedConfig;
	std::map<PerfEventGroupIdentifier, std::unique_ptr<PerfEventGroup> >
		perfEventGroupMap;
