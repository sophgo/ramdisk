...
class PerfDriver : public SimpleDriver {
    public:
	PerfDriver(PerfDriverConfiguration &&configuration, PmuXML &&pmuXml,
	...
	bool summary(ISummaryConsumer &consumer,
		     std::function<uint64_t()> getAndSetMonotonicStarted);
	void coreName(const uint64_t currTime, ISummaryConsumer &consumer,
		      const int cpu);
	void setupCounter(Counter &counter) override;
	lib::Optional<CapturedSpe>
	setupSpe(const SpeConfiguration &spe) override;
	bool enable(const uint64_t currTime, IPerfGroups &group,
		    IPerfAttrsConsumer &attrsConsumer) const;
	void read(IPerfAttrsConsumer &attrsConsumer, const int cpu);
	bool sendTracepointFormats(const uint64_t currTime,
