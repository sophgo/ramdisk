...
#define SET_SPE_CFG(cfg, value)                                                \
	SPE_##cfg##_CFG |= static_cast<uint64_t>(value) << SPE_##cfg##_LO
...
class PerfCounter : public DriverCounter {
	...
	PerfCounter(DriverCounter *next,
		    ...
		    const char *name, const IPerfGroups::Attr &attr,
		    bool usesAux, uint64_t config_id2 = noConfigId2,
		    bool fixUpClockCyclesEvent = false)
		: DriverCounter(next, name),
		  eventGroupIdentifier(groupIdentifier), attr(attr),
	...
	PerfCounter(DriverCounter *next,
		    ...
		    uint64_t sampleType, uint64_t count,
		    uint64_t config_id2 = noConfigId2,
		    bool fixUpClockCyclesEvent = false)
		: PerfCounter(next, groupIdentifier, name, IPerfGroups::Attr{},
			      false, config_id2, fixUpClockCyclesEvent)
	{
		attr.type = type;
		attr.config = config;
     ...
     */
	virtual void read(IPerfAttrsConsumer &, const int /* cpu */,
			  const GatorCpu * /* cluster */)
	{
	}

	inline const PerfEventGroupIdentifier &
	getPerfEventGroupIdentifier() const
	{
	...
	inline void setConfig(const uint64_t config)
	{
		...
		if (mFixUpClockCyclesEvent &&
		    config == armv7AndLaterClockCyclesEvent)
...
class CPUFreqDriver : public PerfCounter {
    public:
	CPUFreqDriver(DriverCounter *next, const char *name, uint64_t id,
		      const GatorCpu &cluster)
		: PerfCounter(next, PerfEventGroupIdentifier(cluster), name,
			      PERF_TYPE_TRACEPOINT, id, PERF_SAMPLE_RAW, 1)
	{
	}
		  ...
		  const GatorCpu *cluster)
	{
		if (cluster == nullptr ||
		    !(*cluster ==
		      *getPerfEventGroupIdentifier().getCluster())) {
			return;
		}
...
template <typename T> inline static T &neverNull(T *t)
{
	if (t == nullptr) {
		handleException();
	}
...
static long long getTracepointId(const char *const counter,
				 const char *const name)
{
	long long result = getTracepointId(name);
	if (result <= 0) {
		logg.logSetup("%s is disabled\n%s was not found", counter,
			      getTracepointPath(name, "id").c_str());
	}
	return result;
}

PerfDriver::PerfDriver(PerfDriverConfiguration &&configuration, PmuXML &&pmuXml,
		       const char *maliFamilyName, const ICpuInfo &cpuInfo)
	: SimpleDriver("Perf"), mTracepoints(nullptr), mIsSetup(false),
	  mConfig(std::move(configuration)), mPmuXml(pmuXml), mCpuInfo(cpuInfo)
{
	// add CPU PMUs
	for (const auto &perfCpu : mConfig.cpus) {
		if ((perfCpu.pmu_type != PERF_TYPE_RAW) &&
		    (perfCpu.pmu_type != PERF_TYPE_HARDWARE)) {
			logg.logMessage(
				"Adding cpu counters for %s with type %i",
				perfCpu.gator_cpu.getCoreName(),
				perfCpu.pmu_type);
		} else if ((perfCpu.gator_cpu.getCpuIds().size() > 1) ||
			   (!perfCpu.gator_cpu.hasCpuId(
				   PerfDriverConfiguration::UNKNOWN_CPUID))) {
			logg.logMessage(
				"Adding cpu counters (based on cpuid) for %s",
				perfCpu.gator_cpu.getCoreName());
		} else {
			logg.logMessage(
				"Adding cpu counters based on default CPU object");
		}
		...
		if (id >= 0) {
			for (const auto &perfCpu : mConfig.cpus) {
				snprintf(buf, sizeof(buf), "%s_softirq",
					 perfCpu.gator_cpu.getId());
				setCounters(new PerfCounter(
					getCounters(),
					PerfEventGroupIdentifier(
						perfCpu.gator_cpu),
					buf, PERF_TYPE_TRACEPOINT, id,
					PERF_SAMPLE_READ, 0));
			}
		}
	...
	if (getConfig().can_access_tracepoints ||
	    getConfig().has_attr_context_switch) {
		setCounters(new PerfCounter(
			getCounters(), PerfEventGroupIdentifier(),
			"Linux_cpu_wait_contention", TYPE_DERIVED, -1, 0, 0));
		for (const auto &perfCpu : mConfig.cpus) {
			snprintf(buf, sizeof(buf), "%s_system",
				 perfCpu.gator_cpu.getId());
			setCounters(new PerfCounter(
				getCounters(),
				PerfEventGroupIdentifier(perfCpu.gator_cpu),
				buf, TYPE_DERIVED, -1, 0, 0));
			snprintf(buf, sizeof(buf), "%s_user",
				 perfCpu.gator_cpu.getId());
			setCounters(new PerfCounter(
				getCounters(),
				PerfEventGroupIdentifier(perfCpu.gator_cpu),
	...
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    ...
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_TASK_CLOCK, 0, 0));
	if (!getConfig().exclude_kernel) {
		// requires ability to read kernel events
		setCounters(new PerfCounter(
			getCounters(), PerfEventGroupIdentifier(),
			"PERF_COUNT_SW_CONTEXT_SWITCHES", PERF_TYPE_SOFTWARE,
			PERF_COUNT_SW_CONTEXT_SWITCHES, 0, 0));
	}
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    "PERF_COUNT_SW_CPU_MIGRATIONS",
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_CPU_MIGRATIONS, 0, 0));
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    "PERF_COUNT_SW_PAGE_FAULTS",
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_PAGE_FAULTS, 0, 0));
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    "PERF_COUNT_SW_PAGE_FAULTS_MAJ",
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_PAGE_FAULTS_MAJ, 0, 0));
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    "PERF_COUNT_SW_PAGE_FAULTS_MIN",
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_PAGE_FAULTS_MIN, 0, 0));
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    "PERF_COUNT_SW_ALIGNMENT_FAULTS",
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_ALIGNMENT_FAULTS, 0, 0));
	setCounters(new PerfCounter(getCounters(), PerfEventGroupIdentifier(),
				    "PERF_COUNT_SW_EMULATION_FAULTS",
				    PERF_TYPE_SOFTWARE,
				    PERF_COUNT_SW_EMULATION_FAULTS, 0, 0));
...
class PerfTracepoint {
    public:
	PerfTracepoint(PerfTracepoint *const next,
		       const DriverCounter *const counter,
		       const char *const tracepoint)
...
{
	for (PerfTracepoint *nextTracepoint = mTracepoints;
	     nextTracepoint != nullptr;) {
...
void PerfDriver::addCpuCounters(const PerfCpu &perfCpu)
{
...

	{
		...
		setCounters(new PerfCounter(
			getCounters(), PerfEventGroupIdentifier(cpu),
	...
	for (int j = 0; j < cpu.getPmncCounters(); ++j) {
		const int len =
			snprintf(NULL, 0, "%s_cnt%d", cpu.getId(), j) + 1;
		const std::unique_ptr<char[]> name{ new char[len] };
	...
	if (speId != nullptr) {
		...
		setCounters(new PerfCounter(
			getCounters(),
			PerfEventGroupIdentifier(mConfig.cpuNumberToSpeType),
			speId, attr, true));
	}
}
...
void PerfDriver::addUncoreCounters(const PerfUncore &perfUncore)
{
	...
	if (pmu.getHasCyclesCounter()) {
		const int len =
			snprintf(NULL, 0, "%s_ccnt", pmu.getCounterSet()) + 1;
		const std::unique_ptr<char[]> name{ new char[len] };
		snprintf(name.get(), len, "%s_ccnt", pmu.getCounterSet());
		setCounters(new PerfCounter(
			getCounters(), PerfEventGroupIdentifier(pmu),
			name.get(), type, -1, PERF_SAMPLE_READ, 0));
	}

	for (int j = 0; j < pmu.getPmncCounters(); ++j) {
		const int len =
			snprintf(NULL, 0, "%s_cnt%d", pmu.getCounterSet(), j) +
			1;
		const std::unique_ptr<char[]> name{ new char[len] };
...
void PerfDriver::readEvents(mxml_node_t *const xml)
{
	...
	while (true) {
		node = mxmlFindElement(node, xml, "event", NULL, NULL,
				       MXML_DESCEND);
		if (node == NULL) {
			break;
		}
		...
		if (tracepoint == NULL) {
			...
			if (regex == NULL) {
				logg.logError(
					"The tracepoint counter %s is missing the required tracepoint attribute",
					counter);
				handleException();
			} else {
				logg.logMessage("Not using perf for counter %s",
						counter);
				continue;
			}
		}
		...
		if (id >= 0) {
			...
			setCounters(new PerfCounter(
				...
				arg == NULL ? 0 : PERF_SAMPLE_RAW, 1));
			mTracepoints = new PerfTracepoint(
				mTracepoints, getCounters(), tracepoint);
		}
	}
}
...
void PerfDriver::addMidgardHwTracepoints(const char *const maliFamilyName)
{
	...
	if (!isSystemWide || !canAccessTrcPnt) {
		logg.logMessage(
			"No Mali Tracepoint counters added, (systemwide (%d), canAccessTracepoints(%d))",
			isSystemWide, canAccessTrcPnt);
		return;
	}

	static const char *const MALI_MIDGARD_AS_IN_USE_RELEASED[] = {
		"MMU_AS_0", "MMU_AS_1", "MMU_AS_2", "MMU_AS_3"
	};

	static const char *const MALI_MIDGARD_PAGE_FAULT_INSERT_PAGES[] = {
		"MMU_PAGE_FAULT_0", "MMU_PAGE_FAULT_1", "MMU_PAGE_FAULT_2",
		"MMU_PAGE_FAULT_3"
	};

	static const char *const MALI_MIDGARD_TOTAL_ALLOC_PAGES =
		"TOTAL_ALLOC_PAGES";
	...
	// add midgard software tracepoints
	auto addCounterWithConfigId2 = [this](const char *name, uint64_t id,
					      uint64_t configId2) {
		IPerfGroups::Attr attr{};
		...
		setCounters(new PerfCounter(getCounters(),
					    PerfEventGroupIdentifier(), name,
					    attr, false, configId2, false));
	};
	auto addCounter = [addCounterWithConfigId2](const char *name,
						    uint64_t id) {
	...
	if (false) { /* These are disabled because they never generate events */
		static const char *const MALI_MIDGARD_PM_STATUS_EVENTS[] = {
			"PM_SHADER_0", "PM_SHADER_1", "PM_SHADER_2",
		...
		if (id >= 0) {
			for (size_t i = 0;
			     i < COUNT_OF(MALI_MIDGARD_PM_STATUS_EVENTS); ++i) {
				snprintf(buf, sizeof(buf), "ARM_Mali-%s_%s",
					 maliFamilyName,
					 MALI_MIDGARD_PM_STATUS_EVENTS[i]);
				addCounter(buf, id);
				mTracepoints = new PerfTracepoint(
					mTracepoints, getCounters(),
					"mali/mali_pm_status");
			}
		}
	}

	id = getTracepointId(MALI_MMU_IN_USE,
			     MALI_TRC_PNT_PATH[MALI_MMU_IN_USE]);
	if (id >= 0) {
		const int id2 = getTracepointId(
			MALI_PM_STATUS, MALI_TRC_PNT_PATH[MALI_PM_STATUS]);
		for (size_t i = 0;
		     i < COUNT_OF(MALI_MIDGARD_PAGE_FAULT_INSERT_PAGES); ++i) {
			snprintf(buf, sizeof(buf), "ARM_Mali-%s_%s",
				 maliFamilyName,
				 MALI_MIDGARD_AS_IN_USE_RELEASED[i]);
			...
			mTracepoints = new PerfTracepoint(
				mTracepoints, getCounters(),
				MALI_TRC_PNT_PATH[MALI_PM_STATUS]);
		}
	}

	id = getTracepointId(MALI_MMU_PAGE_FAULT,
			     MALI_TRC_PNT_PATH[MALI_MMU_PAGE_FAULT]);
	if (id >= 0) {
		for (size_t i = 0;
		     i < COUNT_OF(MALI_MIDGARD_PAGE_FAULT_INSERT_PAGES); ++i) {
			snprintf(buf, sizeof(buf), "ARM_Mali-%s_%s",
				 maliFamilyName,
				 MALI_MIDGARD_PAGE_FAULT_INSERT_PAGES[i]);
			addCounter(buf, id);
			mTracepoints = new PerfTracepoint(
				mTracepoints, getCounters(),
				MALI_TRC_PNT_PATH[MALI_MMU_PAGE_FAULT]);
	...
	id = getTracepointId(MALI_MMU_TOTAL_ALLOC,
			     MALI_TRC_PNT_PATH[MALI_MMU_TOTAL_ALLOC]);
	if (id >= 0) {
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_%s", maliFamilyName,
			 MALI_MIDGARD_TOTAL_ALLOC_PAGES);
		addCounter(buf, id);
		mTracepoints = new PerfTracepoint(
			mTracepoints, getCounters(),
			MALI_TRC_PNT_PATH[MALI_MMU_TOTAL_ALLOC]);
	...
	if (id >= 0) {
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_fragment",
			 maliFamilyName);
		addCounter(buf, id);
		mTracepoints =
			new PerfTracepoint(mTracepoints, getCounters(),
					   MALI_TRC_PNT_PATH[MALI_JOB_SLOT]);
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_vertex",
			 maliFamilyName);
		addCounter(buf, id);
		mTracepoints =
			new PerfTracepoint(mTracepoints, getCounters(),
					   MALI_TRC_PNT_PATH[MALI_JOB_SLOT]);
		snprintf(buf, sizeof(buf), "ARM_Mali-%s_opencl",
			 maliFamilyName);
		addCounter(buf, id);
		mTracepoints =
			new PerfTracepoint(mTracepoints, getCounters(),
...
bool PerfDriver::summary(ISummaryConsumer &consumer,
			 std::function<uint64_t()> getAndSetMonotonicStarted)
{
	...
	char buf[512];
	snprintf(buf, sizeof(buf), "%s %s %s %s %s GNU/Linux", utsname.sysname,
	...
	const uint64_t currTime =
		0; //getTime() - gSessionData.mMonotonicStarted;

	std::map<std::string, std::string> additionalAttributes;

	additionalAttributes["perf.is_root"] = (geteuid() == 0 ? "1" : "0");
	additionalAttributes["perf.is_system_wide"] =
		(getConfig().is_system_wide ? "1" : "0");
	additionalAttributes["perf.can_access_tracepoints"] =
		(getConfig().can_access_tracepoints ? "1" : "0");
	additionalAttributes["perf.has_attr_context_switch"] =
		(getConfig().has_attr_context_switch ? "1" : "0");

	lnx::addDefaultSysfsSummaryInformation(additionalAttributes);

	consumer.summary(currTime, timestamp, monotonicStarted,
			 monotonicStarted, buf, pageSize,
			 getConfig().has_attr_clockid_support,
...
void PerfDriver::coreName(const uint64_t currTime, ISummaryConsumer &consumer,
			  const int cpu)
{
	...
	if (gatorCpu != nullptr) {
		consumer.coreName(currTime, cpu, cpuId,
...
void PerfDriver::setupCounter(Counter &counter)
{
	PerfCounter *const perfCounter =
		static_cast<PerfCounter *>(findCounter(counter));
	if (perfCounter == NULL) {
		counter.setEnabled(false);
		return;
	}

	logg.logMessage("Configuring perf counter %s with event (%d)",
			perfCounter->getName(), counter.getEvent());
...
{
	for (PerfCounter *counter = static_cast<PerfCounter *>(getCounters());
	     counter != NULL;
		...
		if (spe.id == counter->getName()) {
			...
			counter->setCount(
				100000 /* approx 10KHz sample rate at 1GHz CPU clock */);
...
bool PerfDriver::enable(const uint64_t currTime, IPerfGroups &group,
			IPerfAttrsConsumer &attrsConsumer) const
{
	const uint64_t id =
		getConfig().can_access_tracepoints ?
			getTracepointId("Mali: Job slot events",
					"mali/mali_job_slots_event") :
			0 /* never used */;
	bool sentMaliJobSlotEvents = false;

	for (const PerfCpu &cluster : mConfig.cpus) {
		PerfEventGroupIdentifier clusterGroupIdentifier(
			cluster.gator_cpu);
		group.addGroupLeader(currTime, attrsConsumer,
				     clusterGroupIdentifier);
	}
	...
	for (PerfCounter *counter = static_cast<PerfCounter *>(getCounters());
		...
		if (counter->isEnabled() &&
		    (counter->getAttr().type != TYPE_DERIVED)) {
			...
			const bool isMaliJobSlotEvents =
				getConfig().can_access_tracepoints &&
				(counter->getAttr().type ==
				 PERF_TYPE_TRACEPOINT) &&
				(counter->getAttr().config == id);
			const bool skip =
				(isMaliJobSlotEvents && sentMaliJobSlotEvents);

			sentMaliJobSlotEvents |= isMaliJobSlotEvents;

			if (!skip) {
				if (group.add(
					    currTime, attrsConsumer,
					...
					if (counter->hasConfigId2()) {
						if (!group.add(
							    ...
							    counter->usesAux())) {
							logg.logMessage(
								"PerfGroups::add (2nd) failed");
							return false;
						}
					}
				} else {
					logg.logMessage(
						"PerfGroups::add failed");
					return false;
				}
			}
		}
...
bool PerfDriver::sendTracepointFormats(const uint64_t currTime,
				       IPerfAttrsConsumer &attrsConsumer)
{
	...
	for (PerfTracepoint *tracepoint = mTracepoints; tracepoint != NULL;
	     tracepoint = tracepoint->getNext()) {
		if (tracepoint->getCounter()->isEnabled() &&
		    !readTracepointFormat(currTime, attrsConsumer,
