...
PerfEventGroupIdentifier::PerfEventGroupIdentifier(int cpuNumber)
	: cluster(nullptr), pmu(nullptr), cpuNumber(cpuNumber),
	  cpuNumberToType(nullptr)
{
	assert(cpuNumber >= 0);
}
...
PerfEventGroupIdentifier::PerfEventGroupIdentifier(
	const std::map<int, int> &cpuToTypeMap)
	: cluster(nullptr), pmu(nullptr), cpuNumber(-1),
	  cpuNumberToType(&cpuToTypeMap)
{
}

bool PerfEventGroupIdentifier::
operator<(const PerfEventGroupIdentifier &that) const
{
	...
	if (cluster != nullptr) {
		...
		const int minThis =
			*std::min_element(cluster->getCpuIds().begin(),
					  cluster->getCpuIds().end());
		const int minThat =
			*std::min_element(that.cluster->getCpuIds().begin(),
					  that.cluster->getCpuIds().end());
	...
	if (pmu != nullptr) {
		return (that.pmu != nullptr ?
				(strcmp(pmu->getId(), that.pmu->getId()) < 0) :
				true);
	...
	if (cpuNumberToType != nullptr) {
		return (that.cpuNumberToType != nullptr ?
