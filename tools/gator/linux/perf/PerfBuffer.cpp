...
template <typename T> static T readOnceAtomicRelaxed(const T &val)
{
	return __atomic_load_n(static_cast<const volatile T *>(&val),
			       __ATOMIC_RELAXED);
}
...
void validate(const PerfBuffer::Config &config)
{
	if (((config.pageSize - 1) & config.pageSize) != 0) {
		logg.logError(
			"PerfBuffer::Config.pageSize (%zu) must be a power of 2",
			config.pageSize);
		handleException();
	}
	if (((config.dataBufferSize - 1) & config.dataBufferSize) != 0) {
		logg.logError(
			"PerfBuffer::Config.dataBufferSize (%zu) must be a power of 2",
			config.dataBufferSize);
		handleException();
	}
	if (config.dataBufferSize < config.pageSize) {
		logg.logError(
			"PerfBuffer::Config.dataBufferSize (%zu) must be a multiple of PerfBuffer::Config.pageSize (%zu)",
			config.dataBufferSize, config.pageSize);
		handleException();
	}

	if (((config.auxBufferSize - 1) & config.auxBufferSize) != 0) {
		logg.logError(
			"PerfBuffer::Config.auxBufferSize (%zu) must be a power of 2",
			config.auxBufferSize);
		handleException();
	}
	if ((config.auxBufferSize < config.pageSize) &&
	    (config.auxBufferSize != 0)) {
		logg.logError(
			"PerfBuffer::Config.auxBufferSize (%zu) must be a multiple of PerfBuffer::Config.pageSize (%zu)",
			config.auxBufferSize, config.pageSize);
		handleException();
	}
}
...
PerfBuffer::PerfBuffer(PerfBuffer::Config config)
	: mConfig(config), mBuffers(), mDiscard()
{
	validate(mConfig);
}
...
PerfBuffer::~PerfBuffer()
{
	for (auto cpuAndBuf : mBuffers) {
		lib::munmap(cpuAndBuf.second.data_buffer,
			    getDataMMapLength(mConfig));
		if (cpuAndBuf.second.aux_buffer != nullptr)
			lib::munmap(cpuAndBuf.second.aux_buffer,
				    getAuxBufferLength());
	}
}
...
bool PerfBuffer::useFd(const int fd, int cpu, bool collectAuxTrace)
{
	auto mmap = [this, cpu](size_t length, size_t offset, int fd) {
		void *const buf =
			lib::mmap(NULL, length, PROT_READ | PROT_WRITE,
				  MAP_SHARED, fd, offset);

		if (buf == MAP_FAILED) {
			logg.logMessage(
				"mmap failed for fd %i (errno=%d, %s, mmapLength=%zu, offset=%zu)",
				fd, errno, strerror(errno), length, offset);
			if ((errno == ENOMEM) ||
			    ((errno == EPERM) && (geteuid() != 0))) {
				logg.logError(
	...
	if (buffer != mBuffers.end()) {
		if (lib::ioctl(fd, PERF_EVENT_IOC_SET_OUTPUT,
			       buffer->second.fd) < 0) {
			logg.logMessage("ioctl failed for fd %i (errno=%d, %s)",
		...
		struct perf_event_mmap_page &pemp =
			*static_cast<struct perf_event_mmap_page *>(buf);
		// Check the version
		const uint32_t compat_version = pemp.compat_version;
		if (compat_version != 0) {
			logg.logMessage(
				"Incompatible perf_event_mmap_page compat_version (%i) for fd %i",
				compat_version, fd);
			return false;
		}
	}
...
bool PerfBuffer::isEmpty()
{
	for (auto cpuAndBuf : mBuffers) {
		...
		struct perf_event_mmap_page *pemp =
			static_cast<struct perf_event_mmap_page *>(
				cpuAndBuf.second.data_buffer);
		const uint64_t dataTail =
			readOnceAtomicRelaxed(pemp->data_tail);
		const uint64_t dataHead =
			readOnceAtomicRelaxed(pemp->data_head);

		if (dataHead != dataTail) {
			return false;
		}
		if (cpuAndBuf.second.aux_buffer != nullptr &&
		    readOnceAtomicRelaxed(pemp->aux_tail) !=
			    readOnceAtomicRelaxed(pemp->aux_head)) {
			return false;
		}
	}
...
class PerfDataFrame {
	...
	PerfDataFrame(ISender &sender)
		: mSender(sender), mWritePos(-1), mCpuSizePos(-1)
	{
	}

	void add(const int cpu, uint64_t head, uint64_t tail, const char *b,
		 std::size_t length)
	{
		cpuHeader(cpu);
	...
	void send()
	{
		if (mWritePos > 0) {
			writeCpuSize();
			mSender.writeData(mBuf, mWritePos,
					  ResponseType::APC_DATA);
			mWritePos = -1;
			mCpuSizePos = -1;
		}
	}
	...
	void frameHeader()
	{
		if (mWritePos < 0) {
			...
			buffer_utils::packInt(
				mBuf, mWritePos,
				static_cast<uint32_t>(FrameType::PERF_DATA));
		}
	}
	...
	{
		if (sizeof(mBuf) <= mWritePos + buffer_utils::MAXSIZE_PACK32 +
					    sizeof(uint32_t)) {
			send();
		}
		frameHeader();
...
static void sendAuxFrame(ISender &sender, int cpu, uint64_t tail, uint64_t head,
			 const char *buffer, std::size_t length)
{
	const std::size_t bufferMask = length - 1;
	constexpr std::size_t maxHeaderSize =
		buffer_utils::MAXSIZE_PACK32 // frame type
	...
	while (tail < head) {
		...
		const uint64_t thisHead = std::min(
			tail + ISender::MAX_RESPONSE_LENGTH - maxHeaderSize,
			head);
		...
		buffer_utils::packInt(
			header, pos,
			static_cast<uint32_t>(FrameType::PERF_AUX));
		...
		const lib::Span<const char, int> parts[numberOfParts] = {
			{ header, pos },
			{ buffer + tailMasked, firstSize },
			{ buffer, secondSize }
		};
		sender.writeDataParts({ parts, numberOfParts },
				      ResponseType::APC_DATA);
		tail = thisHead;
	}
}
...
bool PerfBuffer::send(ISender &sender)
{
	...
	for (auto cpuAndBufIt = mBuffers.begin();
	     cpuAndBufIt != mBuffers.end();) {
		...
		struct perf_event_mmap_page *pemp =
			static_cast<struct perf_event_mmap_page *>(dataBuf);
		const uint64_t dataHead =
			__atomic_load_n(&pemp->data_head, __ATOMIC_ACQUIRE);
		...
		if (auxBuf != nullptr) {
			const uint64_t auxHead = __atomic_load_n(
				&pemp->aux_head, __ATOMIC_ACQUIRE);
			// Only we write this so no atomic load needed
			const uint64_t auxTail = pemp->aux_tail;

			if (auxHead > auxTail) {
				const char *const b =
					static_cast<char *>(auxBuf);

				sendAuxFrame(sender, cpu, auxTail, auxHead, b,
					     auxBufferLength);

				// Update tail with the aux read and synchronize with the buffer writer
				__atomic_store_n(&pemp->aux_tail, auxHead,
						 __ATOMIC_RELEASE);
			} else if ((auxHead > 0) && (!shouldDiscard) &&
				   (cpuAndBufIt->second.aux_fd >= 0)) {
				const std::size_t usedLength =
					(auxHead % auxBufferLength);
				if ((auxHead < auxTail) ||
				    (usedLength > (auxBufferLength / 2))) {
					logg.logMessage(
						"Aux buffer might be full, hence trying to issue an ioctl() to re-enable tracing. (%" PRIu64
						" == %" PRIu64 " -> %zu)",
						auxHead, auxTail, usedLength);
					if (lib::ioctl(
						    cpuAndBufIt->second.aux_fd,
