...
struct PerfEventGroupSharedConfig {
	inline PerfEventGroupSharedConfig(
		const PerfConfig &perfConfig, size_t dataBufferLength,
		size_t auxBufferLength, int backtraceDepth, int sampleRate,
		bool enablePeriodicSampling, lib::Span<const GatorCpu> clusters,
		lib::Span<const int> clusterIds, int64_t schedSwitchId)
		: perfConfig(perfConfig), schedSwitchId(schedSwitchId),
		  schedSwitchKey(INT_MAX), dummyKeyCounter(INT_MAX - 1),
		  dataBufferLength(dataBufferLength),
		  auxBufferLength(auxBufferLength),
		  backtraceDepth(backtraceDepth), sampleRate(sampleRate),
		  enablePeriodicSampling(enablePeriodicSampling),
		  clusters(clusters), clusterIds(clusterIds)
	{
	}
...
class PerfEventGroup {
    public:
	PerfEventGroup(const PerfEventGroupIdentifier &groupIdentifier,
		       PerfEventGroupSharedConfig &sharedConfig);

	bool requiresLeader() const;
	bool hasLeader() const;
	bool addEvent(bool leader, uint64_t timestamp,
		      IPerfAttrsConsumer &attrsConsumer, int key,
		      const IPerfGroups::Attr &attr, bool hasAuxData);
	bool createGroupLeader(uint64_t timestamp,
			       IPerfAttrsConsumer &attrsConsumer);

	std::pair<OnlineResult, std::string>
	onlineCPU(uint64_t timestamp, int cpu, std::set<int> &tids,
	...
	bool createCpuGroupLeader(uint64_t timestamp,
				  IPerfAttrsConsumer &attrsConsumer);
	bool createUncoreGroupLeader(uint64_t timestamp,
				     IPerfAttrsConsumer &attrsConsumer);

	bool enable(const std::map<int, std::map<int, lib::AutoClosingFd> >
			    &eventIndexToTidToFdMap);
	bool
	checkEnabled(const std::map<int, std::map<int, lib::AutoClosingFd> >
			     &eventIndexToTidToFdMap);
	...
	std::map<int, std::map<int, std::map<int, lib::AutoClosingFd> > >
