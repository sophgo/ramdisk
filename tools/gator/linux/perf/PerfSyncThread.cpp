...
PerfSyncThread::PerfSyncThread(unsigned cpu, bool enableSyncThreadMode,
			       bool readTimer, std::uint64_t monotonicRawBase,
			       ConsumerFunction consumerFunction)
	: thread(), consumerFunction(consumerFunction),
	  monotonicRawBase(monotonicRawBase), cpu(cpu), terminateFlag(false),
	  readTimer(readTimer), enableSyncThreadMode(enableSyncThreadMode)
{
	runtime_assert(
		enableSyncThreadMode || readTimer,
		"At least one of enableSyncThreadMode or readTimer are required");
...
void PerfSyncThread::rename(std::uint64_t currentTime)
{
	...
	if (enableSyncThreadMode && (cpu == 0)) {
		char buffer[16];
		const std::uint64_t uSeconds =
			(currentTime - monotonicRawBase) / NS_TO_US;
		if (uSeconds <= 9999999999ull) {
			snprintf(buffer, sizeof(buffer), "gds-%010u-",
				 static_cast<unsigned>(uSeconds));
			prctl(PR_SET_NAME,
			      reinterpret_cast<unsigned long>(&buffer[0]), 0, 0,
			      0);
		} else {
			prctl(PR_SET_NAME,
			      reinterpret_cast<unsigned long>("gator-sync-0"),
			      0, 0, 0);
		}
	}
}
...
void PerfSyncThread::run() noexcept
{
	...
	// affine the thread to a single CPU
	{
		...
		if (sched_setaffinity(tid, sizeof(cpu_set_t), &cpuset) != 0) {
			logg.logError(
				"Error calling sched_setaffinity on %u: %d (%s)",
	...
	// change thread priority
	{
		...
		if (sched_setscheduler(tid, SCHED_FIFO | SCHED_RESET_ON_FORK,
				       &param) != 0) {
			logg.logMessage(
				"Unable to schedule sync thread as FIFO, trying OTHER: %d (%s)",
				errno, strerror(errno));
			param.sched_priority =
				sched_get_priority_max(SCHED_OTHER);
			if (sched_setscheduler(
				    tid, SCHED_OTHER | SCHED_RESET_ON_FORK,
				    &param) != 0) {
				logg.logMessage(
					"sched_setscheduler failed for %u: %d (%s)",
					cpu, errno, strerror(errno));
			}
		}
	}
	...
	// Mask all signals so that this thread will not be woken up
	{
		sigset_t set;
		if (sigfillset(&set) != 0) {
			logg.logError("sigfillset failed: %d (%s)", errno,
				      strerror(errno));
			handleException();
		}
		if (sigprocmask(SIG_SETMASK, &set, nullptr) != 0) {
			logg.logError("sigprocmask failed %d (%s)", errno,
				      strerror(errno));
			handleException();
		}
	}
	...
	// rename thread
	{
		...
		prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&buffer[0]),
		      0, 0, 0);
	}
	...
	do {
		...
		if (nanosleep(&ts, nullptr) != 0) {
			logg.logError("nanosleep failed for %u: %d (%s)", cpu,
				      errno, strerror(errno));
			handleException();
		}
