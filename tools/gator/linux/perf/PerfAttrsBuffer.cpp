...
void PerfAttrsBuffer::marshalPea(const uint64_t currTime,
				 const struct perf_event_attr *const pea,
				 int key)
{
...
void PerfAttrsBuffer::marshalKeys(const uint64_t currTime, const int count,
				  const uint64_t *const ids,
				  const int *const keys)
{
	buffer.waitForSpace(2 * buffer_utils::MAXSIZE_PACK32 +
			    count * (buffer_utils::MAXSIZE_PACK32 +
...
void PerfAttrsBuffer::marshalKeysOld(const uint64_t currTime,
				     ...
				     const int bytes, const char *const buf)
{
	buffer.waitForSpace((2 + keyCount) * buffer_utils::MAXSIZE_PACK32 +
			    bytes);
...
void PerfAttrsBuffer::marshalFormat(const uint64_t currTime, const int length,
...
void PerfAttrsBuffer::marshalMaps(const uint64_t currTime, const int pid,
				  const int tid, const char *const maps)
{
...
void PerfAttrsBuffer::marshalComm(const uint64_t currTime, const int pid,
				  const int tid, const char *const image,
				  const char *const comm)
{
	const int imageLen = strlen(image) + 1;
	const int commLen = strlen(comm) + 1;
	buffer.waitForSpace(3 * buffer_utils::MAXSIZE_PACK32 + imageLen +
			    commLen);
...
void PerfAttrsBuffer::marshalKallsyms(const uint64_t currTime,
				      const char *const kallsyms)
{
...
void PerfAttrsBuffer::perfCounter(const int core, const int key,
				  const int64_t value)
{
	buffer.waitForSpace(2 * buffer_utils::MAXSIZE_PACK32 +
			    buffer_utils::MAXSIZE_PACK64);
...
void PerfAttrsBuffer::marshalHeaderPage(const uint64_t currTime,
					const char *const headerPage)
{
...
void PerfAttrsBuffer::marshalHeaderEvent(const uint64_t currTime,
					 const char *const headerEvent)
{
