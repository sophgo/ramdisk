...
PerfGroups::PerfGroups(const PerfConfig &perfConfig, size_t dataBufferLength,
		       size_t auxBufferLength, int backtraceDepth,
		       int sampleRate, bool enablePeriodicSampling,
		       lib::Span<const GatorCpu> clusters,
		       lib::Span<const int> clusterIds, int64_t schedSwitchId)
	: sharedConfig(perfConfig, dataBufferLength, auxBufferLength,
		       backtraceDepth, sampleRate, enablePeriodicSampling,
		       clusters, clusterIds, schedSwitchId),
	  perfEventGroupMap()
{
}

PerfEventGroup &
PerfGroups::getGroup(const uint64_t timestamp,
		     IPerfAttrsConsumer &attrsConsumer,
		     const PerfEventGroupIdentifier &groupIdentifier)
{
	// find the actual group object
	std::unique_ptr<PerfEventGroup> &eventGroupPtr =
		perfEventGroupMap[groupIdentifier];
	if (!eventGroupPtr) {
		eventGroupPtr.reset(
			new PerfEventGroup(groupIdentifier, sharedConfig));
	}
...
bool PerfGroups::add(const uint64_t timestamp,
		     IPerfAttrsConsumer &attrsConsumer,
		     const PerfEventGroupIdentifier &groupIdentifier,
		     const int key, const IPerfGroups::Attr &attr,
		     bool hasAuxData)
{
	PerfEventGroup &eventGroup =
		getGroup(timestamp, attrsConsumer, groupIdentifier);
	logg.logMessage(
		"Adding event: timestamp=%" PRIu64
		", group='%s', key=%i, type=%" PRIu32 ", config=%" PRIu64
		", config1=%" PRIu64 ", config2=%" PRIu64 ", period=%" PRIu64
		", sampleType=0x%" PRIx64
		", mmap=%d, comm=%d, freq=%d, task=%d, context_switch=%d, hasAuxData=%d",
		timestamp, std::string(groupIdentifier).c_str(), key, attr.type,
		attr.config, attr.config1, attr.config2, attr.periodOrFreq,
		attr.sampleType, attr.mmap, attr.comm, attr.freq, attr.task,
		attr.context_switch, hasAuxData);

	IPerfGroups::Attr newAttr{ attr };

	// Collect EBS samples
	if (attr.periodOrFreq != 0) {
		newAttr.sampleType |= PERF_SAMPLE_TID | PERF_SAMPLE_IP |
				      (sharedConfig.backtraceDepth > 0 ?
	...
	return eventGroup.addEvent(false, timestamp, attrsConsumer, key,
				   newAttr, hasAuxData);
}

std::pair<OnlineResult, std::string>
PerfGroups::onlineCPU(uint64_t timestamp, int cpu, const std::set<int> &appPids,
		      ...
		      std::function<std::set<int>(int)> childTids)
{
	...
	for (auto &pair : perfEventGroupMap) {
		const auto result =
			pair.second->onlineCPU(timestamp, cpu, tids,
					       enabledState, attrsConsumer,
					       addToMonitor, addToBuffer);
...
{
	for (auto &pair : perfEventGroupMap) {
		if (pair.first.getType() ==
		    PerfEventGroupIdentifier::Type::SPE) {
			return true;
		}
	}
