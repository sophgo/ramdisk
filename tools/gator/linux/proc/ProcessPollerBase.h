...
class ProcessPollerBase {
	...
	struct IProcessPollerReceiver {
		...
		/**
             ...
             */
		virtual void onThreadDirectory(int pid, int tid,
					       const lib::FsEntry &path);

		/**
             * Called with the contents of stat, statm and the parsed exe path
             */
		virtual void onThreadDetails(
			int pid, int tid,
    ...
    protected:
	void poll(bool wantThreads, bool wantStats,
		  IProcessPollerReceiver &receiver);

    private:
	lib::FsEntry procDir;

	void processPidDirectory(bool wantThreads, bool wantStats,
				 IProcessPollerReceiver &receiver,
				 const lib::FsEntry &entry);
	void processTidDirectory(bool wantStats,
				 IProcessPollerReceiver &receiver, int pid,
				 const lib::FsEntry &entry,
				 const lib::Optional<lib::FsEntry> &exe);
};
} // namespace lnx
