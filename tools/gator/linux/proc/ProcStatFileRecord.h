...
class ProcStatFileRecord {
	...
	struct CpuTime {
		...
		CpuTime(unsigned long cpu_id_,
			const unsigned long long (&times_)[10])
			: CpuTime(cpu_id_, times_[0], times_[1], times_[2],
				  times_[3], times_[4], times_[5], times_[6],
				  times_[7], times_[8], times_[9])
		{
		}

		CpuTime(unsigned long cpu_id_, unsigned long long user_ticks_,
			...
			: cpu_id(cpu_id_), user_ticks(user_ticks_),
			  nice_ticks(nice_ticks_), system_ticks(system_ticks_),
			  idle_ticks(idle_ticks_), iowait_ticks(iowait_ticks_),
			  irq_ticks(irq_ticks_), softirq_ticks(softirq_ticks_),
			  steal_ticks(steal_ticks_), guest_ticks(guest_ticks_),
