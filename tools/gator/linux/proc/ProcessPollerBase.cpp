...
namespace
{
	...
	/**
         ...
         */
	static lib::Optional<lib::FsEntry>
	getProcessCmdlineExePath(const lib::FsEntry &entry)
	{
		const lib::FsEntry cmdline_file =
			lib::FsEntry::create(entry, "cmdline");
		const std::string cmdline_contents =
			lib::readFileContents(cmdline_file);
	...
	/**
         ...
         */
	static lib::Optional<lib::FsEntry>
	getProcessExePath(const lib::FsEntry &entry)
	{
		const lib::FsEntry exe_file =
			lib::FsEntry::create(entry, "exe");
		...
		if (exe_path) {
			...
			if ((name == "app_process") ||
				...
				const lib::Optional<lib::FsEntry> cmdline_exe =
					getProcessCmdlineExePath(entry);
				if (cmdline_exe) {
					return cmdline_exe;
				}
			}
...
void ProcessPollerBase::IProcessPollerReceiver::onProcessDirectory(
	int, const lib::FsEntry &)
{
}

void ProcessPollerBase::IProcessPollerReceiver::onThreadDirectory(
	int, int, const lib::FsEntry &)
{
}

void ProcessPollerBase::IProcessPollerReceiver::onThreadDetails(
	int, int, const ProcPidStatFileRecord &,
...
void ProcessPollerBase::poll(bool wantThreads, bool wantStats,
			     IProcessPollerReceiver &receiver)
{
	...
	while (lib::Optional<lib::FsEntry> entry = iterator.next()) {
		if (isPidDirectory(*entry)) {
			processPidDirectory(wantThreads, wantStats, receiver,
					    *entry);
		}
	}
}

void ProcessPollerBase::processPidDirectory(bool wantThreads, bool wantStats,
					    IProcessPollerReceiver &receiver,
					    const lib::FsEntry &entry)
{
	...
	if (wantThreads || wantStats) {
		...
		const lib::FsEntry task_directory =
			lib::FsEntry::create(entry, "task");

		// the /proc/[PID]/task/[PID]/ directory
		const lib::FsEntry task_pid_directory =
			lib::FsEntry::create(task_directory, name);
		const lib::FsEntry::Stats task_pid_directory_stats =
			task_pid_directory.read_stats();
		...
		if ((!task_pid_directory_stats.exists()) ||
		    (task_pid_directory_stats.type() !=
		     lib::FsEntry::Type::DIR)) {
			processTidDirectory(wantStats, receiver, pid, entry,
					    exe_path);
		}

		// scan all the TIDs in the task directory
		lib::FsEntryDirectoryIterator task_iterator =
			task_directory.children();

		while (lib::Optional<lib::FsEntry> task_entry =
			       task_iterator.next()) {
			if (isPidDirectory(*task_entry)) {
				processTidDirectory(wantStats, receiver, pid,
						    *task_entry, exe_path);
			}
		}
	}
...
void ProcessPollerBase::processTidDirectory(
	bool wantStats, IProcessPollerReceiver &receiver, const int pid,
	const lib::FsEntry &entry, const lib::Optional<lib::FsEntry> &exe)
{
	...
	// process stats?
	if (wantStats) {
		lib::Optional<ProcPidStatmFileRecord> statm_file_record{
			ProcPidStatmFileRecord()
		};

		// open /proc/[PID]/statm
		{
			const lib::FsEntry statm_file =
				lib::FsEntry::create(entry, "statm");
			const lib::FsEntry::Stats statm_file_stats =
				statm_file.read_stats();

			if (statm_file_stats.exists() &&
			    statm_file_stats.type() ==
				    lib::FsEntry::Type::FILE) {
				const std::string statm_file_contents =
					lib::readFileContents(statm_file);

				if (!ProcPidStatmFileRecord::parseStatmFile(
					    *statm_file_record,
		...
		// open /proc/[PID]/stat
		{
			const lib::FsEntry stat_file =
				lib::FsEntry::create(entry, "stat");
			const lib::FsEntry::Stats stat_file_stats =
				stat_file.read_stats();

			if (stat_file_stats.exists() &&
			    stat_file_stats.type() ==
				    lib::FsEntry::Type::FILE) {
				const std::string stat_file_contents =
					lib::readFileContents(stat_file);
				ProcPidStatFileRecord stat_file_record;
				if (ProcPidStatFileRecord::parseStatFile(
					    stat_file_record,
					    stat_file_contents.c_str())) {
					receiver.onThreadDetails(
						pid, tid, stat_file_record,
						statm_file_record, exe);
				}
			}
		}
	}
