...
namespace
{
	...
	/**
         ...
         */
	static bool matchToken(const char *string, const unsigned from,
			       const unsigned to, const char *against,
			       const bool full)
	{
		unsigned apos = 0;
		unsigned spos = from;
	...
	/**
         ...
         */
	static unsigned decodeUnsignedLong(unsigned long &result,
					   const char *string,
					   const unsigned from)
	{
		result = 0;
					  ...
					  const unsigned from)
	{
		...
		const unsigned pos = decodeUnsignedLong(
			value, string, skipSpaces(string, from));
	...
	/**
         ...
         * @return As per skipLine
         */
	static unsigned parsePagingCounts(
		lib::Optional<ProcStatFileRecord::PagingCounts> &result,
		const char *string, const unsigned from)
	{
		// decode two values
		unsigned long in = 0;
		unsigned long out = 0;
		...
		// decode first
		const unsigned pos_in = decodeUnsignedLong(
			in, string, skipSpaces(string, from));

		// decode second
		if ((pos_in > from) && (string[pos_in] == ' ')) {
			const unsigned pos_out =
				decodeUnsignedLong(out, string, pos_in + 1);

			if (pos_out > (pos_in + 1)) {
				result.set(ProcStatFileRecord::PagingCounts(
					in, out));
			} else {
				result.clear();
			}
	...
	/**
         ...
         */
	template <unsigned N>
	static unsigned decodeCpuTimes(const char *string, const unsigned from,
				       ...
				       unsigned &out_num_decoded)
	{
		...
		for (unsigned index = 0; index < N; ++index) {
			...
			const unsigned pos =
				decodeUnsignedLong(time_val, string, last_pos);
	...
	/**
         ...
         */
	static unsigned
	parseCpuTime(std::vector<ProcStatFileRecord::CpuTime> &cpus,
		     const char *string, unsigned identifier_from,
		     unsigned token_end)
	{
		...
		if ((identifier_from + 1) < token_end) {
			const unsigned pos = decodeUnsignedLong(
				cpu_id, string, identifier_from);
			runtime_assert((pos + 1) == token_end,
				       lib::Format()
					       << "Unexpected value for 'pos': "
					       << pos << ", expected "
					       << token_end);
		} else {
		...
		const unsigned pos =
			decodeCpuTimes(string, skipSpaces(string, token_end),
				       times, num_decoded);
			...
			if (stat_contents[next_break] != ' ') {
				...
				case 'c': {
					if (matchToken(stat_contents,
						       ...
						       next_break, "pu",
						       false)) {
						current_offset = parseCpuTime(
							cpus, stat_contents,
							current_offset + 3,
							next_break + 1);
					} else if (matchToken(stat_contents,
							      current_offset +
								      1,
							      next_break, "txt",
							      true)) {
				...
				// all others
				default: {
					current_offset = skipLine(stat_contents,
								  next_break);
					break;
				}
				}
...
ProcStatFileRecord::ProcStatFileRecord()
	: cpus(), page(), swap(), intr(), soft_irq(), ctxt(), btime(),
	  processes(), procs_running(), procs_blocked()
{
}

ProcStatFileRecord::ProcStatFileRecord(
	std::vector<CpuTime> &&cpus_, lib::Optional<PagingCounts> &&page_,
	...
	: cpus(std::move(cpus_)), page(std::move(page_)),
	  swap(std::move(swap_)), intr(std::move(intr_)),
	  soft_irq(std::move(soft_irq_)), ctxt(std::move(ctxt_)),
	  btime(std::move(btime_)), processes(std::move(processes_)),
