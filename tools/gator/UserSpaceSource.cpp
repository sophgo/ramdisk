...
UserSpaceSource::UserSpaceSource(
	...
	: Source(child),
	  mBuffer(0, FrameType::BLOCK_COUNTER,
		  gSessionData.mTotalBufferSize * 1024 * 1024, senderSem),
	  mGetMonotonicStarted(getMonotonicStarted), mDrivers(drivers)
{
}
...
{
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-counters"),
	      0, 0, 0);
	...
	while (gSessionData.mSessionIsActive) {
		...
		if (nextTime < currTime) {
			logg.logMessage("Too slow, currTime: %" PRIi64
					" nextTime: %" PRIi64,
					currTime, nextTime);
			nextTime = currTime;
		}
