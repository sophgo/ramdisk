...
SharedData::SharedData()
	: mMaliUtgardCountersSize(0), mMaliUtgardCounters(),
	  mMaliMidgardCountersSize(0), mMaliMidgardCounters()
{
}

SessionData::SessionData()
	: mSharedData(), mImages(), mConfigurationXMLPath(), mSessionXMLPath(),
	  mEventsXMLPath(), mEventsXMLAppend(), mTargetPath(), mAPCDir(),
	  mCaptureWorkingDir(), mCaptureCommand(), mCaptureUser(),
	  mWaitForProcessCommand(), mPids(), mStopOnExit(), mWaitingOnCommand(),
	  mSessionIsActive(), mLocalCapture(), mOneShot(), mIsEBS(),
	  mSentSummary(), mAllowCommands(), mFtraceRaw(), mSystemWide(),
	  mAndroidApiLevel(), mMonotonicStarted(), mBacktraceDepth(),
	  mTotalBufferSize(), mSampleRate(), mLiveRate(), mDuration(),
	  mPageSize(), mAnnotateStart(), parameterSetFlag(),
	  mPerfMmapSizeInPages(), mCounters(), globalCounterToEventMap()
{
}
...
void SessionData::parseSessionXML(char *xmlString)
{
	...
	if ((gSessionData.parameterSetFlag & USE_CMDLINE_ARG_SAMPLE_RATE) ==
		...
		} else if (strcmp(session.parameters.sample_rate, "normal") ==
			   0) {
			mSampleRate = 1009; // 1000
		} else if (strcmp(session.parameters.sample_rate, "low") == 0) {
			mSampleRate = 101; // 100
		} else if (strcmp(session.parameters.sample_rate, "none") ==
			   0) {
			mSampleRate = 0;
		} else {
			logg.logError(
				"Invalid sample rate (%s) in session xml.",
				session.parameters.sample_rate);
			handleException();
		}
	}
	if ((gSessionData.parameterSetFlag &
		...
		mBacktraceDepth =
			session.parameters.call_stack_unwinding == true ? 128 :
									  0;
	}
	...
	if (mLiveRate > 0 && mLocalCapture) {
		logg.logMessage(
			"Local capture is not compatable with live, disabling live");
		mLiveRate = 0;
	}
	if ((!mSystemWide) && (mWaitForProcessCommand == nullptr) &&
	    mCaptureCommand.empty() && mPids.empty()) {
		logg.logError(
			"No command specified in Capture & Analysis Options.");
		handleException();
	}

	if ((!mAllowCommands) && (!mCaptureCommand.empty()) &&
	    ((gSessionData.parameterSetFlag &
