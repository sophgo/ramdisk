...
StreamlineSetup::StreamlineSetup(OlySocket *s, Drivers &drivers,
	...
	: mSocket(s), mDrivers(drivers), mCapturedSpes(capturedSpes)
{
	...
	while (!ready) {
		...
		case COMMAND_APC_STOP:
			logg.logMessage(
				"Received apc stop request before apc start request");
			exit(0);
			break;
		...
		case COMMAND_PING:
			...
			break;
		default:
			logg.logError("Target error: Unknown command type, %d",
				      type);
			handleException();
		}
	}
...
std::vector<char> StreamlineSetup::readCommand(int *command)
{
	...
	response = mSocket->receiveNBytes(reinterpret_cast<char *>(&header),
					  sizeof(header));
	...
	if ((length < 0) || length > 1024 * 1024) {
		logg.logError("Target error: Invalid length received, %d",
			      length);
		handleException();
	}
...
void StreamlineSetup::handleRequest(char *xml)
{
	...
	node = mxmlFindElement(tree, tree, TAG_REQUEST, ATTR_TYPE, NULL,
			       MXML_DESCEND_FIRST);
	if (node) {
	...
	if (attr && strcmp(attr, VALUE_EVENTS) == 0) {
		const auto xml =
			events_xml::getXML(mDrivers.getAllConst(),
		...
		const auto &xml = configuration_xml::getConfigurationXML(
			mDrivers.getPrimarySourceProvider()
				.getCpuInfo()
				.getClusters());
		...
		const auto xml = counters_xml::getXML(
			mDrivers.getPrimarySourceProvider().supportsMultiEbs(),
			mDrivers.getAllConst(),
			mDrivers.getPrimarySourceProvider().getCpuInfo());
		sendString(xml.get(), ResponseType::XML);
		logg.logMessage("Sent counters xml response");
	} else if (attr && strcmp(attr, VALUE_CAPTURED) == 0) {
		const auto xml = captured_xml::getXML(
			false, mCapturedSpes,
			mDrivers.getPrimarySourceProvider(),
...
void StreamlineSetup::handleDeliver(char *xml)
{
	...
	// Determine xml type
	tree = mxmlLoadString(NULL, xml, MXML_NO_CALLBACK);
	if (mxmlFindElement(tree, tree, TAG_SESSION, NULL, NULL,
			    MXML_DESCEND_FIRST)) {
		// Session XML
		gSessionData.parseSessionXML(xml);
		sendData(NULL, 0, ResponseType::ACK);
		logg.logMessage("Received session xml");
	} else if (mxmlFindElement(tree, tree, TAG_CONFIGURATIONS, NULL, NULL,
				   MXML_DESCEND_FIRST)) {
		// Configuration XML
...
void StreamlineSetup::sendData(const char *data, uint32_t length,
			       ResponseType type)
{
	char header[5];
...
void StreamlineSetup::sendDefaults()
{
	...
	auto xml = configuration_xml::getDefaultConfigurationXml(
...
void StreamlineSetup::writeConfiguration(char *xml)
{
	...
	if (writeToDisk(path, xml) < 0) {
		logg.logError(
			"Error writing %s\nPlease verify write permissions to this path.",
			path);
		handleException();
	}
	...
	for (auto &&counter : result.speConfigurations) {
		checkError(configuration_xml::addSpeToSet(speConfigs,
							  std::move(counter)));
	}

	checkError(configuration_xml::setCounters(counterConfigs,
						  !result.isDefault, mDrivers));
}
