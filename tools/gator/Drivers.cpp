...
createPrimarySourceProvider(const char *module, bool systemWide,
			    PmuXML &&pmuXml, const char *maliFamilyName)
{
	std::unique_ptr<PrimarySourceProvider> primarySourceProvider =
		PrimarySourceProvider::detect(
			module, systemWide, std::move(pmuXml), maliFamilyName);
...
Drivers::Drivers(const char *module, bool systemWide, PmuXML &&pmuXml)
	: mMaliHwCntrs{}, mPrimarySourceProvider{ createPrimarySourceProvider(
				  module, systemWide, std::move(pmuXml),
	  ...
	  mAtraceDriver{ mFtraceDriver }, mTtraceDriver{ mFtraceDriver },
	  mExternalDriver{}, mCcnDriver{}, all{}, allPolled{}
{
	all.push_back(&mPrimarySourceProvider->getPrimaryDriver());
	for (PolledDriver *driver :
	     mPrimarySourceProvider->getAdditionalPolledDrivers()) {
