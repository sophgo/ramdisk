...
class MaliHwCntr : public DriverCounter {
    public:
	MaliHwCntr(DriverCounter *next, const char *name,
		   int32_t nameBlockIndex, int32_t counterIndex, uint32_t gpuId)
		: DriverCounter(next, name), mNameBlockIndex(nameBlockIndex),
		  mCounterIndex(counterIndex), mGpuId(gpuId)
	{
	}
