...
MaliHwCntrTask::MaliHwCntrTask(std::function<void()> endSession_,
	...
	: mBuffer(std::move(buffer_)),
	  mGetMonotonicStarted(getMonotonicStarted), mCallback(callback_),
	  endSession(endSession_), isSessionActive(isSessionActive_),
	  mReader(reader)
{
}
...
void MaliHwCntrTask::execute(int sampleRate, bool isOneShot)
{
	...
	const uint32_t sampleIntervalNs =
		(sampleRate > 0 ?
			 (sampleRate < 1000000000 ? (1000000000u / sampleRate) :
						    1u) :
			 10000000u);
	...
	const MaliDeviceCounterList countersList(
		mReader.getDevice().createCounterList(mCallback));
	while (isSessionActive() && !terminated) {
		...
		case WAIT_STATUS_SUCCESS: {
			if (waitStatus.data) {
				const uint64_t sampleTime =
					waitStatus.timestamp - monotonicStarted;
		...
		if (isOneShot && isSessionActive() &&
		    (mBuffer->bytesAvailable() <= 0)) {
