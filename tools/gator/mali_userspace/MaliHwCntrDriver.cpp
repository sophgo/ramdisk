...
MaliHwCntrDriver::MaliHwCntrDriver()
	: SimpleDriver("MaliHwCntrDriver"), mEnabledCounterKeysByGpuId(),
	  mPolledDrivers(),
	  mDevices(mali_userspace::enumerateAllMaliHwCntrDrivers())
{
	...
	for (auto const &device : mDevices) {
		...
		if (std::find(addedGpuIds.begin(), addedGpuIds.end(), gpuId) ==
		    addedGpuIds
			    .end()) { // finds if counter already created for gpu
			// add all the device counters
			const uint32_t numNameBlocks =
				t_device.getNameBlockCount();

			// allocate the enable map
			const uint32_t enabledMapLength =
				(numNameBlocks *
				 MaliDevice::NUM_COUNTERS_PER_BLOCK);
			std::unique_ptr<int[]> mEnabledCounterKeys(
				new int[enabledMapLength]{});

			for (uint32_t nameBlockIndex = 0;
			     nameBlockIndex < numNameBlocks; ++nameBlockIndex) {
				for (uint32_t counterIndex = 0;
					...
					logg.logMessage(
						...
						name, nameBlockIndex,
						counterIndex);
					setCounters(new MaliHwCntr(
						getCounters(), name,
						nameBlockIndex, counterIndex,
						gpuId));
					::free(name);
				}
			}
			mEnabledCounterKeysByGpuId[gpuId] =
				std::move(mEnabledCounterKeys);
	...
	for (auto const &mDevice : mDevices) {
		...
		if (!device.getClockPath().empty()) {
			mPolledDrivers[mDevice.first] =
				std::unique_ptr<PolledDriver>(
					new MaliGPUClockPolledDriver(
						device.getClockPath()));
		} else {
			logg.logSetup(
				"Mali GPU counters\nGPU frequency counters not available for GPU # %d.",
				mDevice.first);
		}
	}
}
...
void MaliHwCntrDriver::resetCounters()
{
	for (auto const &device : mDevices) {
		const uint32_t numNameBlocks =
			device.second->getNameBlockCount();
		const uint32_t enabledMapLength =
			(numNameBlocks * MaliDevice::NUM_COUNTERS_PER_BLOCK);
		for (auto &counterKeys : mEnabledCounterKeysByGpuId) {
			std::unique_ptr<int[]> &counterKey = counterKeys.second;
			std::fill(counterKey.get(),
				  counterKey.get() + enabledMapLength, 0);
		}
	}
	super::resetCounters();
}
...
void MaliHwCntrDriver::setupCounter(Counter &counter)
{
	...
	const int32_t index = (malihwcCounter->getNameBlockIndex() *
				       MaliDevice::NUM_COUNTERS_PER_BLOCK +
			       malihwcCounter->getCounterIndex());
	if (mEnabledCounterKeysByGpuId.find(gpuId) !=
	    mEnabledCounterKeysByGpuId.end()) {
		mEnabledCounterKeysByGpuId.find(gpuId)->second[index] =
			malihwcCounter->getKey();
	}
	malihwcCounter->setEnabled(true);
	counter.setKey(malihwcCounter->getKey());
}

int MaliHwCntrDriver::getCounterKey(uint32_t nameBlockIndex,
				    uint32_t counterIndex, uint32_t gpuId) const
{
	if (counterIndex < MaliDevice::NUM_COUNTERS_PER_BLOCK) {
		const uint32_t index =
			(nameBlockIndex * MaliDevice::NUM_COUNTERS_PER_BLOCK +
			 counterIndex);
...
const char *MaliHwCntrDriver::getSupportedDeviceFamilyName() const
{
	if (!mDevices.empty()) {
		...
		const char *supportedDevice =
			mDevices.begin()->second->getSupportedDeviceFamilyName();
