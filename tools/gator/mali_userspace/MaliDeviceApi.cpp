...
static void logDetectedMaliDevice(const char *maliDevicePath,
				  uint32_t productId, uint32_t major,
				  uint32_t minor, uint32_t frequency,
				  uint32_t l2Slices, uint32_t shaderCores)
{
	...
	if (productName != nullptr) {
		formatter << " as Mali-" << productName << " (0x" << std::hex
			  << productId << std::dec << " r" << major << "p"
			  << minor << ")";
		if (frequency > 0) {
			formatter << " clocked at " << frequency << "MHz";
		}
		formatter << ", " << l2Slices << " L2 Slices, " << shaderCores
			  << " Shader Cores.";
	} else {
		formatter
			<< " but it is not recognized (id: 0x" << std::hex
			<< productId << std::dec << " r" << major << "p"
			<< minor << ", " << l2Slices << " L2 slices, "
			<< shaderCores
...
{
	template <typename T> static int doMaliIoctl(int fd, T &arg)
	{
		union kbase_uk_header *hdr = &arg.header;
		const int cmd = _IOC(_IOC_READ | _IOC_WRITE,
				     LINUX_UK_BASE_MAGIC, hdr->id, sizeof(T));
	...
	class MaliDeviceApi final : public IMaliDeviceApi {
	    public:
		MaliDeviceApi(const char *maliDevicePath,
			      lib::AutoClosingFd devFd,
			      const kbase_uk_gpuprops &props)
			: devFd(std::move(devFd)),
			  ...
			  hwVersion((uint32_t(props.props.core_props
						      .major_revision)
				     << 16) |
				    props.props.core_props.minor_revision)
		{
			logDetectedMaliDevice(
				maliDevicePath,
		...
		virtual lib::AutoClosingFd
		createHwCntReaderFd(size_t bufferCount, uint32_t jmBitmask,
		...
		{
			logg.logMessage(
				"MaliDeviceApi: create (%zu, 0x%x, 0x%x, 0x%x, 0x%x)",
				bufferCount, jmBitmask, shaderBitmask,
				tilerBitmask, mmuL2Bitmask);
			...
			if (doMaliIoctl(*devFd, setup_args) != 0) {
				if (setup_args.header.ret != 0) {
					logg.logMessage(
						"MaliDeviceApi: Failed sending hwcnt reader ioctl. fd=%i ret = %lu",
						*devFd,
						static_cast<unsigned long>(
							setup_args.header.ret));
				} else {
					logg.logMessage(
		...
		virtual uint32_t getNumberOfShaderCores() const override
		{
			return numberOfShaderCores;
		}

		virtual uint32_t getNumberOfL2Slices() const override
		{
			return numberOfL2Slices;
		}

		virtual uint32_t getGpuId() const override
		{
			return gpuId;
		}

		virtual uint32_t getHwVersion() const override
		{
			return hwVersion;
		}
		...
		static uint32_t calcNumShaders(const kbase_uk_gpuprops &props)
		{
			...
			for (uint32_t i = 0;
			     i < props.props.coherency_info.num_core_groups;
			     i++)
				core_mask |= props.props.coherency_info.group[i]
						     .core_mask;
	...
	std::unique_ptr<IMaliDeviceApi> probe(const char *maliDevicePath,
					      lib::AutoClosingFd devFd)
	{
		// get & check API version
		{
			...
			if (doMaliIoctl(*devFd, version_check) != 0) {
				logg.logMessage(
					"MaliDeviceApi: Failed setting ABI version ioctl - may be r21p0 or later...");
				return {};
			} else if (version_check.major < 10) {
				logg.logMessage(
					"MaliDeviceApi: Unsupported ABI version %u.%u",
					version_check.major,
					version_check.minor);
				return {};
			}

			logg.logMessage("MaliDeviceApi: ABI version: %u.%u",
					version_check.major,
					version_check.minor);
		}
		...
		// set the flags / create context
		{
			...
			if (doMaliIoctl(*devFd, flags) != 0) {
				logg.logMessage(
					"MaliDeviceApi: Failed setting flags ioctl");
				return {};
			}
		}
		...
		// probe properties
		{
			...
			if (doMaliIoctl(*devFd, props) != 0) {
				logg.logMessage(
					"MaliDeviceApi: Failed getting props from ioctl");
				return {};
			}

			return std::unique_ptr<IMaliDeviceApi>{
				new MaliDeviceApi(maliDevicePath,
						  std::move(devFd), props)
			};
		}
	}
} // namespace ddk_pre_r21
...
namespace ddk_post_r21
{
	enum {
		...
		KBASE_IOCTL_VERSION_CHECK = MALI_IOWR(
			KBASE_IOCTL_TYPE, 0, struct kbase_ioctl_version_check),
		KBASE_IOCTL_SET_FLAGS = MALI_IOW(KBASE_IOCTL_TYPE, 1,
						 struct kbase_ioctl_set_flags),
		KBASE_IOCTL_GET_GPUPROPS = MALI_IOW(
			KBASE_IOCTL_TYPE, 3, struct kbase_ioctl_get_gpuprops),
		KBASE_IOCTL_HWCNT_READER_SETUP =
			MALI_IOW(KBASE_IOCTL_TYPE, 8,
	...
	{
		runtime_assert(pos < size,
			       "Buffer overflow reading GPU properties");
	...
	{
		runtime_assert((pos + 2) <= size,
			       "Buffer overflow reading GPU properties");

		const uint16_t result =
			buffer[pos] | (uint16_t(buffer[pos + 1]) << 8);
		pos += 2;
		return result;
	}
	...
	{
		runtime_assert((pos + 4) <= size,
			       "Buffer overflow reading GPU properties");

		const uint32_t result = buffer[pos] |
	...
	{
		runtime_assert((pos + 8) <= size,
			       "Buffer overflow reading GPU properties");

		const uint64_t result = buffer[pos] |
					(uint64_t(buffer[pos + 1]) << 8) |
					(uint64_t(buffer[pos + 2]) << 16) |
					(uint64_t(buffer[pos + 3]) << 24) |
					(uint64_t(buffer[pos + 4]) << 32) |
					(uint64_t(buffer[pos + 5]) << 40) |
					(uint64_t(buffer[pos + 6]) << 48) |
	...
	static inline uint64_t readValue(KBaseGpuPropValueSize value_size,
					 uint8_t *buffer, int &pos, int size)
	{
		switch (value_size) {
	...
	static gpu_propeties decodeProperties(uint8_t *buffer, int size)
	{
		...
		for (int pos = 0; pos < size;) {
			...
			const KBaseGpuPropValueSize value_type =
				KBaseGpuPropValueSize(token & 3);
			const uint64_t value =
				readValue(value_type, buffer, pos, size);
	...
	class MaliDeviceApi final : public IMaliDeviceApi {
		...
		MaliDeviceApi(const char *maliDevicePath,
			      lib::AutoClosingFd devFd,
			      const gpu_propeties &props)
			: devFd(std::move(devFd)),
			  numberOfShaderCores(calcNumShaders(props)),
			  numberOfL2Slices(props.num_l2_slices),
			  gpuId(props.product_id),
			  hwVersion((uint32_t(props.major_revision) << 16) |
				    props.minor_revision)
		{
			logDetectedMaliDevice(maliDevicePath, props.product_id,
					      props.major_revision,
					      props.minor_revision, 0,
					      props.num_l2_slices,
					      numberOfShaderCores);
		}
				    ...
				    bool &failedDueToBufferCount) override
		{
			...
			const int hwcntReaderFd = lib::ioctl(
				*devFd, KBASE_IOCTL_HWCNT_READER_SETUP,
				reinterpret_cast<unsigned long>(&setup_args));
			if (hwcntReaderFd < 0) {
				logg.logMessage(
					"MaliDeviceApi: Failed sending hwcnt reader ioctl");
				failedDueToBufferCount = true;
					      ...
					      lib::AutoClosingFd devFd)
	{
		// get & check API version
		{
			...
			if (lib::ioctl(*devFd, KBASE_IOCTL_VERSION_CHECK,
				       reinterpret_cast<unsigned long>(
					       &version_check))) {
				logg.logMessage(
					"MaliDeviceApi: Failed setting ABI version ioctl");
				return {};
		...
		// set the flags
		{
			...
			if (lib::ioctl(
				    *devFd, KBASE_IOCTL_SET_FLAGS,
				    reinterpret_cast<unsigned long>(&flags))) {
				logg.logMessage(
					"MaliDeviceApi: Failed setting flags ioctl");
				return {};
			}
		...
		// read the GPU properties
		{
			...
			int size = lib::ioctl(
				*devFd, KBASE_IOCTL_GET_GPUPROPS,
				reinterpret_cast<unsigned long>(&get_props));
			if (size < 0) {
				logg.logMessage(
					"MaliDeviceApi: Failed getting properties ioctl (1)");
				return {};
			}
			...
			if (size < 0) {
				logg.logMessage(
					"MaliDeviceApi: Failed getting properties ioctl (2)");
				return {};
			}

			// decode the properties data
			{
				const gpu_propeties properties =
					decodeProperties(buffer.get(), size);
				return std::unique_ptr<IMaliDeviceApi>{
					new MaliDeviceApi(maliDevicePath,
							  std::move(devFd),
							  properties)
...
{
	{
		lib::AutoClosingFd devFd{ lib::open(
			devMaliPath, O_RDWR | O_CLOEXEC | O_NONBLOCK) };
		if (!devFd) {
			if (errno != ENOENT) {
				logg.logMessage(
					"MaliDeviceApi: Failed to open mali device '%s' due to '%s'",
					devMaliPath, strerror(errno));
			}
			return {};
		}

		// try first version
		std::unique_ptr<IMaliDeviceApi> result =
			ddk_pre_r21::probe(devMaliPath, std::move(devFd));
		if (result) {
			return result;
		}
	}

	{
		...
		// try first version
		std::unique_ptr<IMaliDeviceApi> result =
			ddk_post_r21::probe(devMaliPath, std::move(devFd));
		if (result) {
			return result;
		}
	}
