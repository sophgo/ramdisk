...
namespace
{
	...
	static const MaliProductVersion PRODUCT_VERSIONS[] = {
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T60X,
				     "T60x", "Midgard",
				     hardware_counters_mali_t60x, true),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T62X,
				     "T62x", "Midgard",
				     hardware_counters_mali_t62x, true),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T72X,
				     "T72x", "Midgard",
				     hardware_counters_mali_t72x, true),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T76X,
				     "T76x", "Midgard",
				     hardware_counters_mali_t76x, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T82X,
				     "T82x", "Midgard",
				     hardware_counters_mali_t82x, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T83X,
				     "T83x", "Midgard",
				     hardware_counters_mali_t83x, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_T86X,
				     "T86x", "Midgard",
				     hardware_counters_mali_t86x, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_OLD, PRODUCT_ID_TFRX,
				     "T88x", "Midgard",
				     hardware_counters_mali_t88x, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_TMIX,
				     "G71", "Bifrost",
				     hardware_counters_mali_tMIx, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_THEX,
				     "G72", "Bifrost",
				     hardware_counters_mali_tHEx, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_TSIX,
				     "G51", "Bifrost",
				     hardware_counters_mali_tSIx, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_TGOX,
				     "G52", "Bifrost",
				     hardware_counters_mali_tGOx, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_TDVX,
				     "G31", "Bifrost",
				     hardware_counters_mali_tDVx, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_TNOX,
				     "G76", "Bifrost",
				     hardware_counters_mali_tNOx, false),
		MALI_PRODUCT_VERSION(PRODUCT_ID_MASK_NEW, PRODUCT_ID_TTRX,
				     "G77", "Valhall",
				     hardware_counters_mali_tTRx, false)
	};
	...
	mapV4BlockIndexToBlockNumber(MaliCounterBlockName nameBlock,
				     uint32_t index)
	{
		/*
		...
		case MaliCounterBlockName::MMU:
			...
			return 5;
		default:
			runtime_assert(nameBlock ==
					       MaliCounterBlockName::SHADER,
				       "Unexpected name block");
	...
	/**
         ...
         */
	static inline uint32_t
	mapV56BlockIndexToBlockNumber(MaliCounterBlockName nameBlock,
				      uint32_t numL2MmuBlocks,
				      uint32_t numShaderBlocks, uint32_t index)
	{
		/*
             * BLOCKS ARE LAYED OUT AS:
             *
		...
		case MaliCounterBlockName::TILER:
			...
			return 1;
		case MaliCounterBlockName::MMU:
			runtime_assert(index < numL2MmuBlocks,
		...
		default:
			runtime_assert(nameBlock ==
					       MaliCounterBlockName::SHADER,
				       "Unexpected name block");
			runtime_assert(index < numShaderBlocks,
				       "Unexpected block index");
	...
	mapNameBlockToIndex(MaliCounterBlockName nameBlock)
	{
		return uint32_t(nameBlock);
	}

	static const MaliProductVersion *
	findMaliProductRecordFromId(uint32_t productId)
	{
		for (int index = 0; index < NUM_PRODUCT_VERSIONS; ++index) {
			if ((productId & PRODUCT_VERSIONS[index].mGpuIdMask) ==
			    PRODUCT_VERSIONS[index].mGpuIdValue) {
...
MaliDeviceCounterList::MaliDeviceCounterList(uint32_t numBlocks,
					     uint32_t numGroups,
					     uint32_t numWords)
	: countersListLength(numBlocks * numGroups * numWords),
	  countersListValid(0), countersList(new Address[countersListLength])
{
}
...
void MaliDeviceCounterList::enable(MaliCounterBlockName nameBlock,
				   uint32_t repeatCount, uint32_t groupIndex,
				   uint32_t wordIndex)
{
...
std::unique_ptr<MaliDevice>
MaliDevice::create(std::unique_ptr<IMaliDeviceApi> deviceApi,
		   std::string clockPath)
{
	if (deviceApi) {
		...
		if (productRecord != nullptr) {
			return std::unique_ptr<MaliDevice>(new MaliDevice(
				*productRecord, std::move(deviceApi),
				std::move(clockPath)));
		}
	}
	return {};
...
MaliDevice::MaliDevice(const MaliProductVersion &productVersion,
		       ...
		       std::string clockPath)
	: mProductVersion(productVersion), deviceApi(std::move(deviceApi)),
...
{
	return std::max(1u, deviceApi->getNumberOfShaderCores());
}

uint32_t MaliDevice::getL2MmuBlockCount() const
{
	return std::max(1u, deviceApi->getNumberOfL2Slices());
}
...
lib::AutoClosingFd MaliDevice::createHwCntReaderFd(
	std::size_t bufferCount, std::uint32_t jmBitmask,
	std::uint32_t shaderBitmask, std::uint32_t tilerBitmask,
	std::uint32_t mmuL2Bitmask, bool &failedDueToBufferCount) const
{
	return deviceApi->createHwCntReaderFd(bufferCount, jmBitmask,
					      shaderBitmask, tilerBitmask,
					      mmuL2Bitmask,
					      failedDueToBufferCount);
}

const char *MaliDevice::getCounterName(uint32_t nameBlockIndex,
				       uint32_t counterIndex) const
{
	if ((nameBlockIndex >= getNameBlockCount()) ||
	    (counterIndex >= NUM_COUNTERS_PER_BLOCK)) {
		return nullptr;
	}

	const char *result =
		mProductVersion.mCounterNames[(nameBlockIndex *
					       NUM_COUNTERS_PER_BLOCK) +
					      counterIndex];
				 ...
				 uint32_t repeatCount)
{
	...
	for (uint32_t groupIndex = 0;
	     groupIndex < MaliDevice::NUM_ENABLE_GROUPS; ++groupIndex) {
		for (uint32_t wordIndex = 0;
		     wordIndex < MaliDevice::NUM_COUNTERS_PER_ENABLE_GROUP;
		     ++wordIndex) {
			const uint32_t counterIndex =
				(groupIndex *
				 MaliDevice::NUM_COUNTERS_PER_ENABLE_GROUP) +
				wordIndex;
			if (counterIndex !=
			    MaliDevice::BLOCK_ENABLE_BITS_COUNTER_INDEX) {
				if (callback.isCounterActive(nameBlockIndex,
							     counterIndex,
							     gpuId)) {
					list.enable(nameBlock, repeatCount,
...
MaliDevice::createCounterList(IMaliDeviceCounterDumpCallback &callback) const
{
	const uint32_t numL2MmuBlocks = getL2MmuBlockCount();
	const uint32_t numShaderBlocks = getShaderBlockCount();

	MaliDeviceCounterList result(4, NUM_ENABLE_GROUPS,
				     NUM_COUNTERS_PER_ENABLE_GROUP);

	initCounterList(mProductVersion.mGpuIdValue, callback, result,
			MaliCounterBlockName::JM, 1);
	initCounterList(mProductVersion.mGpuIdValue, callback, result,
			MaliCounterBlockName::TILER, 1);
	initCounterList(mProductVersion.mGpuIdValue, callback, result,
			MaliCounterBlockName::MMU, numL2MmuBlocks);
	initCounterList(mProductVersion.mGpuIdValue, callback, result,
			MaliCounterBlockName::SHADER, numShaderBlocks);
...
void MaliDevice::dumpAllCounters(uint32_t hardwareVersion,
				 ...
				 const uint32_t *buffer, size_t bufferLength,
				 IBuffer &bufferData,
				 IMaliDeviceCounterDumpCallback &callback) const
{
	switch (hardwareVersion) {
	case 4: {
		dumpAllCounters_V4(counterList, buffer, bufferLength,
				   bufferData, callback);
		break;
	}
	case 5:
	case 6: {
		dumpAllCounters_V56(counterList, buffer, bufferLength,
				    bufferData, callback);
		break;
	}
	default: {
		...
		if (!shownLog) {
			shownLog = true;
			logg.logError(
				"MaliDevice::dumpAllCounters - Cannot process hardware V%u",
				hardwareVersion);
		}
		break;
	}
...
void MaliDevice::dumpAllCounters_V4(
	const MaliDeviceCounterList &counterList, const uint32_t *buffer,
	size_t bufferLength, IBuffer &bufferData,
	IMaliDeviceCounterDumpCallback &callback) const
{
	...
	for (size_t counterListIndex = 0; counterListIndex < counterListSize;
	     ++counterListIndex) {
		const MaliDeviceCounterList::Address &counterAddress =
			counterList[counterListIndex];

		const MaliCounterBlockName nameBlock = counterAddress.nameBlock;
		const uint32_t nameBlockIndex = mapNameBlockToIndex(nameBlock);
		const bool isShaderCore =
			(nameBlock == MaliCounterBlockName::SHADER);

		for (uint32_t blockIndex = 0;
		     blockIndex < counterAddress.repeatCount; ++blockIndex) {
			const uint32_t blockNumber =
				mapV4BlockIndexToBlockNumber(nameBlock,
							     blockIndex);
			const size_t maskBufferIndex =
				(blockNumber * NUM_COUNTERS_PER_BLOCK) +
				BLOCK_ENABLE_BITS_COUNTER_INDEX;
			...
			if (mask & (1 << counterAddress.groupIndex)) {
				const uint32_t counterIndex =
					(counterAddress.groupIndex *
					 NUM_COUNTERS_PER_ENABLE_GROUP) +
					counterAddress.wordIndex;
				...
				if (counterIndex !=
				    BLOCK_ENABLE_BITS_COUNTER_INDEX) {
					if (isShaderCore) {
						shaderCoreCounters[counterIndex] +=
							delta;
					} else {
						callback.nextCounterValue(
							nameBlockIndex,
							counterIndex, delta,
							mProductVersion
								.mGpuIdValue,
	...
	for (uint32_t shaderCounterIndex = 0;
	     shaderCounterIndex < NUM_COUNTERS_PER_BLOCK;
	     ++shaderCounterIndex) {
		if (shaderCoreCounters[shaderCounterIndex].isValid()) {
			callback.nextCounterValue(
				...
				mProductVersion.mGpuIdValue, bufferData);
		}
	}
}

void MaliDevice::dumpAllCounters_V56(
	const MaliDeviceCounterList &counterList, const uint32_t *buffer,
	size_t bufferLength, IBuffer &bufferData,
	IMaliDeviceCounterDumpCallback &callback) const
{
	...
	for (size_t counterListIndex = 0; counterListIndex < counterListSize;
		...
		for (uint32_t blockIndex = 0;
		     blockIndex < counterAddress.repeatCount; ++blockIndex) {
			const uint32_t blockNumber =
				mapV56BlockIndexToBlockNumber(nameBlock,
							      numL2MmuBlocks,
							      numShaderBlocks,
							      blockIndex);
			const size_t maskBufferIndex =
	...
	// now send shader core and mmu/l2 averages
	for (uint32_t mmuL2CounterIndex = 0;
	     mmuL2CounterIndex < NUM_COUNTERS_PER_BLOCK; ++mmuL2CounterIndex) {
		if (mmuL2Counters[mmuL2CounterIndex].isValid()) {
			callback.nextCounterValue(
				mapNameBlockToIndex(MaliCounterBlockName::MMU),
				mmuL2CounterIndex,
