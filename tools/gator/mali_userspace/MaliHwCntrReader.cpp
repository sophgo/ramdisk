...
namespace
{
	enum {
		...
		KBASE_HWCNT_READER_GET_HWVER =
			MALI_IOR(KBASE_HWCNT_READER, 0x00, uint32_t),
		KBASE_HWCNT_READER_GET_BUFFER_SIZE =
			MALI_IOR(KBASE_HWCNT_READER, 0x01, uint32_t),
		KBASE_HWCNT_READER_DUMP =
			MALI_IOW(KBASE_HWCNT_READER, 0x10, uint32_t),
		KBASE_HWCNT_READER_CLEAR =
			MALI_IOW(KBASE_HWCNT_READER, 0x11, uint32_t),
		KBASE_HWCNT_READER_GET_BUFFER =
			MALI_IOR(KBASE_HWCNT_READER, 0x20,
				 struct kbase_hwcnt_reader_metadata),
		KBASE_HWCNT_READER_PUT_BUFFER =
			MALI_IOW(KBASE_HWCNT_READER, 0x21,
				 struct kbase_hwcnt_reader_metadata),
		KBASE_HWCNT_READER_SET_INTERVAL =
			MALI_IOW(KBASE_HWCNT_READER, 0x30, uint32_t),
		KBASE_HWCNT_READER_ENABLE_EVENT =
			MALI_IOW(KBASE_HWCNT_READER, 0x40, uint32_t),
		KBASE_HWCNT_READER_DISABLE_EVENT =
			MALI_IOW(KBASE_HWCNT_READER, 0x41, uint32_t),
		KBASE_HWCNT_READER_GET_API_VERSION =
			MALI_IOW(KBASE_HWCNT_READER, 0xFF, uint32_t)
	};

	enum { PIPE_DESCRIPTOR_IN, /**< The index of a pipe's input descriptor. */
	...
	enum { POLL_DESCRIPTOR_SIGNAL, /**< The index of the signal descriptor in poll fds array. */
...
MaliHwCntrReader::MaliHwCntrReader(
	const MaliDevice &device, lib::AutoClosingFd hwcntReaderFd,
	lib::AutoClosingFd selfPipe0, lib::AutoClosingFd selfPipe1,
	MmappedBuffer sampleMemory, uint32_t bufferCount,
	uint32_t sampleBufferSize, uint32_t hardwareVersion)

	: device(device), hwcntReaderFd(std::move(hwcntReaderFd)), selfPipe(),
	  sampleMemory(std::move(sampleMemory)), bufferCount(bufferCount),
	  sampleBufferSize(sampleBufferSize), hardwareVersion(hardwareVersion)
{
...
{
	if (lib::ioctl(*hwcntReaderFd, KBASE_HWCNT_READER_SET_INTERVAL,
		       interval) != 0) {
		return false;
	}
...
bool MaliHwCntrReader::configureJobBasedSampled(bool preJob, bool postJob)
{
	if (ioctl(*hwcntReaderFd,
		  (preJob ? KBASE_HWCNT_READER_ENABLE_EVENT :
			    KBASE_HWCNT_READER_DISABLE_EVENT),
	...
	if (ioctl(*hwcntReaderFd,
		  (postJob ? KBASE_HWCNT_READER_ENABLE_EVENT :
			     KBASE_HWCNT_READER_DISABLE_EVENT),
...
SampleBuffer MaliHwCntrReader::waitForBuffer(int timeout)
{
	...
	if (ready < 0) {
		...
		if ((fds[POLL_DESCRIPTOR_SIGNAL].revents & POLLIN) == POLLIN) {
			// read the data
			poll_data_t value;
			int result = lib::read(*selfPipe[PIPE_DESCRIPTOR_IN],
					       &value, sizeof(value));
			if (result < 0) {
				temp.status = WAIT_STATUS_SUCCESS;
				return temp;
		...
		return temp;
	} else if ((fds[POLL_DESCRIPTOR_HWCNT_READER].revents & POLLIN) ==
		   POLLIN) {
		// get the buffer
		kbase_hwcnt_reader_metadata metadata;
		if (lib::ioctl(*hwcntReaderFd, KBASE_HWCNT_READER_GET_BUFFER,
			       reinterpret_cast<unsigned long>(&metadata)) !=
		    0) {
			logg.logError(
				"MaliHwCntrReader: Could not get buffer due to ioctl failure (%s)",
				strerror(errno));
			temp.status = WAIT_STATUS_ERROR;
			return temp;
		}
		...
		unique_ptr_with_deleter<uint8_t> data_temp(
			&(sampleMemory[sampleBufferSize * metadata.buffer_idx]),
			[=](uint8_t *) {
				kbase_hwcnt_reader_metadata metadata_tmp =
					metadata;
		...
		temp.status = WAIT_STATUS_SUCCESS;
		return temp;
	} else if ((fds[POLL_DESCRIPTOR_HWCNT_READER].revents & POLLHUP) ==
		   POLLHUP) {
		// terminated
		temp.status = WAIT_STATUS_TERMINATED;
		return temp;
		...
		// error occurred
		logg.logError(
			"MaliHwCntrReader::waitForBuffer - unexpected event 0x%x",
...
{
	poll_data_t exit = 0;
	if (lib::write(*selfPipe[PIPE_DESCRIPTOR_OUT], &exit, sizeof(exit)) <
	    0) {
		logg.logError("MaliHwCntrReader::interrupt failed (%s)",
			      strerror(errno));
	}
}
...
{
	return lib::ioctl(*hwcntReaderFd, KBASE_HWCNT_READER_PUT_BUFFER,
			  reinterpret_cast<unsigned long>(&metadata)) == 0;
}

std::unique_ptr<MaliHwCntrReader>
MaliHwCntrReader::create(const MaliDevice &device, CounterBitmask jmBitmask,
			 ...
			 CounterBitmask mmuL2Bitmask)
{
	...
	if (shaderBlockCount < 1) {
		logg.logError("MaliHwCntrReader: shaderBlockCount = %u",
			      shaderBlockCount);
		return {};
	}
	...
	for (unsigned bufferCount = 1; bufferCount < 1024; ++bufferCount) {
		...
		// create the reader
		{
			...
			hwcntReaderFd = device.createHwCntReaderFd(
		...
		// verify the API version
		{
			...
			if (lib::ioctl(*hwcntReaderFd,
				...
				logg.logError(
					"MaliHwCntrReader: Invalid API version (%lu)",
		...
		if (lib::ioctl(*hwcntReaderFd, KBASE_HWCNT_READER_GET_HWVER,
			       reinterpret_cast<unsigned long>(
				       &hardwareVersion)) != 0) {
			logg.logError(
		...
		if ((hardwareVersion < 4) || (hardwareVersion > 6)) {
			logg.logError(
				"MaliHwCntrReader: Hardware version %u is not supported",
				hardwareVersion);
			return {};
		}
		...
		if ((hardwareVersion > 4) && (mmuL2BlockCount < 1)) {
			logg.logError(
				"MaliHwCntrReader: Hardware version %u detected, but mmuL2BlockCount = %u",
				hardwareVersion, mmuL2BlockCount);
			return {};
		}

		// mmap the data
		uint8_t *const sampleMemoryPtr = reinterpret_cast<uint8_t *>(
			lib::mmap(nullptr, bufferCount * sampleBufferSize,
				  PROT_READ, MAP_PRIVATE, *hwcntReaderFd, 0));
		if ((sampleMemoryPtr == nullptr) ||
		    (sampleMemoryPtr == reinterpret_cast<uint8_t *>(-1ul))) {
			logg.logMessage(
				"MaliHwCntrReader: Could not mmap sample buffer");
			continue;
		}
		MmappedBuffer sampleMemory{
			sampleMemoryPtr,
			[bufferCount, sampleBufferSize](uint8_t *ptr) -> void {
				if (ptr != nullptr) {
					lib::munmap(ptr,
						    bufferCount *
							    sampleBufferSize);
		...
		lib::AutoClosingFd selfPipe[2];
		{
			...
			if (pipe(selfPipeFd) != 0) {
				logg.logError(
					"MaliHwCntrReader: Could not create pipe (%s)",
					strerror(errno));
				return {};
			}
		...
		logg.logMessage(
			"MaliHwCntrReader: Successfully created reader, with buffer size of %u",
			bufferCount);
		return std::unique_ptr<MaliHwCntrReader>{ new MaliHwCntrReader(
			device, std::move(hwcntReaderFd),
			std::move(selfPipe[0]), std::move(selfPipe[1]),
			std::move(sampleMemory), bufferCount, sampleBufferSize,
			hardwareVersion) };
	}
	return nullptr;
}
...
std::unique_ptr<MaliHwCntrReader>
MaliHwCntrReader::createReader(const MaliDevice &device)
{
