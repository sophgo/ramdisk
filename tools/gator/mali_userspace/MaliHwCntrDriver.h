...
class MaliHwCntrDriver : public SimpleDriver {
	...
	inline const std::map<unsigned, std::unique_ptr<PolledDriver> > &
	getPolledDrivers() const
	{
		return mPolledDrivers;
	}

	inline const std::map<unsigned, std::unique_ptr<MaliDevice> > &
	getDevices() const
	{
		return mDevices;
	}

	int getCounterKey(uint32_t nameBlockIndex, uint32_t counterIndex,
