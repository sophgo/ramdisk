...
MaliHwCntrSource::MaliHwCntrSource(
	Child &child, sem_t *senderSem,
	std::function<std::int64_t()> getMonotonicStarted,
	MaliHwCntrDriver &driver)
	: Source(child), mDriver(driver),
	  mGetMonotonicStarted(getMonotonicStarted), mReaders(), tasks()
{
...
void MaliHwCntrSource::createTasks(sem_t *mSenderSem)
{
	...
	auto funIsSessionActive = [&]() -> bool {
		return gSessionData.mSessionIsActive;
	};
	...
	for (const auto &pair : mDriver.getDevices()) {
		...
		std::unique_ptr<MaliHwCntrReader> reader =
			MaliHwCntrReader::createReader(device);
		if (!reader) {
			logg.logError(
				"Failed to create reader for mali GPU # %d. Please try again, and if this happens repeatedly please try rebooting your device.",
				deviceNumber);
			handleException();
		} else {
			MaliHwCntrReader &readerRef = *reader;
			mReaders[deviceNumber] = std::move(reader);
			std::unique_ptr<Buffer> taskBuffer(new Buffer(
				deviceNumber, FrameType::BLOCK_COUNTER,
				gSessionData.mTotalBufferSize * 1024 * 1024,
				mSenderSem));
			std::unique_ptr<MaliHwCntrTask> task(new MaliHwCntrTask(
				funChildEndSession, funIsSessionActive,
				mGetMonotonicStarted, std::move(taskBuffer),
				*this, readerRef));
...
{
	prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(&"gatord-malihwc"),
	      0, 0, 0);
	...
	std::for_each(threadsCreated.begin(), threadsCreated.end(),
...
void MaliHwCntrSource::nextCounterValue(uint32_t nameBlockIndex,
					uint32_t counterIndex, uint64_t delta,
					uint32_t gpuId, IBuffer &buffer)
{
	const int key =
		mDriver.getCounterKey(nameBlockIndex, counterIndex, gpuId);
	if (key != 0) {
		buffer.event64(key, delta);
	}
}

bool MaliHwCntrSource::isCounterActive(uint32_t nameBlockIndex,
				       uint32_t counterIndex,
				       uint32_t gpuId) const
{
	const int key =
		mDriver.getCounterKey(nameBlockIndex, counterIndex, gpuId);
	return (key != 0);
