...
{
static void
enumerateMaliGpuClockPaths(const lib::FsEntry &currentDirectory,
			   std::map<unsigned int, std::string> &gpuClockPaths)
{
	...
	if (currentDirectory.read_stats().type() != lib::FsEntry::Type::DIR) {
		logg.logMessage("Failed to open '%s'",
	...
	const lib::Optional<lib::FsEntry> dirsParent =
		currentDirectory.parent();
	const lib::Optional<lib::FsEntry> parentClockPath =
		(dirsParent.valid() ?
			 lib::Optional<lib::FsEntry>{ lib::FsEntry::create(
				 dirsParent.get(), "clock") } :
	...
	while ((childEntry = iterator.next()).valid()) {
		...
		if (childStats.type() == lib::FsEntry::Type::DIR) {
			...
			unsigned int id = 0;
			if (dirIsCalledMisc &&
			    sscanf(childEntry->name().c_str(), "mali%u", &id) ==
				    1) {
				...
				lib::FsEntry childClockPath =
					lib::FsEntry::create(*childEntry,
							     "clock");
				if (childClockPath.exists() &&
				    childClockPath.canAccess(true, false,
							     false)) {
					gpuClockPaths[id] =
						childClockPath.path();
				}
				// use ../../clock ?
				else if (parentClockPath.valid() &&
					 parentClockPath->exists() &&
					 parentClockPath->canAccess(true, false,
								    false)) {
					gpuClockPaths[id] =
						parentClockPath->path();
				}
			}
			// try to recursively scan the child directory
			else if (!childStats.is_symlink()) {
				enumerateMaliGpuClockPaths(*childEntry,
							   gpuClockPaths);
			}
		}
	}
...
std::map<unsigned int, std::unique_ptr<MaliDevice> >
enumerateAllMaliHwCntrDrivers()
{
	...
	for (unsigned int i = 0; i < MAX_DEV_MALI_TOO_SCAN_FOR; ++i) {
		...
		std::unique_ptr<IMaliDeviceApi> device =
			IMaliDeviceApi::probe(pathBuffer);
		if (device) {
	...
	if (!detectedDevices.empty()) {
		...
		enumerateMaliGpuClockPaths(lib::FsEntry::create("/sys"),
					   gpuClockPaths);
		...
		for (auto &detectedDevice : detectedDevices) {
			...
			std::unique_ptr<MaliDevice> device = MaliDevice::create(
				std::move(detectedDevice.second),
