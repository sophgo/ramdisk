...
static const char OPTSTRING_SHORT[] =
	"ahvVS:d::p:s:c:e:E:P:m:N:u:r:t:f:x:o:w:C:A:i:Q:Z:X:R:";
...
ParserResult::ParserResult()
	: mSpeConfigs(), mCaptureWorkingDir(), mCaptureCommand(), mPids(),
	  mSessionXMLPath(), mTargetPath(), mConfigurationXMLPath(),
	  mEventsXMLPath(), mEventsXMLAppend(), mWaitForCommand(),
	  mBacktraceDepth(), mSampleRate(), mDuration(), mAndroidApiLevel(),
	  mPerfMmapSizeInPages(-1), mFtraceRaw(), mStopGator(false),
	  mSystemWide(true), mAllowCommands(false), module(nullptr),
	  pmuPath(nullptr), port(DEFAULT_PORT), parameterSetFlag(0), events(),
	  mode(ExecutionMode::DAEMON), printables()
{
}
...
void GatorCLIParser::addCounter(int startpos, int pos, std::string &counters)
{
	...
	if ((eventpos = subStr.find(":")) != std::string::npos) {
		if (!stringToInt(
			    &event,
			    subStr.substr(eventpos + 1, subStr.size()).c_str(),
			    10)) { //check for decimal
			if (!stringToInt(&event,
					 subStr.substr(eventpos + 1,
						       subStr.size())
						 .c_str(),
					 16)) { //check for hex
	...
	while (it != result.events.end()) {
		if (strcasecmp(it->first.c_str(), counterType.c_str()) == 0) {
			logg.logError("Counter already added. %s ",
				      counterType.c_str());
			result.mode = ExecutionMode::EXIT;
			return;
		}
...
int GatorCLIParser::findAndUpdateCmndLineCmnd(int argc, char **argv)
{
	...
	for (int j = 1; j < argc; j++) {
		...
		if (arg.compare(shortAppArg) == 0 ||
		    arg.compare(longAppArg) == 0) {
			found = j;
			break;
		}
	}
		...
		if (argc > found) {
			if (argc > found + 1) {
				for (int i = found + 1; i < argc; i++) {
					result.mCaptureCommand.push_back(
						argv[i]);
				}
			}
			...
			result.parameterSetFlag =
				result.parameterSetFlag |
				USE_CMDLINE_ARG_CAPTURE_COMMAND;
		}
	}
	return found;
}
...
static void split(const std::string &data, char delimiter,
	...
	if (!spe_data.empty()) {
		...
		if (!spe_data[0].empty()) { //check if cpu id provided
			...
			for (const auto &spe_data_it : spe_data) {
				...
				split(spe_data_it, SPE_KEY_VALUE_DELIMITER,
				      spe);
				if (spe.size() ==
				    2) { //should be a key value pair to add
					if (spe[0].compare(
						    SPE_MIN_LATENCY_KEY) == 0) {
						if (!stringToInt(
							    &(data.min_latency),
							    spe[1].c_str(),
							    0)) {
							logg.logError(
								"latency not an integer %s (%s)",
								data.id.c_str(),
								spe[1].c_str());
							result.mode =
								ExecutionMode::
									EXIT;
							return;
						} else if (data.min_latency <
								   0 ||
							   data.min_latency >=
								   4096) {
							logg.logError(
								"Invalid minimum latency for %s (%d)",
								data.id.c_str(),
								data.min_latency);
							result.mode =
								ExecutionMode::
					...
					} else if (spe[0].compare(
							   SPE_EVENTS_KEY) ==
						   0) {
						std::vector<std::string>
							spe_events;
						split(spe[1],
						      SPES_KEY_VALUE_DELIMITER,
						      spe_events);
						for (std::string spe_event :
						     spe_events) {
							int event;
							if (!stringToInt(
								    &event,
								    ...
								    10)) {
								logg.logError(
									"Event filter cannot be a non integer , failed for %s ",
									spe_event
										.c_str());
								result.mode =
									ExecutionMode::
								...
								return;
								    ...
								    event > 63)) {
								logg.logError(
									"Event filter should be a bit position from 0 - 63 , failed for %d ",
									event);
								result.mode =
									ExecutionMode::
										EXIT;
						...
						logg.logError(
							"--spe arguments not in correct format %s ",
							spe_data_it.c_str());
						result.mode =
							ExecutionMode::EXIT;
						return;
...
void GatorCLIParser::parseCLIArguments(int argc, char *argv[],
				       ...
				       const char *gSrcMd5)
{
	...
	opterr = 1;
	int c;
	while ((c = getopt_long(argc, argv, OPTSTRING_SHORT, OPTSTRING_LONG,
				NULL)) != -1) {
		const int optionInt =
			optarg == nullptr ? -1 : parseBoolean(optarg);
		...
		switch (c) {
		case 'N':
			if (!stringToInt(&result.mAndroidApiLevel, optarg,
					 10)) {
		...
		case 'p': //port
			if (strcasecmp(optarg, "uds") == 0) {
				...
				if ((result.port == 8082) ||
				    (result.port == 8083)) {
					logg.logError(
						"Gator can't use port %i, as it already uses ports 8082 and 8083 for annotations. Please select a different port.",
						result.port);
					result.mode = ExecutionMode::EXIT;
					return;
				}
		...
		case 'u': //-call-stack-unwinding
			result.parameterSetFlag =
				result.parameterSetFlag |
				USE_CMDLINE_ARG_CALL_STACK_UNWINDING;
			if (optionInt < 0) {
				logg.logError(
					"Invalid value for --call-stack-unwinding (%s), 'yes' or 'no' expected.",
					optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
			...
			break;
		case 'r': //sample-rate
			result.parameterSetFlag = result.parameterSetFlag |
			...
			if (sampleRate != invalid) {
				...
				logg.logError("Invalid sample rate (%s).",
					      optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
		...
		case 't': //max-duration
			result.parameterSetFlag = result.parameterSetFlag |
						  USE_CMDLINE_ARG_DURATION;

			if (!stringToInt(&result.mDuration, optarg, 10)) {
				logg.logError("Invalid max duration (%s).",
					      optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
		...
		case 'f': //use-efficient-ftrace
			result.parameterSetFlag = result.parameterSetFlag |
						  USE_CMDLINE_ARG_FTRACE_RAW;
			if (optionInt < 0) {
				logg.logError(
					"Invalid value for --use-efficient-ftrace (%s), 'yes' or 'no' expected.",
					optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
		...
		case 'S': //--system-wide
			if (optionInt < 0) {
				logg.logError(
					"Invalid value for --system-wide (%s), 'yes' or 'no' expected.",
					optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
		...
		case 'w': //app-pwd
			result.parameterSetFlag =
				result.parameterSetFlag |
				USE_CMDLINE_ARG_CAPTURE_WORKING_DIR;
		...
		case 'x': //stop on exit
			result.parameterSetFlag = result.parameterSetFlag |
						  USE_CMDLINE_ARG_STOP_GATOR;
			if (optionInt < 0) {
				logg.logError(
					"Invalid value for --stop-on-exit (%s), 'yes' or 'no' expected.",
					optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
		...
		case 'C': //counter
			...
			while ((counterSplitPos =
					value.find(",", startpos + 1)) !=
			       std::string::npos) {
				addCounter(startpos + 1, counterSplitPos,
					   value);
		...
		case 'i': // pid
		{
			...
			std::vector<int> pids =
				lib::parseCommaSeparatedNumbers<int>(stream);
			if (stream.fail() || !stream.eof()) {
				logg.logError(
					"Invalid value for --pid (%s), comma separated and numeric list expected.",
					optarg);
				result.mode = ExecutionMode::EXIT;
				return;
			}
		...
		case 'V':
			logg.logError("%s\nSRC_MD5: %s\nBUILD_ID: %s",
				      version_string, gSrcMd5,
				      STRIFY(GATORD_BUILD_ID));
			result.mode = ExecutionMode::EXIT;
			return;
		case 'Q':
		...
		case 'Z':
			...
			if (!stringToInt(&result.mPerfMmapSizeInPages, optarg,
					 0)) {
				logg.logError(
					"Invalid value for --mmap-pages (%s): not an integer",
					optarg);
				result.mode = ExecutionMode::EXIT;
				result.mPerfMmapSizeInPages = -1;
			} else if (result.mPerfMmapSizeInPages < 1) {
				logg.logError(
					"Invalid value for --mmap-pages (%s): not more than 0",
					optarg);
				result.mode = ExecutionMode::EXIT;
				result.mPerfMmapSizeInPages = -1;
			} else if (((result.mPerfMmapSizeInPages - 1) &
				    result.mPerfMmapSizeInPages) != 0) {
				logg.logError(
					"Invalid value for --mmap-pages (%s): not a power of 2",
					optarg);
				result.mode = ExecutionMode::EXIT;
				result.mPerfMmapSizeInPages = -1;
			}
		...
		case 'R': {
			...
			for (const auto &part : parts) {
				if (strcasecmp(part.c_str(), "events.xml") == 0)
					result.printables.insert(
						ParserResult::Printable::
							EVENTS_XML);
				else if (strcasecmp(part.c_str(),
						    "counters.xml") == 0)
					result.printables.insert(
						ParserResult::Printable::
							COUNTERS_XML);
	...
	const bool haveProcess = !result.mCaptureCommand.empty() ||
				 !result.mPids.empty() ||
				 result.mWaitForCommand != nullptr;

	// default to stopping on process exit unless user specified otherwise
	if (haveProcess &&
	    ((result.parameterSetFlag & USE_CMDLINE_ARG_STOP_GATOR) == 0)) {
	...
	if (result.mode == ExecutionMode::LOCAL_CAPTURE) {
		if (result.mAllowCommands) {
			logg.logError(
				"--allow-command is not applicable in local capture mode.");
			result.mode = ExecutionMode::EXIT;
			return;
		}
		if (result.port != DEFAULT_PORT) {
			logg.logError(
				"--port is not applicable in local capture mode");
			result.mode = ExecutionMode::EXIT;
			return;
		}

		if (!result.mSystemWide && result.mSessionXMLPath == nullptr &&
		    !haveProcess) {
			logg.logError(
				"In local capture mode, without --system-wide=yes, a process to profile must be specified with --session-xml, --app, --wait-process or --pid.");
			result.mode = ExecutionMode::EXIT;
			return;
		}
		...
		if (result.events.empty() &&
			...
			logg.logWarning(
				"No counters (--counters) specified, default counters will be used");
		}
	} else if (result.mode == ExecutionMode::DAEMON) {
		if (!result.mSystemWide && !result.mAllowCommands &&
		    !haveProcess) {
			logg.logError(
				"In daemon mode, without --system-wide=yes, a process to profile must be specified with --allow-command, --app, --wait-process or --pid.");
			result.mode = ExecutionMode::EXIT;
			return;
		}
		if (result.mSessionXMLPath != NULL) {
			logg.logError(
				"--session-xml is not applicable in daemon mode.");
			result.mode = ExecutionMode::EXIT;
			return;
		}
		if (!result.events.empty()) {
			logg.logError(
				"--counters is not applicable in daemon mode.");
			result.mode = ExecutionMode::EXIT;
			return;
		}
	}
	...
	if (result.mDuration < 0) {
		logg.logError(
			"Capture duration cannot be a negative value : %d ",
			result.mDuration);
		result.mode = ExecutionMode::EXIT;
		return;
	}
	...
	if ((indexApp > 0) && (result.mWaitForCommand != nullptr)) {
		logg.logError(
			"--app and --wait-process are mutually exclusive");
		result.mode = ExecutionMode::EXIT;
		return;
	}
	...
	if (optind < argc) {
		logg.logError(
			"Unknown argument: %s. Use --help to list valid arguments.",
			argv[optind]);
		result.mode = ExecutionMode::EXIT;
		return;
	}
...
static int findIndexOfArg(std::string arg_toCheck, int argc,
...
bool GatorCLIParser::hasDebugFlag(int argc, const char *const argv[])
{
	...
	for (int j = 1; j < argc; j++) {
		std::string arg(argv[j]);
		if (arg.compare(debugArgShort) == 0 ||
		    arg.compare(debugArgLong) == 0) {
