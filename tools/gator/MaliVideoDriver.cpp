...
class MaliVideoCounter : public DriverCounter {
    public:
	MaliVideoCounter(DriverCounter *next, const char *name,
			 const MaliVideoCounterType type, const int id)
...
void MaliVideoDriver::readEvents(mxml_node_t *const xml)
{
	...
	while (true) {
		...
		if (counter == NULL) {
			// Ignore
		} else if (strncmp(counter, COUNTER, sizeof(COUNTER) - 1) ==
			   0) {
			int i;
			if (!stringToInt(&i, counter + sizeof(COUNTER) - 1,
					 10)) {
				logg.logError(
					"The counter attribute of the Mali video counter %s is not an integer",
					counter);
				handleException();
			}
			setCounters(new MaliVideoCounter(getCounters(), counter,
							 MVCT_COUNTER, i));
		} else if (strncmp(counter, EVENT, sizeof(EVENT) - 1) == 0) {
			int i;
			if (!stringToInt(&i, counter + sizeof(EVENT) - 1, 10)) {
				logg.logError(
					"The event attribute of the Mali video counter %s is not an integer",
					counter);
				handleException();
			}
			setCounters(new MaliVideoCounter(getCounters(), counter,
							 MVCT_EVENT, i));
		} else if (strncmp(counter, ACTIVITY, sizeof(ACTIVITY) - 1) ==
			   0) {
			int i;
			if (!stringToInt(&i, counter + sizeof(ACTIVITY) - 1,
					 10)) {
				logg.logError(
					"The activity attribute of the Mali video counter %s is not an integer",
					counter);
				handleException();
			}
			setCounters(new MaliVideoCounter(getCounters(), counter,
...
void MaliVideoDriver::marshalEnable(const MaliVideoCounterType type,
				    char *const buf, int &pos)
{
	// size
	int numEnabled = 0;
	for (MaliVideoCounter *counter =
		     static_cast<MaliVideoCounter *>(getCounters());
	     counter != NULL;
...
bool MaliVideoDriver::start(const int mveUds)
{
	...
	buffer_utils::packInt(buf, pos,
			      MVE_INSTR_COUNTERS | MVE_INSTR_EVENTS |
				      MVE_INSTR_ACTIVITIES |
				      MVE_INSTR_PACKED_COMM);
