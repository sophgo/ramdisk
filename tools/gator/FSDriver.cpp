...
class FSCounter : public DriverCounter {
    public:
	FSCounter(DriverCounter *next, const char *name, char *path,
		  const char *regex);
	~FSCounter();
...
FSCounter::FSCounter(DriverCounter *next, const char *name, char *path,
		     const char *regex)
	: DriverCounter(next, name), mPath(path), mReg(),
...
int64_t FSCounter::read()
{
	...
	if (mUseRegex) {
		...
		if (match[1].rm_so < 0) {
			...
			if (errno != 0) {
				logg.logError("Parsing %s failed: %s", mPath,
					      strerror(errno));
				handleException();
			}
		}
...
void FSDriver::readEvents(mxml_node_t *const xml)
{
	...
	while (true) {
		...
		if (counter[0] == '/') {
			logg.logError(
				"Old style filesystem counter (%s) detected, please create a new unique counter value and move the filename into the path attribute, see events-Filesystem.xml for examples",
				counter);
			handleException();
		}
		...
		if (path == NULL) {
			logg.logError(
				"The filesystem counter %s is missing the required path attribute",
				counter);
			handleException();
		}
		const char *regex = mxmlElementGetAttr(node, "regex");
		setCounters(new FSCounter(getCounters(), counter, strdup(path),
					  regex));
	}
}
...
{
	int count = 0;
	for (FSCounter *counter = static_cast<FSCounter *>(getCounters());
	     counter != NULL;
