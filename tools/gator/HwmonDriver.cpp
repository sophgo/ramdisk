...
class HwmonCounter : public DriverCounter {
    public:
	HwmonCounter(DriverCounter *next, char *const name,
    ...
    private:
	void init(const sensors_chip_name *chip,
		  const sensors_feature *feature);
...
HwmonCounter::HwmonCounter(DriverCounter *next, char *const name,
			   const sensors_chip_name *const chip,
			   const sensors_feature *feature)
	: DriverCounter(next, name), mChip(chip), mFeature(feature),
	  mLabel(nullptr), mTitle(nullptr), mDisplay(nullptr),
	  mCounterClass(nullptr), mUnit(nullptr), mPreviousValue(0.0),
	  mMultiplier(1.0), mMonotonic(false), mDuplicate(false)
{
	...
	for (HwmonCounter *counter = static_cast<HwmonCounter *>(next);
	     counter != NULL;
	     counter = static_cast<HwmonCounter *>(counter->getNext())) {
		if (strcmp(mLabel, counter->getLabel()) == 0 &&
		    strcmp(mTitle, counter->getTitle()) == 0) {
			mDuplicate = true;
...
int64_t HwmonCounter::read()
{
	...
	subfeature = sensors_get_subfeature(mChip, mFeature,
					    getInput(mFeature->type));
	if (!subfeature) {
	...
	if (sensors_get_value(mChip, subfeature->number, &value) != 0) {
		logg.logError("Can't get input value for hwmon sensor %s",
			      mLabel);
		handleException();
	}
...
void HwmonDriver::readEvents(mxml_node_t *const)
{
	...
	while ((chip = sensors_get_detected_chips(NULL, &chip_nr))) {
		...
		while ((feature = sensors_get_features(chip, &feature_nr))) {
			...
			len = snprintf(NULL, 0, "hwmon_%s_%d_%d",
				       chip_name.get(), chip_nr,
				       feature->number) +
			      1;
			const std::unique_ptr<char[]> name{ new char[len] };
			snprintf(name.get(), len, "hwmon_%s_%d_%d",
				 chip_name.get(), chip_nr, feature->number);

			setCounters(new HwmonCounter(getCounters(), name.get(),
						     chip, feature));
		}
	}
}
...
void HwmonDriver::writeEvents(mxml_node_t *root) const
{
	...
	char buf[1024];
	for (HwmonCounter *counter = static_cast<HwmonCounter *>(getCounters());
	     counter != NULL;
	     counter = static_cast<HwmonCounter *>(counter->getNext())) {
		...
		if (counter->isDuplicate()) {
			mxmlElementSetAttrf(node, "name", "%s (0x%x)",
					    counter->getLabel(),
					    counter->getKey());
		} else {
		...
		snprintf(buf, sizeof(buf), "libsensors %s sensor %s (%s)",
			 counter->getTitle(), counter->getLabel(),
			 counter->getName());
...
{
	for (DriverCounter *counter = getCounters(); counter != NULL;
